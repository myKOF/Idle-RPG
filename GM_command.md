...
```text
【指令格式】
inv_cap 容量 （直接設定總背包容量 / 支援 + - 增減）
```

別名：`invcap`、`inv_size`、`bag_cap`、`capacity`、`inv_expand 增加量`

此指令可將背包容量擴充至任意數值（如 `2000`、`5000`），超越常規玩家的最大擴充上限 `1000` 格。

範例：

```text
inv_cap 2000 （將背包總容量直接設定為 2,000 格）
inv_cap +500 （在目前容量基礎上再增加 500 格）
