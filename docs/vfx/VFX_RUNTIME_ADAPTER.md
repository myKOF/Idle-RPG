...
震碎斬／三色飛行刀波以分段刀弧的 `offsetX/YOverLife` 推進外緣，成形後徑向厚度維持不變，切向長度沿圓周展開。`area.r` 指刀波前緣抵達範圍，不代表將原本整張粗刀光等比例膨脹填滿；前緣曲線仍來自 Skills2，中央已掃過處不留下傷害場域。迴旋斬專用 `SG_CLEAVE_WAVE_GAP_SEC` 同時控制傷害與事件 `delayMs`，目前使用者指定為 0.3 秒。
