...
## 5.6 P4 量測數據（2026-07-27 實測）

以 `?worker=1&measure=1` 開啟量測模式取得。量測預設關閉——為了算 payload 大小要多做一次
序列化，成本可能比訊息本身還高，開著量會扭曲被量測的對象。

測試條件：後期存檔 fixture（背包 800 件、等級 9999、存檔 317 KB）。

| 訊息 | 次數 | 平均 bytes | 最大 bytes | Worker postMs（最大） | 主執行緒分派 ms（最大） |
|---|---|---|---|---|---|
| `tick` | 78 | 929 | 2,465 | 0.1 | 0.3 |
| `panel` | 6 | 57,959 | **312,824** | 2.2 | **3.2** |
| `persist` | 2 | — | — | 0.4 | **5.9** |
| `booted` | 1 | 434 | 434 | 0 | 0.3 |

⚠️ `persist` 的 bytes 數字失真：payload 內的 `json` 已經是字串，量測時再 `JSON.stringify`
一次會因跳脫字元膨脹約一倍。實際傳輸量接近存檔大小 317 KB。時間數字不受影響。
...
**瓶頸已移到 `ui.js` 的渲染。** 以 PerformanceObserver 量 longtask：

- 穩態（停在裝備頁 20 秒、85 個 tick）：**超過 50 ms 的長工作 0 次**。遊戲操作是順的。
- 切頁時會卡，但只有特定頁籤：

  | 頁籤 | longtask | 出現頻率 | 面板 payload | 該頁 DOM 節點 |
  |---|---|---|---|---|
  | 裝備／背包 | **66～88 ms** | 每次必現（7/7） | 140 KB（`inv`） | 3,029 |
  | 天賦 | 65～81 ms | 間歇（3/5） | 1 KB | 1 → 展開後建樹 |
  | 技能 | 無 | 0/5（冷熱渲染皆無） | 0 KB | 283 |
  | 寶石 | 無 | 0/2 | 2 KB | 1,141 |

裝備頁是唯一每次必現的，66～88 ms 與 3,029 個節點（800 格背包）相稱，屬於單純的
DOM 量體問題——虛擬捲動或分頁可解。天賦頁間歇出現，成本應在天賦樹的首次建構。

⚠️ **本表更正了初版的錯誤結論。** 初版寫「技能頁 135 ms 最慢、payload 0 KB、
成本在重算」，是 longtask 歸屬算錯：那筆 135 ms 發生在切到技能頁的 **388 ms 之後**，
而實測渲染都落在切頁後 92～187 ms 內，時間對不上。以冷渲染（載入後第一個切到技能頁）
與暖渲染各重測，開／關 `measure=1` 都測過，技能頁**從未產生 longtask**。
歸屬邏輯若只取「最近一次切頁」而不檢查間隔，就會把無主的長工作掛到錯的頁籤上。

P4 的工作落在 Codex 的 `ui.js` 渲染節流那一欄，Worker 端暫時沒有可收斂的空間。

**已做但無效的嘗試**：`8f3d105` 把 `switchTab` 的頁籤 class 更新從
`querySelectorAll(...).forEach(toggle)` 改成只動前後兩個元素。頁面內直接對拍
兩種寫法各 200 次：舊 0.0405 ms/次、新 0.0015 ms/次，**每次切頁省下 0.039 ms**，
相對 66～88 ms 的 longtask 約為 0.05%。改動本身正確（`.tab-btn` 與 `.tab` 各 8 個，
`switchTab` 是 `active` class 的唯一寫入者，不會殘留多重 active），但沒有觸及成本所在。

### 5.6.2 背包虛擬捲動的成效（`4dcb431`，2026-07-27 實測）

同一份 `save_lategame.json`，`?worker=1`：

| 指標 | 虛擬化前 | 虛擬化後 |
|---|---|---|
| 切到裝備頁的 longtask | 66～88 ms，7/7 每次必現 | **0**（6 次切頁、400 ms 內皆無） |
| `#inventory-grid` 子節點 | 800 格 + 容器 ≈ 3,029 節點 | 18 格 + 1 spacer = **19** |
| 捲動視窗正確性 | — | 800 件 / 6 欄 = 134 列，切片與 spacer 高度皆正確 |

只在 `workerUiStateEnabled()` 時啟用，舊路徑維持整份渲染。
...
- 2026-07-27　**數值權威順序**（使用者裁決）：
  ...
  文檔沒寫或寫錯則修正文檔。
  查證結果：`node tools/apply_params.cjs` 顯示 496 個參數中 492 一致、**0 將變更**，
  即程式碼已完全符合 CSV，爭議批次不需改任何 `js/`；只需更新測試與文檔。
  ⚠️ 本裁決只適用於**數值爭議**，不適用於功能缺失或行為錯誤
  （那類仍需個別裁決，不得用改測試的方式合理化）。
