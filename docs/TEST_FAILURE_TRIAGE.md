...
下表保留裁決前的差異紀錄。2026-07-27 使用者已裁決數值權威順序為：
`config/CSV/game_parameters.csv` ＞ 程式碼寫死值 ＞ 公式文檔；因此本表的 CSV 現值已成為測試與文檔更新基準。
...
- 有 CSV 的數值以 CSV 為準；沒有 CSV 才以程式碼為準。
- 本批程式碼已與 CSV 一致，只更新落後的測試與文檔，不修改 `js/`、CSV 或 Excel。
- 此裁決不適用於缺失功能；`gemAttrDmgBaseV1`、gem-convert Shift 與 save-folder focus/rescan 案例維持 B 類。
