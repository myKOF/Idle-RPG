...
閃避一路線性成長，角色命中卻靠詞條一條一條加，於是線上收益一路衰減到 0.16 倍。
關卡 188 的普通怪要 1,925 次命中才打得死，實測每分鐘擊殺數 0～1。
...
閃避甩開命中那一條還在：關卡 188 的有效命中率仍只有 22.2%，線上收益相對關卡 90
仍衰減到 0.16 倍。這是為什麼 15 隻/分那一檔的關卡中位數停在 150——
那正是有效命中率開始塌掉的地方。要讓後期關卡值得推，得壓平
`4-野外怪物／閃避率` 的成長曲線，或讓命中率詞條的成長跟得上。
