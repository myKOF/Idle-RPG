...
>
> 三種修法，由小到大：
> 1. 離線擊殺數取 `min(固定費率, 實際打得動的速率)`——用 `血量 ÷ (攻擊 × 出手速度 ×
>    有效命中率)` 算真實 TTK 再取 `max(20秒, TTK)`。離線就永遠不會超過線上做得到的事，
>    而且前期完全不受影響
> 2. 離線關卡改吃當前關卡而非 `G.stage.best`——推得深不再是免費棘輪
> 3. 壓平怪物閃避曲線，讓有效命中率不隨關卡塌掉——同時修好線上，是治本
