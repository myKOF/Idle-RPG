...
## 產出閘門：先全輸，修好遊戲之後變成最大的一筆改善

> **結論先講**：六份策略都已啟用，門檻 15 隻/分。
> 重度策略中位等級 **118 → 299.5**、死亡數 108 → 41。
>
> 但它一開始是**全輸**的，而輸的原因不在策略，在遊戲的離線結算公式。
> 這一節照時間順序寫，因為那個順序本身就是結論：
> **策略調不動的時候，要先確認遊戲有沒有給錯誘因。**
...
改動在 `js/formula.js §10` 與 `js/combat.js`：野外擊殺時記錄實測速率，
離線結算時 `間隔 = max(固定費率 20 秒, 實測間隔)`。

```
G.stage.killPace = { kills, sec, at }   // 指數加權的擊殺數／觀測時長／最後更新牆鐘時刻
offlineKillIntervalFor(pace, savedAt)   // 空窗期算進分母、同步衰減分子
```

⚠️ **刻意用實測而不是用血量除以傷害去推算**。推算會漏掉技能、連擊、範圍攻擊，
實測比對顯示只算普攻會低估實際輸出約 5 倍——那等於全面砍離線收益，不是設下限。

三個必須處理的邊界（各有測試，`tests/offline-kill-pace.test.cjs`）：

| 邊界 | 不處理會怎樣 |
| :--- | :--- |
| 暖機：分子分母要分開衰減 | 只維護「速率的移動平均」要 tau 才暖機完，開局 120 秒低估 5 倍 |
| 空窗：存檔前沒擊殺的時間算進分母 | 停在打不動的關卡整晚，離線仍照著八小時前的速率發獎 |
| 離線回來：結算後把時間錨點移到當下 | 回來打的第一隻怪會把整段離線時間算成「打不動」，之後再也領不到離線收益 |

舊存檔與新角色的預設值就是固定費率 `{ kills: 1, sec: 20, at: 0 }`，
所以升級這一版**不會**讓任何人的離線收益先掉一截；等真的打過怪才換成實測值。
彈窗也會在被壓速時明講原因，否則玩家只會看到收益變少而不知道該退關。

### 第二輪實測：同樣的閘門，這次是最大的一筆改善

離線修好之後重跑同一組 seed（重度、100 小時、4 個 seed）：
...
**先前那個 22.2% 是漏洞造成的**：AI 停在關卡 188 掛機，裝備從此不再更新
（面板命中卡在 163），於是看起來像數值曲線的問題。刷關恢復之後，
裝備升級與缺口洗煉一起把命中頂上去，`4-野外怪物／閃避率` 完全不必動。

⚠️ 這是**第三次**在同一件事上改歸因（先怪離線太強、再怪深度主導、再怪閃避曲線）。
前兩次錯在只看了症狀所在的那一份存檔。教訓是：**在被漏洞扭曲的狀態下量到的任何
數值曲線問題，都要在漏洞修好之後重量一次**——症狀會跟著消失，而在那之前
所有歸因都不可信。

### 現在真正的節奏限制

沒有牆，也沒有停滯：兩場重度到 100 小時仍在往前推（60h 關卡 135 → 80h 145 → 100h 154），
物攻 372 → 176,368。限制是一般的跑步機——怪物血量每階 ×1.06，
要維持 15 隻/分，傷害就得同步成長。後期大約每 20 牆鐘小時推進 10 關。
這是節奏設計問題，不是失衡；要調快就調 `4-野外怪物／生命` 的指數底，或加快裝備取得。

### 目前的設定與哨兵

六份策略都掛上了 `requireTargets: ["killRate"]` 與 `targetRetreat`，門檻 15 隻/分。
要換成別的取捨只需改 `atLeast` 一個數字（上表是各檔的實測成績）。
