...
### 為什麼：關卡深度的價值是指數，殺敵速度補不回來

先看最能說明問題的一組數字（單殺經驗取自 `monsterStatsFor`，速率取自實測剖面）：

| | 單殺經驗 | 速率 | 經驗/時 |
| :--- | ---: | ---: | ---: |
| 關卡 130 快速刷 | 459,618 | 8 隻/分 | 220,616,436 |
| 關卡 188 幾乎打不動 | 16,624,459 | **0.5 隻/分** | **498,733,770** |

**站在打不動的關卡、每兩分鐘才殺一隻，經驗仍是快速刷關卡 130 的 2.3 倍。**
怪物經驗每關約 ×1.065（40 關就 ×13，指數），而升級所需經驗是 `≈ 20 × 等級³`
（多項式）——速率掉 16 倍完全補不回深度多 58 關。**這一條線上離線都成立**，
所以「退回去刷得快一點」在任何時段都是虧的。

離線再把這件事放大兩層：`applyOfflineProgress`（js/save.js）算的是
`offlineStageFor(G.stage.best)` 那一關的**菁英怪**（普通怪的 2 倍經驗），每 20 秒 1 隻
——**擊殺數固定，與關卡無關**；而且它吃的是**歷史最高**而不是當前關卡，退關也不會掉。
班表下離線佔 2/3 的時間。單殺價值：
