...
> **結論先講**：這套機制已實作、有測試，但**四份強度策略都沒有啟用**。
> 實測它會讓成績變差，原因寫在下面。要啟用只需在 `stageGate` 加兩個欄位。
...
### 為什麼：離線收益把「推得深」的價值推成指數

`applyOfflineProgress`（js/save.js）算的是 `offlineStageFor(G.stage.best)` 那一關的
**菁英怪**，每 20 秒 1 隻——**擊殺數固定，與關卡無關**。而單殺價值：
...
單殺經驗**指數成長**，升級所需經驗只是**多項式成長**，於是每深 10 關，一個離線夜的
價值大約翻倍。班表下 16/24 小時是離線，所以 `stage.best` 是壓倒性的主要收入來源。

退關本身不會讓 `best` 縮水（`stageGo` 只改 `current`，`best` 是單向棘輪，
js/combat.js:801），但**重爬要花在線時間**，而在線時間換到的擊殺是線性收益。
拿指數的東西去換線性的東西，換多少都是虧的。

> 這也是這場實驗真正的產出：**模擬器找到了一個設計大概沒打算給的最佳解**——
> 「推到推不動為止然後掛機」。AI 停在關卡 188 一隻都殺不掉看起來很蠢，其實是對的。
> 若要讓線上經營有意義，得先把離線經驗曲線壓平，或讓離線收益也吃當前關卡而非歷史最高。

### 保留下來的東西

- `ratePerMin` 目標種類與 `stageGate.requireTargets` / `targetRetreat`：機制與測試都在，
  離線經濟一改就能用（`tests/sim-policy-targets.test.cjs`）。
- 若要的是**穩定**而非峰值，`atLeast: 10` 是現成選項：最差的一次從 Lv.86 拉到 Lv.147、
  關卡 109 拉到 131，死亡數從 108 降到 74，代價是最高只到關卡 150。
- 三支哨兵（`tests/policy-keys.test.cjs`）：`requireTargets` 必須指到宣告過的目標、
  退關間隔不得短於取樣視窗、計數器路徑必須在真實面板裡解得出來。
