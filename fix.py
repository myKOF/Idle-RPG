...
content = re.sub(
    r"var iconClass = m\.isBoss \? 'cb-icon boss' : 'cb-icon monster';\s+var mvImg = \$id\('mv-emoji'\)\.querySelector\('img'\);\s+var targetSrc = 'images/icon_avatar\.png';\s+if \(!mvImg\) \{[^}]+}\s+} else \{[^}]+}[^}]+}\s+if \(mvImg\.className !== iconClass\) mvImg\.className = iconClass;\s+}",
...
content = re.sub(
    r"var mvImgEmpty = \$id\('mv-emoji'\)\.querySelector\('img'\);\s+var targetSrcEmpty = 'images/icon_avatar\.png';\s+if \(!mvImgEmpty\) \{[^}]+}\s+} else \{[^}]+}[^}]+}\s+if \(mvImgEmpty\.className !== 'cb-icon'\) mvImgEmpty\.className = 'cb-icon';\s+}",
...
content = re.sub(
    r"var bossIcon = bd\.img \? 'images/' \+ bd\.img : 'images/icon_avatar\.png';\s+h \+= '<div class=\"tower-floor' \+ \(cleared \? ' cleared' : ''\) \+ \(unlocked \? '' : ' locked'\) \+ '\" data-tower-tip=\"' \+ fl \+ '\">' \+\s+'<span class=\"tf-emoji\" style=\"margin-right:12px;\"><img src=\"' \+ bossIcon \+ '\" style=\"width:32px;height:32px;vertical-align:middle;border-radius:4px;box-shadow:0 0 5px #000;\"></span>' \+\s+'<span class=\"tf-name\" style=\"vertical-align:middle;\">第 ' \+ fl \+ ' 層・' \+ bd\.name \+ \(cleared \? ' ✅' : ''\) \+ '</span>' \+",
...
content = re.sub(
    r"var bossImgSrc = TOWER\.boss\.img \? 'images/' \+ TOWER\.boss\.img : 'images/icon_avatar\.png';\s+var tbImg = \$id\('tb-emoji'\)\.querySelector\('img'\);\s+if \(!tbImg\) \{[^}]+}\s+} else \{[^}]+}[^}]+}\s+if \(tbImg\.className !== 'cb-icon boss'\) tbImg\.className = 'cb-icon boss';\s+}",
