...
function newForgeDefaultFurnace(id, previousFurnace) {
  ...
  return {
    ...
    parts: []                                    // 已置入零件 key/tier（升級時由 newForgeUpgradePart 同步更新）
  };
}
...
function newGameState() {
  ...
  return {
    ...
    zoneProgress: {
      ...
      god_chaos: { current: 1, best: 1, cleared: 0 },
      god_sanctuary: { current: 1, best: 1, cleared: 0 }
    },
    factory: {
    ...
    forge: {  // 神鑄系統：六芒星槽位 / 六格魔塵符位 / 自動魔塵 / 自動鑄造 / 等待狀態 / 上次產物 / 法陣紀錄
      ...
      autoDust: false, result: null, log: [], unlockNotified: false, unlocked: false,
      autoFill: null, autoForge: true, crafting: null
    },
