...
var PART_TYPES = {
  ...
  extractLens:   { name: '精粹透鏡',   emoji: '🔬', node: 'salvage', desc: '精粹提取機率 +{v}%', perTier: 3 },
  essenceCoil:   { name: '精華凝結線圈', emoji: '🧪', node: 'salvage', desc: '精粹提取時額外精華 +{v}', perTier: 1 },
  gemSieve:      { name: '寶石篩選器', emoji: '💎', node: 'salvage', desc: '分解時 {v}% 機率額外掉落 1 級寶石', perTier: 0.6 },
  gemPurifier:   { name: '寶石提純器', emoji: '🔷', node: 'salvage', desc: '精粹提取的寶石每級 {v}% 機率再提升 1 級', perTier: 6 },
...
var PART_SLOTS = { synth: 2 }; // 各節點零件槽數（分解槽改為依等級動態，見 formula.js salvageSlotCount）
function slotsForNode(node) {
  if (node === 'salvage') return salvageSlotCount(); // 分解槽：初始 2、每 500 級 +1、上限 10
  return PART_SLOTS[node] || PART_SLOTS_PER_NODE;
}
var PART_KEEP_PER_KEY = 3;     // 零件庫存收斂：每種零件未安裝者最多保留幾顆（避免 parts 陣列無限成長）
...
var GEM_SHOP_TABLE = [
  ...
  { lv: 7, price: 8000000 },
  { lv: 8, price: 80000000 },
  { lv: 9, price: 800000000 },
  { lv: 10, price: 8000000000 }
];
...
var FIELD_DROP_TABLE = [   // 野外：依怪物等級
  { min: 150, rates: [0,  0,  25, 15, 5, 2, 0.25, 0] },
