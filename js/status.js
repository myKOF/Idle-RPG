...
var STATUS = {
  ...
  sgPoison: { name: '血刃劇毒', icon: '☠️', kind: 'debuff', effect: 'dot', key: '', elem: 'poison', dmgSource: 'skill', dmg: 25, capStat: '', capMult: 0, val: 0, dur: 4, interval: 0.5, stack: 'strongest', maxStacks: 1, desc: '新版技能【血毒刃】：毒屬性持續傷害；每跳量與間隔由技能各階決定（引擎以 dps 覆寫）。', vfx: { apply: 'curse-poison', tick: 'hit-poison' } },
  ...
  sgStorm: { name: '暴風化身', icon: '🌪️', kind: 'buff', effect: 'stat', key: 'sgStorm', elem: '', dmgSource: '', dmg: 0, capStat: '', capMult: 0, val: 0, dur: 3, interval: 0, stack: 'refresh', maxStacks: 1, desc: '新版技能【暴風亂舞】：化身暴風，期間自動施放雙刀亂舞、可以普攻及施放技能。', vfx: { aura: 'ground-storm-dance' } },
  sgBloodrage: { name: '嗜血狂怒', icon: '💢', kind: 'buff', effect: 'stat', key: 'sgBloodrage', elem: '', dmgSource: '', dmg: 0, capStat: '', capMult: 0, val: 20, dur: 8, interval: 0, stack: 'refresh', maxStacks: 1, desc: '新版技能【嗜血狂怒】：攻速乘算提高（突破攻速上限）；期間各階狂怒效果生效。', vfx: { aura: 'aura-bloodrage' } },
  ...
  sgBurn: { name: '烈焰燃燒', icon: '🔥', kind: 'debuff', effect: 'dot', key: '', elem: 'fire', dmgSource: 'skill', dmg: 20, capStat: '', capMult: 0, val: 0, dur: 5, interval: 0.5, stack: 'strongest', maxStacks: 1, desc: '新版技能【火球術／火柱】：火屬性持續傷害；每跳量與間隔由技能各階決定（引擎以 dps 覆寫）。', vfx: { tick: 'st-tick-fire' } },
  ...
  sgFrost: { name: '寒霜', icon: '❄️', kind: 'debuff', effect: 'stat', key: 'sgFrost', elem: 'ice', dmgSource: '', dmg: 0, capStat: '', capMult: 0, val: 20, dur: 5, interval: 0, stack: 'stack', maxStacks: 5, desc: '新版技能【寒霜狀態】的層數與緩速：每疊 1 層使移動速度下降（效果值＝單層下降% × 層數），攻速倍率則為（1 − 單層下降%）^層數，疊滿層數時凍結。比照泥沼緩速為場域型重塗，緩速本身不吃控場遞減與韌性。' },
  sgFrostBite: { name: '寒霜凍傷', icon: '🧊', kind: 'debuff', effect: 'dot', key: '', elem: 'ice', dmgSource: 'skill', dmg: 50, capStat: '', capMult: 0, val: 0, dur: 5, interval: 0.5, stack: 'strongest', maxStacks: 1, desc: '新版技能【寒霜狀態】的寒冰持續傷害；每跳量＝施放群組本體傷害 B × 寒霜共用倍率 ×（50×目前寒霜總層數＋該來源寒霜階每級增量×該階等級）%，總層數包含所有技能來源（引擎以 dps 覆寫）。', vfx: { tick: 'st-tick-ice' } },
  ...
  sgWindRend: { name: '風切', icon: '💨', kind: 'debuff', effect: 'stat', key: 'sgWindRend', elem: 'wind', dmgSource: '', dmg: 0, capStat: '', capMult: 0, val: 80, dur: 4, interval: 0, stack: 'stack', maxStacks: 6, desc: '新版技能【風切狀態】的減益本體：移動速度下降（效果值＝單層下降%，不隨層數再加深）且命中率下降；層數只影響風切割裂的每跳傷害，當下允許的層數上限由真空斬【無限風切】與傳奇【裂痕】決定（表上的上限只是天花板）。' },
  sgWindCut: { name: '風切割裂', icon: '💨', kind: 'debuff', effect: 'dot', key: '', elem: 'wind', dmgSource: 'skill', dmg: 50, capStat: '', capMult: 0, val: 0, dur: 4, interval: 0.5, stack: 'strongest', maxStacks: 1, desc: '新版技能【風切狀態】的風系持續傷害；每跳量占施放群組的本體技能傷害並隨風切層數提高（引擎以 dps 覆寫）。' },
  ...
  sgStormBarrier: { name: '暴風屏障', icon: '🌪️', kind: 'buff', effect: 'stat', key: 'sgStormBarrier', elem: 'wind', dmgSource: '', dmg: 0, capStat: '', capMult: 0, val: 10, dur: 8, interval: 0, stack: 'refresh', maxStacks: 1, desc: '新版技能【暴風屏障】：屏障的剩餘時間與傷害減免（效果值＝減免%）；第 2～6 階的效果只在此期間生效（護盾由節拍逐次給予）。', vfx: { aura: 'ground-storm-barrier' } },
  ...
  sgStormGod: { name: '暴風神體', icon: '⛈️', kind: 'buff', effect: 'stat', key: 'sgStormGod', elem: 'wind', dmgSource: '', dmg: 0, capStat: '', capMult: 0, val: 99, dur: 2, interval: 0, stack: 'refresh', maxStacks: 1, desc: '新版技能【暴風神體】：風暴之神附體期間的傷害減免（效果值＝減免%），且風系傷害額外乘算提高。與暴風屏障是分開的兩個狀態。', vfx: { aura: 'ground-storm-god' } },
  ...
  sgDeathReaper: { name: '死亡收割', icon: '☠️', kind: 'buff', effect: 'stat', key: 'sgDeathReaper', elem: '', dmgSource: '', dmg: 0, capStat: '', capMult: 0, val: 25, dur: 8, interval: 0, stack: 'stack', maxStacks: 20, desc: '超神進化【死亡收割者】：飛刀每殺死 1 個敵人疊 1 層，每層使你造成的所有傷害提高（效果值＝單層提高%）；層數上限與單層值由技能參數決定。' },
  sgKnifeWaltz: { name: '輪舞刃', icon: '🔪', kind: 'buff', effect: 'stat', key: 'sgKnifeWaltz', elem: '', dmgSource: '', dmg: 0, capStat: '', capMult: 0, val: 0, dur: 5, interval: 0, stack: 'refresh', maxStacks: 1, desc: '傳奇特效【輪舞刃】：繞行在你周圍的飛刀還會留在場上多久（環繞場域的剩餘時間投影，比照火狩）。' },
  sgSlayerMark: { name: '殺神', icon: '⚰️', kind: 'buff', effect: 'stat', key: 'sgSlayerMark', elem: '', dmgSource: '', dmg: 0, capStat: '', capMult: 0, val: 2, dur: 6, interval: 0, stack: 'stack', maxStacks: 100, desc: '超神進化【殺神領域】：領域內的敵人死亡時疊 1 層，每層使你造成的所有傷害提高（效果值＝單層% × 層數）。' },
  ...
  sgDeathDefer: { name: '不屈之誓', icon: '🕯️', kind: 'buff', effect: 'stat', key: 'sgDeathDefer', elem: '', dmgSource: '', dmg: 0, capStat: '', capMult: 0, val: 100, dur: 10, interval: 0, stack: 'refresh', maxStacks: 1, desc: '傳奇【不屈之誓】：死亡延後生效的期間，你造成的所有傷害提高。', vfx: { aura: 'ground-cyclone-avatar' } },
  ...
  sgCounterWrath: { name: '怒火', icon: '😡', kind: 'buff', effect: 'stat', key: 'sgCounterWrath', elem: '', dmgSource: '', dmg: 0, capStat: '', capMult: 0, val: 30, dur: 6, interval: 0, stack: 'refresh', maxStacks: 1, desc: '傳奇【怒火】：反擊累積達門檻時獲得，期間你造成的所有傷害提高。' },
  sgBurnBlood: { name: '燃血', icon: '🩸', kind: 'buff', effect: 'stat', key: 'sgBurnBlood', elem: '', dmgSource: '', dmg: 0, capStat: '', capMult: 0, val: 1, dur: 4, interval: 0, stack: 'stack', maxStacks: 50, desc: '傳奇【燃血】：【血飲術】每造成 1 次自身生命損失疊 1 層，每層使你造成的所有傷害提高（效果值＝單層% × 層數）。' },
  ...
  sgAsuraFist: { name: '阿修羅霸王拳', icon: '👊', kind: 'buff', effect: 'stat', key: 'sgAsuraFist', elem: '', dmgSource: '', dmg: 0, capStat: '', capMult: 0, val: 500, dur: 1, interval: 0, stack: 'refresh', maxStacks: 1, desc: '超神進化【阿修羅霸王拳】：每隔一段時間自動發動，短暫期間內你造成的所有傷害大幅提高。', vfx: { aura: 'aura-bloodrage' } },
  ...
  sgPetrify: { name: '石化', icon: '🗿', kind: 'debuff', effect: 'stat', key: 'sgPetrify', elem: 'earth', dmgSource: '', dmg: 0, capStat: '', capMult: 0, val: 400, dur: 4, interval: 0, stack: 'refresh', maxStacks: 1, desc: '超神進化【超重岩之術】：被壓縮的巨岩之力封住的敵人。行動限制本身由「暈眩」承擔（因此完整沿用 BOSS 控場免疫、韌性折減與控場遞減），本狀態只負責「受到的土系傷害額外提高」（效果值＝提高%）。' },
  sgStiffen: { name: '僵化', icon: '🌐', kind: 'debuff', effect: 'stat', key: 'sgStiffen', elem: 'earth', dmgSource: '', dmg: 0, capStat: '', capMult: 0, val: 65, dur: 5, interval: 0, stack: 'refresh', maxStacks: 1, desc: '超神進化【超重力場】：重力場扭曲造成的僵化。移動速度、攻速與造成的傷害同時下降（效果值＝三者共用的下降%）；比照【泥沼緩速】不吃控場遞減與韌性。' },
  sgMireBleed: { name: '泥沼裂傷', icon: '🩸', kind: 'debuff', effect: 'dot', key: '', elem: '', dmgSource: 'skill', dmg: 150, capStat: '', capMult: 0, val: 0, dur: 5, interval: 0.5, stack: 'strongest', maxStacks: 1, desc: '傳奇特效【侵蝕】：泥沼術附加的流血；每跳量與間隔由特效參數決定（引擎以 dps 覆寫）。比照【穿心裂血】採「取高並重新計時」，因此每一拍重塗不會把剩餘時間鎖死在滿值。' },
  sgPlague: { name: '惡疫', icon: '🦠', kind: 'debuff', effect: 'dot', key: '', elem: 'poison', dmgSource: 'skill', dmg: 200, capStat: '', capMult: 0, val: 0, dur: 8, interval: 0.35, stack: 'strongest', maxStacks: 1, desc: '超神進化【惡疫魔沼】：沼澤塗上的毒性持續傷害；每跳量與間隔由超神進化參數決定（引擎以 dps 覆寫）。與泥沼的其他狀態不同，離開沼澤後仍會走完自己的持續時間，因此不採「只給兩跳」的重塗規則。' },
  sgInferno: { name: '火獄烙印', icon: '🌋', kind: 'debuff', effect: 'stat', key: 'sgInferno', elem: 'fire', dmgSource: '', dmg: 0, capStat: '', capMult: 0, val: 1, dur: 8, interval: 0, stack: 'refresh', maxStacks: 1, desc: '超神進化【深淵火獄】：敵人的屬性標籤強制改為火屬性（效果值本身沒有意義，比照【地爆天星】只是一個計時容器）。屬性改寫走 skill2ForcedAttr，與【寒冰逆轉】同一個掛點。' },
  ...
  sgGravityDomain: { name: '超重力場', icon: '🌐', kind: 'buff', effect: 'stat', key: 'sgGravityDomain', elem: 'earth', dmgSource: '', dmg: 0, capStat: '', capMult: 0, val: 0, dur: 10, interval: 0, stack: 'refresh', maxStacks: 1, desc: '超神進化【超重力場】：岩甲護盾存在期間展開的重力場，進入的敵人僵化；持續特效依領域半徑縮放、跟著你移動。持續到岩甲護盾結束（實際秒數由岩甲術給）。', vfx: { aura: 'ground-domain-earth-10' } },
...
/* 狀態的特效欄位（2026-09-03 VFX Preset 化）：st.vfx = { apply 施加, aura 持續, tick 作用 }，
   ...
   （2026-09-18：哪個技能附加這個狀態，看起來都一樣）。
   statusVfxRoles(sid, 'apply'／'tick') 回傳可直接掛在 VFX 事件上的角色表 { hit: <presetId> }：
   施加與作用都是「落在目標身上的一下」，語意就是受擊（身體中心）；持續特效由顯示層依面板快照
   掛在腳底（js/vfx-runtime.js syncStatuses）。沒填回傳 null。 */
...
function statusVfxRoles(sid, role) {
  ...
  if (!id) return null;
  return role === 'aura' ? { ground: id } : { hit: id };
}
...
function applyStatus(ent, sid, ctx) {
  ...
  /* 施加特效只在「第一次出現在目標身上」時播（重新計時／疊層不重播）：先記下施加前在不在。 */
