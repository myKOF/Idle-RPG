...
var STATUS = {
  ...
  /* 風系（2026-08-18 技能改造第八批）：【風切狀態】同樣拆成「疊層減益」＋「持續傷害」兩筆，
     理由與寒霜相同（一筆狀態不能同時是 dot 與 stat）。差別在傷害**要**隨層數提高
     （設計文檔【無限風切】明寫「每堆疊 1 次會額外受到 50% 風系傷害」），
     因此每跳量由引擎依當下層數重算後以 dps 覆寫，疊加規則取 strongest。
     命中率折減與移速折減是同一筆風切的兩個數字，狀態表只有一個 val 欄：
     val＝移速下降%（比照泥沼緩速），命中下降%由技能表 fx.hit 提供（skill2WindRendHitFactor）。 */
  sgWindRend: { name: '風切', icon: '🌀', kind: 'debuff', effect: 'stat', key: 'sgWindRend', elem: 'wind', dmgSource: '', dmg: 0, capStat: '', capMult: 0, val: 80, dur: 4, interval: 0, stack: 'stack', maxStacks: 3, desc: '新版技能【風切狀態】的層數與減益：移動速度下降（效果值＝下降%）、命中率下降，層數上限由【無限風切】開放。比照泥沼緩速為技能型減益，不吃控場遞減與韌性。' },
