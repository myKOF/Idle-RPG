...
function reincarnationExpMultiplier(count) {
  ...
  if (typeof REINCARNATION_EXP_MULTIPLIERS !== 'undefined' && REINCARNATION_EXP_MULTIPLIERS[n] !== undefined) {
    return Number(REINCARNATION_EXP_MULTIPLIERS[n]) || 1;
  }
  return Math.pow(10, Math.min(10, n));
}
...
// 屬性傷害提升 key（elemDmgFire...）→ 元素 key（2026-07-23 新增：提升「自身」該屬性元素傷害輸出）
function affixElemDmgUp(key) {
  if (!/^elemDmg[A-Z]/.test(key)) return null;
  var e = key.slice(7);
  e = e.charAt(0).toLowerCase() + e.slice(1);
  return ELEMENTS.indexOf(e) >= 0 ? e : null;
}
...
/* 防禦減傷率 = (1 + max(0, 同類型攻擊 - 防禦)) ×
   防禦 / (防禦 + 常數 + 每級係數 × 攻擊者等級)

   物理攻擊只與物理防禦比較，魔法攻擊只與魔法防禦比較；攻防差值不會為負。
...
function defReduction(def, attackerLevel, attackerAttack) {
...
/* ---- 元素特效觸發參數 ----
   技能屬性化段與元素附加段合併後，每系只判定一次（避免同屬性重複觸發）。
   2026-07-26 技能屬性化改造後，這些係數的作用基數由「元素附加值」放大為
   「技能整段屬性傷害」，數值集中於此處供調整。 */
var ELEM_PROC = {
  iceSlowChance: 15,        // 冰：觸發減速機率%
  iceSlowDur: 2,            // 冰：減速秒數（再乘控場遞減 ccFactor）
  lightningChance: 10,      // 雷：觸發連鎖電擊機率%
  lightningChainMult: 0.8,  // 雷：連鎖電擊追加傷害＝該系元素傷害 × 此倍率
  poisonChance: 25,         // 毒：觸發中毒機率%
...
/* ---- 穿透 → 忽略防禦（2026-08-06 改版：飽和曲線）----
   忽略敵人防禦比率 = 穿透倍率 ÷ (穿透倍率 + a)
     穿透倍率 = 總穿透加成% ÷ 100（穿透 350% → 3.5）
   a 由參數表「3-戰鬥核心／穿透忽略防禦」控制（a 越大越難忽略防禦），因此下面不列
   對照數字——調 a 就會全錯。與 a 無關的性質只有一條：穿透 = a×100% 時剛好忽略 50%。
...
function penIgnoreRatio(penPct) {
  ...
  if (rate <= 0) return 0;
  return clamp(rate / (rate + PEN_IGNORE_A), 0, 1);   // a 設成 0/負值的異常參數夾回 [0,1]
}
// 面板／說明用：實際忽略防禦%（趨近但到不了 100%）
...
function resolveHit(attacker, defender, aCfg, dCfg) {
  ...
  if (aCfg.dmgType !== 'magic') {
    ...
    var pDmg = (aCfg.atk || 0) * (1 - defReduction(pDef, aCfg.level || 1, aCfg.atk));
  ...
  if (aCfg.dmgType === 'magic' || aCfg.dmgType === 'both') {
    ...
    var mDmg = baseMAtk * (1 - defReduction(mDef, aCfg.level || 1, baseMAtk));
  ...
  var skillElemParts = null;       // {屬性鍵: 該系傷害值}；供元素特效與後續縮放使用
  var skillElemBase = 0;           // 屬性化後、浮動與暴擊前的合計值（縮放基準）
  var skillMix = skillElemMixOf(aCfg);
  if (skillMix && dmg > 0) {
    var sRes = dCfg.resist || {};
    var sLv = aCfg.level || 1;
    skillElemParts = {};
  ...
  if (skillElemParts && skillElemBase > 0) {
    var sScale = dmg / skillElemBase;
    for (var sk2 in skillElemParts) skillElemParts[sk2] *= sScale;
  }
  // 元素附加（各自受對應抗性影響）
  // 每系元素值 = 固定值元素攻擊 + 附傷基底 × 天賦附傷%；附傷基底 = 元素附加前的當次傷害（含防禦/抗性/浮動/暴擊）
  var elemTotals = skillElemParts;   // 技能屬性化段先入列，稍後與附加段合併判定元素特效
  ...
  if (elem || elemPct) {
    ...
    if (dCfg.isPlayer) {
      ...
      elemMagicMit = (1 - defReduction(elemMDef, aCfg.level || 1, elemMagicAttack)) *
    ...
    for (var i = 0; i < ELEMENTS.length; i++) {
      ...
      if (!ev) continue;
      // 屬性傷害提升：自身該屬性元素傷害合計 ×(1+%)（元素特效的衍生值——連鎖電擊/中毒/暗影汲取——一併放大）
      if (aCfg.elemDmgUp && aCfg.elemDmgUp[ek]) ev *= 1 + aCfg.elemDmgUp[ek] / 100;
      var edmg = ev * elemMagicMit * elementalResistanceMultiplier(res, ek, aCfg.level || 1);
      dmg += edmg;
      if (!elemTotals) elemTotals = {};
      elemTotals[ek] = (elemTotals[ek] || 0) + edmg;
    }
  }
...
/* ---- 高塔 BOSS 數值（三塔獨立參數） ----
   ...
   每塔各自一整套：對應野外階段、等級加值、命中、生命/攻擊/防禦倍率（直接乘基準值、不鏈乘）、
   攻速、閃避、控制抵抗、元素附傷、經驗與戰鬥規則（限時/狂暴/蓄力）。 */
function towerBossCfg(floor) {
  if (isPurgatoryTowerFloor(floor)) return TOWER_BOSS_PURGATORY;
  if (isHellTowerFloor(floor)) return TOWER_BOSS_HELL;
  return TOWER_BOSS_TRIAL;
}
function bossStatsFor(floor) {
  var cfg = towerBossCfg(floor);
  var refStage = cfg.refStageBase + floor * cfg.refStagePerFloor;
  var base = monsterStatsFor(refStage, false);
  var bossAtk = base.atk * cfg.atkMult;   // BOSS 總魔攻（該塔總倍率）
  return {
    refStage: refStage,
    level: refStage + cfg.levelBonus,
    hell: isHellTowerFloor(floor),
    purgatory: isPurgatoryTowerFloor(floor),
    hp: base.hp * cfg.hpMult,
    atk: bossAtk,
    def: base.def * cfg.defMult,
    mdef: base.mdef * cfg.defMult,
    aspd: cfg.aspd,
    dodge: Math.min(cfg.dodgeBase + floor * cfg.dodgePerFloor, cfg.dodgeCap),
    hit: cfg.hitBase + floor * cfg.hitPerFloor,
    ctrlRes: cfg.ctrlRes,
    // 元素附傷以「總魔攻 bossAtk」為基準再乘該塔元素係數
    elemAtkVal: bossAtk * cfg.elemMult,
    xp: base.xp * cfg.xpMult
  };
}
...
function rollAncientAffixCount(affixCount, luck) {
  ...
  var m = luckDenom > 0
    ? ANCIENT_LUCK_WEIGHT_MULT * luckTotal / (luckTotal + luckDenom)
    : ANCIENT_LUCK_WEIGHT_MULT;
  var highCountFactor = 1 + m;
  var adjusted = [], total = 0;
  for (var i = 0; i < w.length; i++) {
    adjusted[i] = w[i] * Math.pow(highCountFactor, i);
    total += adjusted[i];
  }
  if (!(total > 0)) return 0;
  var pairs = [];
...
/* ---- 裝備等級分段（2026-07-23 裝備等級改造） ----
   裝備等級不再隨關卡/怪物等級連續變化，改為固定套級：
   來源等級 1~49 → 1 級裝；50~99 → 50 級裝；100~149 → 100 級裝…（每 EQUIP_TIER_SIZE=50 級一套）。
   所有裝備生成入口（makeEquipment）統一套用；詞條等數值算法不變，僅代入的裝備等級為分段值。 */
function equipmentTierLevel(level) {
  level = Math.max(1, Math.floor(Number(level) || 1));
  if (level < EQUIP_TIER_SIZE) return 1;
...
function affixRerollUnit(affixCap) {
  var cap = Math.max(0, Number(affixCap) || 0);
  var lowerWeight = Math.max(0, Number(AFFIX_REROLL_LOWER_WEIGHT) || 0);
  var upperWeight = Math.max(0, Number(AFFIX_REROLL_UPPER_BASE_WEIGHT) || 0);
  var exponent = Math.max(0, Number(AFFIX_REROLL_BIAS_EXPONENT) || 0);
  // 詞條上限率是以「百分點」儲存（100 代表 100%）；0 時維持 A/B 等機率。
    if (cap > 0) upperWeight += Math.pow(cap / 100, exponent);
  var total = lowerWeight + upperWeight;
  if (!(total > 0)) return rnd(0, 1);
...
/* ---- 裝備戰力評分（裝備比較與其他評估用）----
   ...
   未列出的詞條權重以 1 計。
   權重表 SCORE_WEIGHTS 已移至 js/data.js（由 Equipment_Affix 表「池=戰力權重」管理）。 */
...
/* 離線收益是「以歷史最高關卡為基準的固定費率」模型：不看命中率、不看怪物血量、
   也不看玩家傷害，時間到就照表發。下面五個值就是這個模型的全部旋鈕。

   ⚠️ 模擬器實測過這個模型的後果：關卡數一旦跑贏傷害，固定費率會發放玩家實際
   做不到的擊殺（停在關卡 188 時線上每分鐘殺 0~1 隻，離線照發 3 隻菁英怪），
   於是「推到推不動然後掛機」成為最佳解。這是刻意保留的設計，用下面的旋鈕調整
   強度即可；量測與取捨記在 docs/SIM_HARNESS.md「產出閘門」一節。 */
...
function offlineStageFor(best) {
  ...
  /* 取整單位若被設成 0 或負數，除法會得到 Infinity/NaN 而不是「不取整」。
     這是配置檔常見的手誤，夾在 1 以上讓它退化成「不取整」而不是壞掉。 */
  var unit = Math.max(1, Math.floor(Number(OFFLINE_STAGE_ROUND) || 1));
  var ratio = Number(OFFLINE_STAGE_RATIO);
  if (!isFinite(ratio) || ratio <= 0) ratio = 1;
  var s = Math.floor((b * ratio - OFFLINE_LEVEL_REDUCE) / unit) * unit;
  return Math.max(1, s);
}

// 離線是否結算菁英怪。掉落倍率也跟著這個旗標，才不會出現「普通怪卻吃菁英掉落倍率」。
function offlineUsesElite() { return !!Number(OFFLINE_ELITE); }

// 離線擊殺數 = ⌊有效離線秒數 / 擊殺間隔 × (1 + 離線預言%)⌋；每隻怪的掉落單獨擲骰（save.js）
