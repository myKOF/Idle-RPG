...
var SKILLS2 = {
  ...
  firehunt: { name: '火狩', emoji: '☄️', range: '3*3', dmgType: 'magic', elem: 'fire', cd: 15, cost: 40, tiers: [{ name: '火狩', unlock: { reinc: 0, lv: 100 }, cost: 40, fx: { pct: 100, pctPer: 10, count: 2, sec: 4, rps: 0.455, castM: 8, m: 8 }, goldBase: 100000, goldGrow: 1.5, desc: '召喚 {count} 團火狩環繞自身（環繞半徑 {m} 米、每秒 {rps} 圈），碰到敵人即命中一次，每次造成 {pct}% 火焰傷害，持續 {sec} 秒', vfx: { attack: 'burst-fire', projectile: 'orb-firehunt', hit: 'hit-fire-explosion', ground: 'ground-orbit-ring-fire' }, status: { self: [{ id: 'sgFirehunt' }] } }, { name: '強化火狩', unlock: { reinc: 0, lv: 150 }, cost: 60, fx: { pct: 15, pctPer: 1.5 }, goldBase: 200000, goldGrow: 1.5, desc: '火狩的體積與環繞範圍同步擴大 {pct}%' }, { name: '伴生火狩', unlock: { reinc: 0, lv: 200 }, cost: 80, fx: { chance: 20, chancePer: 2, m: 1 }, goldBase: 400000, goldGrow: 1.5, desc: '火狩命中時有 {chance}% 機率在母體外緣後方留 {m} 米間隙伴生一團火狩（每團只能伴生一次，伴生出的不再伴生）', triggerVfx: { projectile: 'orb-firehunt-companion' } }, { name: '三重火狩', unlock: { reinc: 0, lv: 250 }, cost: 100, fx: { count: 3, pct: 120, pctPer: 12, sec: 4 }, goldBase: 800000, goldGrow: 1.5, desc: '改為召喚 {count} 團火狩，每團造成 {pct}% 火焰傷害，持續 {sec} 秒' }, { name: '極速火狩', unlock: { reinc: 0, lv: 300 }, cost: 140, fx: { pct: 25, pctPer: 2.5 }, goldBase: 1500000, goldGrow: 1.5, desc: '火狩的旋轉速度 +{pct}%' }, { name: '再生', unlock: { reinc: 0, lv: 350 }, cost: 200, fx: { sec: 0.4, secPer: 0.04 }, goldBase: 3000000, goldGrow: 1.5, desc: '火狩每擊殺 1 個敵人，全部火狩的持續時間延長 {sec} 秒' }, { name: '狩神之舞', unlock: { reinc: 0, lv: 400 }, cost: 320, fx: { rings: 2, pct: 150, pctPer: 15, sec: 6, m: 6 }, goldBase: 5000000, goldGrow: 1.5, desc: '改為一次施放 {rings} 道火狩（外圈距內圈 {m} 米、兩道旋轉方向相反），每團造成 {pct}% 火焰傷害、出現時自帶伴生，持續 {sec} 秒' }], ult: [{ id: 'solarRing', name: '烈陽星環', cost: 300, fx: { count: 1, grow: 60, growSec: 4, spin: 30, pct: 50, pctPer: 10 }, goldBase: 10000000, goldGrow: 1.5, desc: '火狩數量 +{count} 團，體積在 {growSec} 秒內逐漸增大最多 {grow}%，環繞速度 +{spin}%，且造成傷害 +{pct}%', vfx: { projectile: 'orb-firehunt-solar' }, triggerVfx: { projectile: 'orb-firehunt-solar-companion' } }, { id: 'infiniteRing', name: '無限星環', cost: 300, fx: { count: 10, countPer: 1, m: 40 }, goldBase: 10000000, goldGrow: 1.5, desc: '火狩改為從自身中心呈螺旋狀向外擴散（在 {m} 米處達到最外圈），並於持續時間內不斷放出火狩，最多額外 +{count} 團', triggerVfx: { projectile: 'orb-firehunt-companion' } }, { id: 'fireGodDescend', name: '火神降臨', cost: 300, fx: { pct: 300, pctPer: 30, orbs: 3, orbsPer: 0.3, rps: 0.5, gap: 0.5, speed: 24, m: 6, flyM: 50, orbitM: 8 }, goldBase: 10000000, goldGrow: 1.5, desc: '你的身體被火焰包裹：每 {gap} 秒對周圍 {m} 米內的敵人造成 {pct}% 火焰傷害；普攻同時朝目標射出 {orbs} 顆火狩星環，以半徑 {orbitM} 米組成圓環，每秒順時針旋轉 {rps} 圈，整組以 {speed} 米／秒貫穿飛行 {flyM} 米', vfx: { projectile: 'proj-firehunt-ring', hit: 'hit-fire' }, triggerVfx: { projectile: 'orb-firehunt-firegod-companion' }, status: { self: [{ id: 'sgFireGodBody' }] } }] },
...
function resetSkill2RT() {
  ...
  SKILL2_RT = {
    ...
    earthguardRevival: null, // 【天地共生】五秒復甦演出（不入存檔）
...
function sgGroundTick(f, enemies, ctx) {
  if (f.kind === 'mirefireball') { sgMireFireballBlast(f, enemies, ctx); return; }
...
function sgTickGrounds(dt, ctx) {
  ...
  for (var i = list.length - 1; i >= 0; i--) {
    ...
    if (f.kind === 'mire' && f.mire && f.mire.inferno) sgMireInfernoTick(f, sgGroundVictims(f, enemies));
...
function sgCastFirehunt(pEnt, st, g, lvs, pool, primary, floatSel, out) {
  ...
  var activeUlt = skills2Ult('firehunt');
  var cfg = {
    tgt: primary, floatSel: floatSel, rings: rings, count: count, ringGapPx: ringGapPx,
    /* 母體＝本體欄的飛行子彈：沒選超神讀第一階；選了會改變環繞方式的超神（烈陽／無限）讀那一列，
       留白沿前階繼承。火神降臨不改環繞，而且它那一列的飛行子彈是普攻星環，所以母體照第一階。
       vfxBase：只讀本體欄——那一列的觸發子彈是伴生，不能把母體、軌道環與命中特效攔走。 */
    vfxTier: 1, vfxUlt: ultSolar ? 'solarRing' : (ultInfinite ? 'infiniteRing' : ''), vfxBase: true,
    /* 伴生＝觸發子彈：選了超神讀那一列，否則讀第三階（伴生火狩）那一列；見 sgOrbitEmitVfx。 */
    companionVfxUlt: activeUlt ? activeUlt.id : '',
...
function sgSpawnOrbitField(pEnt, st, gid, cfg) {
  var f = {
    ...
    vfxTier: cfg.vfxTier || 0, vfxUlt: cfg.vfxUlt || '', vfxGid: cfg.vfxGid || '', vfxBase: !!cfg.vfxBase,
    companionVfxUlt: cfg.companionVfxUlt || '',
...
function sgOrbitEmitVfx(f) {
  ...
  /* 伴生的外觀讀觸發子彈：選了超神讀那一列（例：烈陽星環紅色、火神降臨青色），否則讀第三階伴生火狩那一列。
     觸發欄不繼承，所以每個超神列都要自己填；留白＝顯示層沿用母體外觀。 */
  var companionRoles = detailed
    ? sgVfxRoles(f.gid, f.companionVfxUlt ? { vfxUlt: f.companionVfxUlt } : { vfxTier: 3 }) : null;
...
/* 超神【深淵火獄】：熔岩沼定時朝範圍內不同敵人的當下位置拋出火球。 */
function sgMireInfernoSpec(lvs, base) {
  var u = sgUlt('mire', 'abyssInferno');
  if (!u || !(lvs[6] > 0)) return null;
  return {
    gap: Math.max(0.2, sgGeometryNumber(u.def.fx, 'gap') || 1),
    count: Math.max(1, Math.floor(Number(u.def.fx.count) || 3)),
    dmgVal: base * sgUltVal(u, 'pct') / 100,
    radiusPx: bfMeterPx(Math.max(1, sgGeometryNumber(u.def.fx, 'm') || 6)),
    travelSec: Math.max(0.05, Number(u.def.fx.ballSec) || 0.9),
    arcM: Math.max(0, sgGeometryNumber(u.def.fx, 'arcM') || 12),
    projectile: sgVfxRoles('mire', { vfxUlt: 'abyssInferno', vfxBase: true }).projectile,
    burst: sgVfxRoles('mire', { vfxUlt: 'abyssInferno' }).attack
  };
}
...
/* 每一攤沼澤（含傳染出去的）各自計時；同一輪最多選三個不同敵人。
   落點於發射時固定，飛行期間不追蹤；落地才查當下範圍內的敵人。 */
function sgMireInfernoTick(f, victims) {
  var spec = f.mire && f.mire.inferno;
  if (!spec) return;
  if (!(f.infernoAt > 0)) f.infernoAt = f.bornAt + spec.gap;
  while (f.infernoAt <= GT && (!(f.expiresAt > 0) || f.infernoAt < f.expiresAt)) {
    f.infernoAt += spec.gap;
    if (!victims.length) continue;
    bfRandomOthers(null, victims, spec.count, 0, null).forEach(function (spot) {
      var end = bfPos(spot), start = f.pos;
      var area = end && start ? { x: end.x, y: end.y, sourceX: start.x, sourceY: start.y, fixedLanding: true } : null;
      sgEmitVfx('mire', area ? [] : [spot], f.floatSel, {
        fxKind: 'projectile', variant: 'dragon-devour-ball', projectile: true, hit: false,
        travelMs: [spec.travelSec * 1000], arcM: spec.arcM, area: area,
        vfxRoles: { projectile: spec.projectile }
      });
      sgSpawnGround(f.pEnt, f.st, 'mire', {
        kind: 'mirefireball', from: end, tgt: spot, floatSel: f.floatSel,
        radius: spec.radiusPx, dmgVal: spec.dmgVal, gap: spec.travelSec, hits: 1
      });
      SKILL2_RT.grounds[SKILL2_RT.grounds.length - 1].mireBurst = spec.burst;
    });
  }
}

function sgMireFireballBlast(f, enemies, ctx) {
  if (!f.pEnt || f.pEnt.hp <= 0) return;
  var victims = f.pos ? sgGroundVictims(f, enemies) : (f.tgt && f.tgt.hp > 0 ? [f.tgt] : []);
  sgEmitVfx('mire', [], f.floatSel, { fxKind: 'burst', variant: 'dragon-devour-impact',
    area: sgGroundArea(f), hit: false, vfxRoles: { attack: f.mireBurst } });
  var out = { killed: false, dmg: 0, crit: false };
  victims.forEach(function (e) { sgHitOne(f.pEnt, f.st, e, f.dmgVal, 'mire', f.floatSel, out, 0, 0, 'fire'); });
  if (ctx && ctx.onDamage && out.dmg > 0) ctx.onDamage(out.dmg);
  if (out.killed && ctx && ctx.onDeaths) ctx.onDeaths();
}
...
function skills2TryRebirth(pEnt) {
  ...
  pEnt._skillCastId = '';
  var riseSec = Math.max(0.5, Number(fx.riseSec) || 5);
  pEnt._sgRevival = { startAt: GT, endAt: GT + riseSec, mode: 'earthguard' };
  SKILL2_RT.earthguardRevival = { pEnt: pEnt, startAt: GT, endAt: GT + riseSec, invulnSec: Math.max(0.5, Number(fx.sec) || 5) };
  ...
  if (typeof blog === 'function') {
    ...
    blog('🌍 【天地共生】大地將你托起——' + riseSec + ' 秒後於原地復活！' + left, 'info');
  }
  if (typeof UI !== 'undefined' && UI.dirty) { UI.dirty.battle = true; UI.dirty.skills = true; }
  return true;
}
function sgTickEarthguardRevival(pEnt) {
  var r = SKILL2_RT && SKILL2_RT.earthguardRevival;
  if (!r || !pEnt || r.pEnt !== pEnt || !pEnt._sgRevival) return false;
  var progress = GT + 1e-8 >= r.endAt ? 1
    : Math.max(0, Math.min(1, (GT - r.startAt) / (r.endAt - r.startAt)));
  pEnt.hp = getStats().hp * progress;
  if (progress < 1) return true;
  pEnt.hp = getStats().hp;
  delete pEnt._sgRevival;
  SKILL2_RT.earthguardRevival = null;
  sgApplySlot(pEnt, 'earthguard', '7', 'self', 0, { dur: r.invulnSec });
  SKILL2_RT.rebirthInvuln = GT + r.invulnSec;
  if (typeof floatPlayerEvent === 'function') floatPlayerEvent('pv-float', '復活完成!', 'buff');
  if (typeof UI !== 'undefined' && UI.dirty) UI.dirty.battle = true;
  return true;
}
...
function skill2DownedActive() {
  ...
  var eg = SKILL2_RT && SKILL2_RT.earthguardRevival;
  return !!((ls && !ls.done && ls.reviveAt > GT) || (eg && eg.endAt > GT));
}
