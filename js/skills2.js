...
function sgCastCleave(pEnt, st, g, lvs, pool, primary, floatSel, out) {
  ...
  for (var cw = 0; cw < slashes; cw++) {
    sgEmitVfx('cleave', targets, floatSel, {
      ...
      travelMs: [Math.max(50, cleaveVfxRange / sgConfiguredFlightSpeed('cleave', 1, SG_FLYING_PROJECTILE_SPEED) * 1000)],
...
function sgWaterballShot(pEnt, st, g, lvs, pool, target, floatSel, out, cfg) {
  if (!target || target.hp <= 0) return;
  var travelMs = (typeof bfTravelSeconds === 'function') ? Math.round(sgConfiguredTravelSeconds('waterball', target) * 1000) : 0;
  ...
  for (var b = 0; ; b++) {
    ...
    var hopTravelMs = (typeof bfTravelSeconds === 'function')
      ? Math.round(sgConfiguredTravelSeconds('waterball', next) * 1000 / cfg.bounceSpeed) : 0;
