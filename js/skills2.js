...
/* 烈焰暴風：每道龍捲獨立計時；直線射向發射當下的敵人位置，抵達才查爆炸範圍。 */
function sgTickInfernoTempest(f, enemies) {
  ...
  while (t.nextAt <= GT && t.nextAt <= endAt) {
    ...
    // 距離近者優先，每輪至多三個不同目標，少怪時不重複補射。
    if (f.pos) targets.sort(function(a,b) {
      var pa=bfPos(a),pb=bfPos(b);
      return (pa?Math.hypot(pa.x-f.pos.x,pa.y-f.pos.y):Infinity)-(pb?Math.hypot(pb.x-f.pos.x,pb.y-f.pos.y):Infinity);
    });
    targets.slice(0,t.count).forEach(function(target) {
      ...
      var area=start&&end?{x:end.x,y:end.y,r:t.radius,sourceX:start.x,sourceY:start.y,fixedLanding:true}:null;
...
function sgResolveInfernoTempest(shot, ctx) {
  ...
  var out={killed:false,dmg:0,crit:false};
  victims.forEach(function(e){sgHitOne(shot.pEnt,shot.st,e,shot.dmgVal,'firepillar',shot.floatSel,out,0,0,'fire');});
