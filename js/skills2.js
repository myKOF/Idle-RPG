...
function sgConfiguredFlightSpeed(gid, tier, fallbackPx) {
  ...
  var speed = t && t.fx && Number(t.fx.speed);
...
/* ---- 特效欄位（2026-09-03 VFX Preset 化）----
   ...
   vfxUsage=effect 的列只供明確指定該列的附加事件使用，角色留白不繼承。
...
function sgVfxRoles(gid, extra) {
  ...
  if (extra && extra.vfxUlt) {
    ...
    if (active && active.def.vfxUsage !== 'effect') selected = active.def;
    if (!tier) {
  ...
  // 附加效果的用途不沿階級繼承，只有明確指定來源列的事件能取用。
  var effectRow = extra && extra.vfxUlt ? selected
    : extra && extra.vfxTier > 0 ? tiers[Math.floor(extra.vfxTier) - 1] : null;
  if (effectRow && effectRow.vfxUsage === 'effect') {
    overlay(effectRow);
    return roles;
  }
  ...
  for (var ti = 0; ti < Math.min(tier, tiers.length); ti++) {
    if (tiers[ti] && tiers[ti].vfxUsage !== 'effect') overlay(tiers[ti]);
  }
  overlay(selected);
  return roles;
}
...
function sgTickFlyingProjectiles(dt, ctx) {
  ...
  for (var pi = list.length - 1; pi >= 0; pi--) {
    ...
    var crossed;
    if (projectile.radialCurve && projectile.origin && typeof bfEnemiesInArea === 'function') {
...
function sgCastGale(pEnt, st, g, lvs, pool, primary, floatSel, out) {
  ...
  function strike() {
    ...
    if (lvs[3] > 0) {
      var extra = bfNearestOthers(impact, pool.filter(function (e) { return e !== primary; }), 1, bfMeterPx(sgVal(t[3].fx, 'm', lvs[3])))[0] || primary;
...
function sgCastFirepillar(pEnt, st, g, lvs, pool, primary, floatSel, out) {
  ...
  var speedPx = chase ? bfMeterPx(Number(chase.mps) || 0)
    : infinite ? bfMeterPx(Number(t[6].fx.speed) || 6)
...
function skills2OnBasicAttack(pEnt, target, floatSel, st) {
  ...
  var speed = bfMeterPx(Math.max(1, Number(u.def.fx.speed) || Number(u.def.fx.mps) || 24));
...
function sgThunderMatrix(pEnt, st, g, pool, floatSel, out) {
  ...
  var speedPx = Math.min(bfMeterPx(Math.max(1, Number(u.def.fx.speed) || Number(u.def.fx.mps) || 30)),
...
function sgCastWaterball(pEnt, st, g, lvs, pool, primary, floatSel, out) {
  ...
  var burstR = bfMeterPx(Number(t[0].fx.m) || 6) * (nova ? nova.scale : 1);
