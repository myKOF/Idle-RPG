...
function migrateSave(data) {
  ...
  // 3) 裝備數值存檔改造（2026-08-01）：舊存檔的凍結數值換算為可重算的來源值——
  //    詞條 val → roll（強度值）、傳奇特效丟掉 val（由 key+稀有度推導）、
  //    神鑄特效 val → roll、附魔 val → gemLv（附魔當下的寶石等級）；
  //    之後數值一律由參數表／公式當場算出（normalizeItemValueSources → js/formula.js §6）。
  //    冪等、無旗標：已是新格式即為 no-op，所以不是一次性遷移，
  //    屬於「讓舊格式讀得進來的結構相容處理」（分類見 ONE_TIME_MIGRATIONS.md）。
  ...
  if (typeof newForgeAllQueuedItems === 'function') {
    ...
    data.newForge.furnaces.forEach(function (fu) {
      ...
      if (Array.isArray(fu.belt)) fu.belt.forEach(function (it) { fixLoadedItem(it); });
    });
  }
  if (data.settings) delete data.settings.useAncientEssence;
  ...
  var curZ = data.stage && data.stage.zone;
  var hasGodProgress = (curZ === 'god_battlefield' || curZ === 'god_chaos' || curZ === 'god_sanctuary') ||
    (data.zoneProgress && (data.zoneProgress.god_battlefield || data.zoneProgress.god_chaos || data.zoneProgress.god_sanctuary));
  ...
  if (data.player.skills2 && data.player.skills2.levels && typeof SKILLS2 !== 'undefined') {
    ...
    if (Array.isArray(data.player.loadout)) {
      data.player.loadout = data.player.loadout.filter(function (loId) {
        ...
        /* 主動型被動（反擊）同樣佔裝載欄（裝上才生效），故與主動群組一視同仁，
           只清掉指向未知群組的鍵。 */
  ...
  function processOfflineKill() {
    ...
    for (var r = 0; r < rates.length; r++) {
      ...
      for (var i = 0; i < n; i++) {
        ...
        if (!conveyorFull) {
          if (!pushConveyor(makeEquipment(stage, { rarity: r }))) {
            conveyorFull = true;
