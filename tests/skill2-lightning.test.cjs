...
test('CHAIN 藍白電弧每隔 200ms 連接下一目標，傷害顯示沿用同一時刻',()=>{
 ...
 assert.deepEqual(chains.map(s=>s.delayMs||0),[0,200,400,600]);assert.ok(chains.every(s=>s.vfx.attack==='bolt-chain-bluewhite'&&!s.vfx.projectile));
 assert.deepEqual(hits,[0,200,400,600]);
});
