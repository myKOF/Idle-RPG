...
test('霹靂一閃最後一擊起發射 3+3 道，主打擊不繼承附加光束',()=>{
...
test('100×10 米矩形以玩家為中心貫穿前後50米，命中所有範圍內敵人且不提早、不重複',()=>{
 const h=setup(),target=h.enemy(100,0,'target'),edge=h.enemy(499,49,'edge'),behind=h.enemy(-499,0,'behind');
 const outside=[h.enemy(501,0,'too-far'),h.enemy(100,51,'too-wide'),h.enemy(-501,0,'too-back')];
 h.setPool([target,edge,behind,...outside]);h.cast(target);h.tick(.7);
 const e=h.beams()[0];assert.deepEqual(JSON.parse(JSON.stringify(e.area)),{x:-500,y:0,w:1000,h:100,a:0});
...
test('角色0連擊發射3道，疾風破自身追加打擊不增加雷電數量',()=>{
 for(const tier2 of [0,5]){
  ...
  assert.equal(h.beams().length,3);assert.equal(h.bolts().length,3);
 }
});
