...
function setup(lv=1){
 ...
 cast(primary){c.sgCastGale(p,{atk:1000},c.SKILLS2.gale,[1,0,0,0,0,0,0],pool,primary,'mv-float',{dmg:0});},
...
test('霹靂一閃最後一擊起發射 1+3 道，主打擊不繼承附加光束',()=>{
 const h=setup(),e=h.enemy(100,0,'target');h.setPool([e]);h.cast(e);
...
test('逐道使用最新玩家位置並重新隨機選敵，20米無敵人即永久終止剩餘序列',()=>{
 const h=setup(2),a=h.enemy(100,0,'a'),b=h.enemy(0,150,'b');h.setPool([a,b]);h.cast(a);h.tick(.7);h.tick(.78);
...
test('同一候選仍可在後續雷電再次隨機選中，换場清空所有排程',()=>{
 const h=setup(),a=h.enemy(100,0,'a');h.setPool([a]);h.cast(a);h.tick(.7);h.tick(.78);h.tick(.9);h.tick(.98);
