...
test('超神進化：二十二個已開放群組各三個選項，欄位齊全且說明模板的參數鍵都存在', () => {
  const c = loadContext();
  ['thrust', 'cleave', 'knife', 'gale', 'bloodblade', 'dualdance', 'counter', 'bloodrage',
    ...
    'waterball', 'frostnova', 'windblade', 'vacuumslash'].forEach((gid) => {
  ...
  // 只剩暴風屏障尚未開放，因此由它當控制組（設計文檔尚未給它超神進化）
  ...
  assert.equal(c.sgSlotCount('stormbarrier'), 7);
...
test('讀檔正規化：越界／非法的超神進化紀錄一律刪除，合法的夾在 1..上限', () => {
    ...
    player: {
      skills2: {
        ...
        ult: {
          ...
          stormbarrier: { pick: 0, lv: 1 },   // 該群組沒有超神進化 → 刪除
  ...
  assert.equal(c.sgUltPickOf(data.player.skills2.ult, 'stormbarrier'), null);
