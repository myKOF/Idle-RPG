...
test('太古詞條與太古精華掉落機率符合規則與上限', () => {
  ...
  assert.equal(context.ancientEssenceSalvageChanceForRarity(5), 2);
...
test('太古精華萃取器使用目前拆解機率的倍率加成，太古詞條另逐條獨立判定', () => {
  ...
  context.chance = (p) => { calls.push(p); return p === 50 && calls.length === 3; };
  const item = {
    rarity: 5,
    level: 200,
    affixes: [
  ...
  const result = context.salvageResult(item, 0, 175);
  assert.equal(calls[0], 5.5);
  assert.deepEqual(calls.slice(1, 3), [50, 50]);
  assert.equal(result.ancientEssence, 1);
});

test('勾選太古精華後洗煉會消耗 1 個，並以 20% 機率逐詞條判定', () => {
  const context = loadGameContext();
  context.G = {
...
test('太古資源與裝備詳情 UI 已註冊', () => {
  ...
  assert.match(data, /ANCIENT_REROLL_CHANCE\s*=\s*20/);
