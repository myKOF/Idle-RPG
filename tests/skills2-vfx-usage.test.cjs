...
test('特殊效果：中文欄位往返、逐風者設定及舊表相容，錯字拒絕',()=>{
 const a=api(),rows=a.parse(fs.readFileSync(root+'/config/CSV/Skills2.csv','utf8'));
 const header=rows[0],index=header.findIndex(h=>['特殊效果','特效用途特效','特殊用途特效'].includes(h));assert(index>=0);
 ...
 for(const label of ['特殊效果','特效用途特效','特殊用途特效']) {
  ...
  assert.equal(parse(rows.slice(1),renamed).cleave.ult[1].vfxUsage,'effect');
 }
...
test('Excel、CSV 與 JS 的技能資料一致，說明置頂且明列用途／空欄／觸發限制',()=>{
 ...
 assert.match(notes,/^特殊效果/);assert.match(notes,/附加效果/);assert.match(notes,/空欄不繼承/);assert.match(notes,/不會新增觸發能力/);
});
