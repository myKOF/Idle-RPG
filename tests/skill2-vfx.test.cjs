...
test('普攻觸發角色動作；飛刀彈射與連鎖不觸發，目標離場後延遲事件失效', () => {
  ...
  /* 2026-09-22 起只有普攻帶動角色動作；技能的施法動作改由模擬層的 act 事件驅動（onAct，協議 v36），
     行為測試在 tests/player-cast-act.test.cjs CAST-7；普攻只認主普攻的斬擊（天罰落雷不算），行為測試在 basic-melee */
  assert.match(onVfx, /if \(shouldAnimatePlayer\(spec\) && spec\.cat === 'basic' && spec\.variant === 'melee' && vfxTargetsLive\(spec\)\)/);
...
/* 持續場域沒有「結束事件」，靠模擬層逐拍重送續命；玩家判死那一刻模擬層就把
   執行期狀態整批清掉、復活倒數期間更不再送任何事件，所以孤兒場域只能由顯示層自己收。
   模擬層那一側叫不到顯示層（它跑在 Worker 內，BattleRenderer 不存在於該環境）。 */
test('倒地時由顯示層收掉玩家錨定的持續場域，模擬層不再嘗試呼叫顯示層', () => {
  const renderer = read('js/battle-renderer.js');
  const runtime = read('js/vfx-runtime.js');
  const skills2 = read('js/skills2.js');

  assert.match(renderer, /function clearPlayerFields\(\)/);
  assert.match(renderer, /for \(var auraKey in _followAuras\) clearFollowAura\(auraKey\);/);
  assert.match(renderer, /for \(var ringKey in _fireHuntRings\)/);
  assert.match(renderer, /if \(S\.vfxrt\) S\.vfxrt\.clearFields\(\);/);
  // 每張面板收一次：死亡前送出的事件還押在 POS_BUFFER_MS 的顯示緩衝裡
  assert.match(renderer, /if \(dead\) clearPlayerFields\(\);/);

  assert.match(runtime, /function clearFields\(\)/);
  assert.match(runtime, /clearFields: clearFields,/);

  // 模擬層跑在 Worker 內：任何 BattleRenderer.xxx 呼叫都是永遠不會執行的死碼
...
test('玩家錨定的領域一律交給狀態：技能不再送 follow-aura，每個領域都登記成狀態格子', () => {
  /* 2026-09-18 領域類光環：火神降臨、岩甲領域、水牢天瀑、海淵葬界、血刃斬兩個領域都改成玩家身上的狀態，
     畫面＝狀態表的持續特效，依狀態實例的半徑縮放、顯示層逐幀跟著玩家（不再有「圈畫在世界原點」的錨定問題）。 */
  const skills2 = read('js/skills2.js');
  const runtime = read('js/vfx-runtime.js');
  assert.doesNotMatch(skills2, /variant: 'follow-aura'/, '技能不再送 follow-aura 地板事件');
  assert.match(runtime, /position: footOf\(a\.key\), scaleX: a\.sx, scaleY: a\.sy/, '狀態光環逐幀跟著實體並帶補間後的尺寸');
  for (const role of ['slayerDomain', 'venomDomain', 'fireGodBody', 'petrifyDomain', 'gravityDomain', 'waterPrisonDomain', 'abyssDomain']) {
    assert.match(skills2, new RegExp("sgSyncDomainStatus\\([^)]*'" + role + "'"), role + ' 由技能同步成狀態');
  }
});
