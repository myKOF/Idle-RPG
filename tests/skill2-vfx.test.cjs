...
test('追蹤風刃不建立綠色方框，且舊事件不會以座標重建跳格節點', () => {
  ...
  assert.match(index, /js\/vfx-runtime\.js\?v=1\.0\.15/);
  assert.match(index, /js\/skills2\.js\?v=1\.0\.95/);
  assert.match(bridge, /WORKER_ASSET_VERSION = '20260909-thrust-tuning'/);
  assert.match(worker, /\.\.\/skills\.js\?v=20260903-vfx-preset-fields/);   // 本輪未改 skills.js，版號不動
  assert.match(worker, /\.\.\/skills2\.js\?v=20260909-thrust-tuning/);
