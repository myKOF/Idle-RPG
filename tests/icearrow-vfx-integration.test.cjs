...
test('icearrow ordinary, piercing, homing and rain mappings ship all approved assets',()=>{
 ...
 context.skills2Levels=()=>[1,1,1,1,1,1,1];context.sgIcearrowSpeed=()=>585;context.sgSpawnGround=(_p,_st,_gid,spec)=>{context.spawned=spec;};context.sgSpawnIcearrowHoming({}, {}, g.tiers[6].fx, {}, 1, null, 'pv-float', {});assert.equal(context.spawned.vfxTier,7);assert.equal(g.tiers[context.spawned.vfxTier-1].vfx.ground,presets[2].id);assert.equal(g.tiers[0].fx.speed,58.5);
