  ...
  function frame() {
    ...
    for (const n of [...nodes].sort((a, b) => (a.t?.zIndex || 0) - (b.t?.zIndex || 0))) {
      ...
      drawSprite(pixels, width, height, tex, { x: 0, y: 0, w: tex.width, h: tex.height },
        { ...n.t, x: n.t.x + 100, y: n.t.y + 80 }, n.spec.blendMode);
    }
    return pixels;
  }
  let atOneSecond;
  for (let i = 0; i < (onFrame ? 480 : 120); i++) {
    if (i < 240 && i % 12 === 0) for (let j = 0; j < 6; j++) shim.playCombatVfx({
      fxKind: 'projectile', variant: 'firehunt-ring', targets: ['enemy'], hit: false,
      vfx: { projectile: preset.id }, travelMs: [1667], angle: 0, lineLength: 400,
      lineWidth: 16, delayMs: j * 80
    });
...
test('火神星環實際 Runtime 高速連發仍可見，不疊成白色實心光塊', () => {
  ...
  assert.equal(count, 50, 'must retain every active projectile, not suppress overlapping volleys');
