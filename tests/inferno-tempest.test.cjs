...
test('烈焰暴風：不倍增龍捲，每道獨立每秒鎖定至多3名24米內敵人',()=>{
 ...
 const shots=events.filter(e=>e.variant==='inferno-tempest-ball');assert.equal(shots.length,3);
 assert.equal(new Set(c.SKILL2_RT.projectiles.map(p=>p.target)).size,3);
 ...
 c.sgTickInfernoTempest(f,c.FIELD.monsters);assert.equal(c.SKILL2_RT.projectiles.length,3);
 c.GT+=1;c.FIELD.monsters.splice(1);c.sgTickInfernoTempest(f,c.FIELD.monsters);assert.equal(c.SKILL2_RT.projectiles.length,4);
});
...
test('烈焰暴風：只升火球傷害，重生龍捲重新計時，死亡與重置停止傷害',()=>{
 ...
 const child=a.c.SKILL2_RT.grounds.at(-1);assert.notEqual(child,a.f);assert.equal(child.tempest.nextAt,4);assert.notEqual(child.tempest,a.f.tempest);
...
test('烈焰暴風：實際 Runtime 依事件從龍捲平射，半程位置沒有拋物線高度',()=>{
 ...
 adapter.registerPresets([{schemaVersion:1,id:'proj-fireball',duration:1.5,loop:false,layers:[{id:'body',type:'sprite',assetId:'ball',scale:{x:1,y:1}}]}]);
