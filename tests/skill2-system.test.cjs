...
test('疾風斬逐段傷害與新版特效同拍，月牙以目標為中心且只播放一次', () => {
  for (const [moon, adjusted] of [[false,false], [true,false], [true,true]]) {
    ...
    assert.equal(out.dmg, 8100 * (moon ? 1 + c.sgVal(c.SKILLS2.gale.tiers[6].fx, 'pct', 1) / 100 : 1));
    c.resetSkill2RT(); assert.equal(c.SKILL2_RT.galeStrikes.length, 0);
  }
});
test('疾風破死亡後剩餘連擊轉移，傷害與特效同目標且不超出射程', () => {
  for (const killEach of [false, true]) {
    const c = loadContext();
    c.resetSkill2RT(); c.GT = 0;
    c.sgLegend = () => ({}); c.sgUlt = () => null; c.sgGaleOnHit = () => {};
    const events = [], hits = [];
    c.playCombatVfx = s => events.push({ at: c.GT, target: s.targets[0] });
    c.enemyEventFloatTarget = e => e.name;
    ...
    assert.deepEqual(events, hits);
    assert.deepEqual(hits.map(h => h.target), killEach ? ['a','b'] : ['a','a','a']);
    assert.deepEqual(hits.map(h => h.at), killEach ? [0,gap] : [0,gap,gap*2]);
...
test('cleave 迴旋斬改為近戰範圍內不限人數，強化斬同步放大範圍', () => {
  const c = loadContext();
  const calls = stubHits(c);
  c.chance = () => false;
  c.G.player.skills2.levels.cleave = [1, 0, 0, 0, 0, 0, 0];
  const p = playerEnt();
...
test('SKILLS2：23 個群組、每組 7 階、欄位齊全且說明模板的參數鍵都存在', () => {
  const c = loadContext();
  ...
  assert.deepEqual(gids, ['thrust', 'cleave', 'knife', 'gale', 'bloodblade', 'dualdance', 'counter', 'bloodrage',
    ...
    'chainlightning', 'thunderstrike', 'thunderorb',
    'icearrow', 'waterball', 'frostnova',
    'windblade', 'vacuumslash', 'stormbarrier']);
  gids.forEach((gid) => {
