...
function loadContext() {
  ...
  for (const file of ['js/util.js', 'js/data.js', 'js/status.js', 'js/formula.js', 'js/player.js']) {
...
test('初始資源已登錄於遊戲參數表', () => {
  ...
  assert.match(csv, /\d+,0,0-遊戲預設,開場金幣,a,[^\n]*,50000,/);
  assert.match(csv, /\d+,0,0-遊戲預設,開場裝備碎片,a,[^\n]*,500,/);
  assert.match(csv, /\d+,0,0-遊戲預設,開場附魔精華,a,[^\n]*,100,/);
});
