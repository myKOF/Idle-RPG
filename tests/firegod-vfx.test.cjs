...
test('火神星環發射只派送表定子彈，不繼承爆炸／地板或預播受擊',()=>{const {c,events}=setup();const u=c.sgUlt('firehunt','fireGodDescend');u.def.vfx.projectile='configured-ring';c.skills2OnBasicAttack(c.FIELD.player,c.FIELD.monsters[0],'mv-float',c.getStats());assert.ok(events.length>=3);for(const e of events){assert.deepEqual(Object.keys(e.vfx),['projectile']);assert.equal(e.vfx.projectile,'configured-ring');assert.equal(e.hit,false);assert.equal(e.lineLength,500);assert.equal(e.lineWidth,c.SG_FLYING_PROJECTILE_HALF_WIDTH*2);assert.equal(e.travelMs[0],2083);}assert.equal(c.SKILL2_RT.projectiles.length,events.length);for(const p of c.SKILL2_RT.projectiles)assert.equal(p.halfWidthPx,events[0].lineWidth/2);});
...
test('星環同時起飛，六枚等距成環、共同中心等速前進並順時針旋轉',()=>{
  ...
  for(const t of [0,.25,.5,1]){
    ...
    assert.ok(Math.abs(center.x-240*t)<1e-8);assert.ok(Math.abs(center.y)<1e-8);
  ...
  const first=c.projectileOrbitPoint(orbits[0],.5);assert.ok(Math.abs(first.x-200)<1e-8);assert.ok(Math.abs(first.y)<1e-8);
