...
test('pushConveyor：一律進總佇列；「總量」（總佇列＋各爐專屬）滿載丟棄', () => {
  ...
  // 上限以總量計：總佇列＋各爐專屬佇列合計達上限即拒收
  G.newForge.queue.length = c.NEW_FORGE_QUEUE_CAP - 5000;
  G.newForge.furnaces[0].queue.length = 5000;
  assert.equal(c.newForgeTotalQueued(), c.NEW_FORGE_QUEUE_CAP);
  assert.equal(c.pushConveyor(c.makeEquipment(10, { rarity: 0, level: 10 })), false);
});
...
test('預設熔爐：單傳送帶結構、品質勾選預設普通~傳說、零件格 3', () => {
  ...
  assert.ok(Array.isArray(fu.queue), '每爐應有專屬佇列');
  assert.ok(Array.isArray(fu.belt));
  assert.equal(fu.qualities.length, c.RARITIES.length);
  assert.deepEqual(plain(fu.qualities.slice(0, 6)), [true, false, false, false, false, false]);
...
test('新增熔爐會複製上一座熔爐的品質勾選，且不共用同一個陣列', () => {
  const c = loadContext(LOGIC_FILES);
  const G = freshG(c);
  G.player.reincarnations = 2;
  const first = G.newForge.furnaces[0];
  first.qualities[1] = true;
  first.qualities[4] = true;
  first.qualities[0] = false;

  assert.equal(c.addNewForgeFurnace(), null);
  const second = G.newForge.furnaces[1];
  assert.deepEqual(plain(second.qualities), plain(first.qualities));
  assert.notEqual(second.qualities, first.qualities);

  second.qualities[2] = true;
  assert.equal(c.addNewForgeFurnace(), null);
  const third = G.newForge.furnaces[2];
  assert.deepEqual(plain(third.qualities), plain(second.qualities));
});

test('品質派發：勾選品質進該爐專屬佇列（總佇列遞減）、未勾/上鎖/神鑄創世保留入包；專屬佇列滿載留總佇列', () => {
  const c = loadContext(LOGIC_FILES);
  const G = freshG(c);
  const fu = G.newForge.furnaces[0];
  fu.qualities[1] = true;
  const a = c.makeEquipment(10, { rarity: 0, level: 10 });      // 勾選 → 專屬佇列
  ...
  assert.equal(fu.queue.length, 1, '派發進該爐專屬佇列');
  assert.equal(fu.queue[0].id, a.id);
  assert.equal(G.inventory.length, 3);
  assert.equal(G.newForge.stats.kept, 3);
  assert.equal(G.newForge.queue.length, 0, '派發後總佇列遞減');
  // 傳送帶自專屬佇列補位
  c.newForgeRefillBelt(fu);
  assert.equal(fu.belt.length, 1);
  assert.equal(fu.belt[0].id, a.id);
  assert.equal(fu.queue.length, 0);
  // 專屬佇列滿載 → 留總佇列等待（FIFO）
  fu.queue.length = c.NEW_FORGE_FURNACE_QUEUE_CAP;
  const w = c.makeEquipment(10, { rarity: 1, level: 10 });
  G.newForge.queue.push(w);
  c.newForgeRouteQueue();
  assert.equal(G.newForge.queue.length, 1, '專屬佇列滿載時應留在總佇列');
  assert.equal(G.newForge.queue[0].id, w.id);
});

test('多熔爐派發：裝備進勾選該品質的熔爐專屬佇列；停用熔爐跳過', () => {
  const c = loadContext(LOGIC_FILES);
  const G = freshG(c);
  ...
  c.newForgeRouteQueue();
  assert.equal(fu2.queue.length, 1);
  assert.equal(fu2.queue[0].id, common.id);
  assert.equal(fu1.queue.length, 1);
  assert.equal(fu1.queue[0].id, fine.id);
  // 停用 fu2 → 普通品質無人收 → 保留入包
  fu2.enabled = false;
  const common2 = c.makeEquipment(10, { rarity: 0, level: 10 });
  G.newForge.queue.push(common2);
  c.newForgeRouteQueue();
  assert.equal(fu2.queue.length, 1, '停用熔爐不收件');
  assert.ok(G.inventory.some((x) => x && x.id === common2.id), '無熔爐可收時保留入包');
});

test('派發平均分流：同設定多爐依（帶＋專屬佇列）負載較少者優先，各爐數量獨立且加總守恆', () => {
  const c = loadContext(LOGIC_FILES);
  const G = freshG(c);
  ...
  c.newForgeRouteQueue();
  assert.equal(G.newForge.queue.length, 0, '派發後總佇列遞減');
  assert.equal(fu1.queue.length, 0, '滿載爐（負載 30）不搶件');
  assert.equal(fu2.queue.length, 10, '空爐平均分流');
  assert.equal(fu3.queue.length, 10);
  // 續派 70 件：先補平 fu2/fu3 至負載 30，之後三爐輪流——負載均衡且加總守恆
  for (let i = 0; i < 70; i++) G.newForge.queue.push(c.makeEquipment(5, { rarity: 0, level: 5 }));
  c.newForgeRouteQueue();
  const load = (f) => f.queue.length + f.belt.length;
  assert.equal(fu1.queue.length + fu2.queue.length + fu3.queue.length, 90, '各爐專屬佇列加總＝總派發量');
  assert.equal(load(fu1), 40);
  assert.equal(load(fu2), 40);
  assert.equal(load(fu3), 40);
});
...
test('熔爐上限：0轉2座、轉生後遞增、cap 12；移除熔爐帶內容退回', () => {
  ...
  // 移除：帶上裝備退回背包、專屬佇列回總佇列重新派發
  const fu = G.newForge.furnaces[11];
  const it = c.makeEquipment(10, { rarity: 0, level: 10 });
  const qit = c.makeEquipment(10, { rarity: 1, level: 10 });
  fu.belt.push(it);
  fu.queue.push(qit);
  c.removeNewForgeFurnace(fu.id);
  assert.equal(G.newForge.furnaces.length, 11);
  assert.ok(G.inventory.some((x) => x && x.id === it.id), '帶上裝備退回背包');
  assert.ok(G.newForge.queue.some((x) => x && x.id === qit.id), '專屬佇列回總佇列');
});
...
test('帶尾 +N＝各爐專屬佇列真實件數（獨立非共用）；品質取消/停用時退回總佇列重新派發', () => {
  const c = loadContext(LOGIC_FILES);
  const G = freshG(c);
  G.player.reincarnations = 1;
  c.addNewForgeFurnace();
  const fu1 = G.newForge.furnaces[0];
  const fu2 = G.newForge.furnaces[1];
  // 兩座同設定：10 件平均分流 → 各自 5 件（不再各顯示 10）
  for (let i = 0; i < 10; i++) G.newForge.queue.push(c.makeEquipment(5, { rarity: 0, level: 5 }));
  c.newForgeRouteQueue();
  assert.equal(fu1.queue.length, 5);
  assert.equal(fu2.queue.length, 5);
  assert.equal(G.newForge.queue.length, 0, '總佇列派發後遞減——各爐加總＝總量，不重複計數');
  // 共用計數已移除（+N 由 UI 直讀 fu.queue.length）
  assert.equal(typeof c.newForgePendingCounts, 'undefined');
  // 取消 fu2 的普通勾選 → 健檢把不符項退回總佇列
  fu2.qualities[0] = false;
  c.newForgeReturnUnroutable(fu2);
  assert.equal(fu2.queue.length, 0);
  assert.equal(G.newForge.queue.length, 5, '不符品質退回總佇列重新派發');
  // 停用 fu1 → 專屬佇列全退回
  fu1.enabled = false;
  c.newForgeReturnUnroutable(fu1);
  assert.equal(fu1.queue.length, 0);
...
test('V3 淨化：qualities 補長度且神鑄創世恆 false、partSlots 夾 3~8、壞帶項剔除、專屬佇列補建/超量回總佇列', () => {
  const c = loadContext(LOGIC_FILES.concat(['js/save.js']));
  const state = c.newGameState();
  const fu = state.newForge.furnaces[0];
  ...
  delete fu.queue; // 合併前存檔：熔爐無專屬佇列欄位
  c.migrateSave(state);
  assert.equal(fu.qualities.length, c.RARITIES.length);
  ...
  assert.ok(Array.isArray(fu.queue), '專屬佇列應補建');
  // 專屬佇列超量 → 尾端回總佇列
  for (let i = 0; i < c.NEW_FORGE_FURNACE_QUEUE_CAP + 5; i++) fu.queue.push(c.makeEquipment(5, { rarity: 0, level: 5 }));
  const globalBefore = state.newForge.queue.length;
  c.migrateSave(state);
  assert.equal(fu.queue.length, c.NEW_FORGE_FURNACE_QUEUE_CAP);
  assert.equal(state.newForge.queue.length, globalBefore + 5, '超量退回總佇列');
});
