...
test('敵方區普攻固定白色、技能固定黃色，爆擊不改變來源顏色', () => {
  assert.match(combat, /floatEnemyEvent\(mEnt,\s*floatSel,\s*dmgStr,\s*\(res\.crit \? 'crit ' : 'dmg '\) \+ 'enemy-attack'\)/);
  assert.match(combat, /'crit enemy-attack'/);
  assert.match(skills, /floatEnemyEvent\(targetEnt,\s*floatSel,\s*sk\.emoji \+ dmgStr,\s*\(dmgRes\.crit \? 'crit ' : 'dmg '\) \+ 'enemy-skill'\)/);
  ...
  assert.match(css, /\.float-txt\.enemy-hit-float\.enemy-attack\s*\{[\s\S]*?z-index:\s*1/);
  assert.match(css, /\.float-txt\.enemy-hit-float\.enemy-skill\s*\{[\s\S]*?z-index:\s*2/);
