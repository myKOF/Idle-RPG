...
/* ---- 裝等：比品質更前面的一道閘 ----
   ...
   詞條數值 =（基礎值 + 成長基礎值×每級成長×(裝等−1)）× 品質倍率（js/item.js 的 affixValue），
...
/* ============ 技能欄 ============
   ...
   技能欄格數 = clamp(基準 + ⌊等級/每級⌋, 下限, 上限)（js/formula.js 的 loadoutSize；
   數值見參數表「1-成長經驗／技能裝載欄」），要好幾十級才多一格，
   而開局就預設塞了 powerSlash／arcaneBurst／manaBarrier 三個。
   對照真人 24 小時存檔（觀測當時上限還是 20 格）：12 格全滿、全 Lv.10、含 3 個融合技；
