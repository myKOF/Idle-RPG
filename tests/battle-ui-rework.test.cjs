...
test('戰鬥場景 HTML 結構包含頂部 quest-bar 與 canvas 內部的 battle-skill-bar', () => {
  ...
  // 確認 quest-bar 在 battle-scene-wrapper 內，battle-skill-bar 在 #battle-canvas-host 內
  const wrapperIdx = html.indexOf('<div class="battle-scene-wrapper"');
  const canvasHostIdx = html.indexOf('<div id="battle-canvas-host"');
  const questIdx = html.indexOf('<div id="quest-bar"', wrapperIdx);
  const skillBarIdx = html.indexOf('<div id="battle-skill-bar">', canvasHostIdx);

  assert.ok(questIdx > wrapperIdx, '#quest-bar 應在 .battle-scene-wrapper 內');
  assert.ok(skillBarIdx > canvasHostIdx, '#battle-skill-bar 應在 #battle-canvas-host 內');
});
