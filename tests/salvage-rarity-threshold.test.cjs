...
test('salvage all with ancient count threshold (<= 1 ancient)', () => {
  const context = loadGameContext();
  const salvaged = [];
  context.G = {
    inventory: [
      { id: '0-ancient', rarity: 4, level: 50, locked: false, affixes: [] },
      { id: '1-ancient', rarity: 4, level: 50, locked: false, affixes: [{ ancient: true }] },
      { id: '2-ancient', rarity: 4, level: 50, locked: false, affixes: [{ ancient: true }, { ancient: true }] }
    ]
  };
  context.doSalvage = (it) => {
    salvaged.push(it.id);
  ...
  context.salvageAllUnlocked(-1, null, 1);

  assert.deepEqual(salvaged, ['0-ancient', '1-ancient']);
  assert.deepEqual(Array.from(context.G.inventory, (it) => it.id), ['2-ancient']);
});
