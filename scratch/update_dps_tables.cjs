...
results.forEach(r => {
  ...
  mdComparison += `| ${gName} | ${name} | ${dpsFormatted} |\n`;
});
...
Object.keys(groups).forEach(gid => {
  ...
  mdReport += `| **${b.target.groupName}** | **基礎 (未點超神)** | ${(bMob / 1e6).toFixed(2)}M (基準 1.0x) | ${(bElite / 1e6).toFixed(2)}M (基準 1.0x) | ${(bBoss / 1e6).toFixed(2)}M (基準 1.0x) | 第 1~7 階滿級基準形態，未點超神進化第 8 階。 |\n`;
  ...
  g.ults.forEach(u => {
    ...
    mdReport += `| ${u.target.groupName} | **${u.target.name}** | ${(uMob / 1e6).toFixed(2)}M (${multMob}x) | ${(uElite / 1e6).toFixed(2)}M (${multElite}x) | ${(uBoss / 1e6).toFixed(2)}M (${multBoss}x) | ${u.target.note} |\n`;
  });
});
