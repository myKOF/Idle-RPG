...
const id='codex-authored/tornado/fire-tip-flow.png',buf=atlas.toBuffer('image/png');
...
const at=index.assets.findIndex(a=>a.assetId===id);if(at<0)index.assets.push(entry);else index.assets[at]=entry;index.assetCount=index.assets.length;
fs.writeFileSync(ip,JSON.stringify(index,null,2)+'\n');
console.log('Baked pointed fire column: '+dest);
