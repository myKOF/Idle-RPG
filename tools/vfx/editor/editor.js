  ...
  var COMMON_FIELDS = [
    ...
    { key: 'followDirection', label: '跟著發射方向轉', kind: 'bool', default: true,
      ...
      hint: '勾選（預設）：技能往哪打，這一層就跟著轉向哪裡；貼地的圖形還會在地面上轉成斜橢圓。\n' +
        ...
        '（預覽區看不出差別，差別在遊戲畫面上。）' },
    /* 第三種變形：遊戲把整份特效沿一軸拉長（光束拉到敵人身上、場域依判定尺寸撐開）。
       2026-09-24 使用者把前兩個都取消之後，光束尾端的星芒仍然被壓扁——來源就是這個。 */
    { key: 'followStretch', label: '跟著特效拉長', kind: 'bool', default: true,
      enabledWhen: function (l) { return cameraFlagAllowed(l) && l.type !== 'particle'; },
      disabledHint: '粒子的圖本來就只吃等比縮放，不會被拉長，所以沒有東西可關；' +
        '變形圖層（閃電那種）的形狀由變形決定，也不開放。',
      hint: '勾選（預設）：特效被拉長時（例如光束拉到敵人身上、場域依判定尺寸撐開）這一層跟著拉。\n' +
        '取消：圖維持原本的長寬比（只跟著等比的那一份縮放），位置照樣跟著拉——\n' +
        '例如光束尾端的星芒留在尾端，但不會被壓扁。\n' +
        '（預覽區看不出差別，差別在遊戲畫面上。）' }
  ];
