...
edits.push({ file: 'formula', re: /aspd: (-?[\d.]+),\s*\n\s*dodge: 0, gold/, grp: 1, value: P('4-野外怪物', '攻擊速度', 0), label: 'mob-aspd' });
...
inline('formula', 'st.base.hp = 120 + (lv - 1) * 22 + rawVit * ', P('2-屬性派生', '生命上限', 2), 'hp每耐');
inline('formula', 'st.base.mp = ', P('2-屬性派生', '法力上限', 0), 'mp基底');
inline('formula', 'st.base.mp = 40 + rawInt * ', P('2-屬性派生', '法力上限', 1), 'mp每智');
inline('formula', 'st.mpRegen = ', P('2-屬性派生', '法力恢復/秒', 0), 'mpregen基底');
inline('formula', 'st.mpRegen = 2 + st.int * ', P('2-屬性派生', '法力恢復/秒', 1), 'mpregen每智');
inline('formula', 'st.base.atk = ', P('2-屬性派生', '物理攻擊', 0), 'atk基底');
inline('formula', 'st.base.atk = 8 + (lv - 1) * ', P('2-屬性派生', '物理攻擊', 1), 'atk每級');
inline('formula', 'st.base.atk = 8 + (lv - 1) * 1.6 + st.str * ', P('2-屬性派生', '物理攻擊', 2), 'atk每力');
inline('formula', 'st.base.matk = ', P('2-屬性派生', '魔法攻擊', 0), 'matk基底');
inline('formula', 'st.base.matk = 6 + (lv - 1) * ', P('2-屬性派生', '魔法攻擊', 1), 'matk每級');
inline('formula', 'st.base.matk = 6 + (lv - 1) * 1.2 + st.int * ', P('2-屬性派生', '魔法攻擊', 2), 'matk每智');
inline('formula', 'st.base.def = ', P('2-屬性派生', '物理防禦', 0), 'def基底');
inline('formula', 'st.base.def = 4 + (lv - 1) * ', P('2-屬性派生', '物理防禦', 1), 'def每級');
inline('formula', 'st.base.def = 4 + (lv - 1) * 1.0 + st.vit * ', P('2-屬性派生', '物理防禦', 2), 'def每耐');
inline('formula', 'st.base.mdef = ', P('2-屬性派生', '魔法防禦', 0), 'mdef基底');
inline('formula', 'st.base.mdef = 3 + (lv - 1) * ', P('2-屬性派生', '魔法防禦', 1), 'mdef每級');
inline('formula', 'st.base.mdef = 3 + (lv - 1) * 0.8 + st.int * ', P('2-屬性派生', '魔法防禦', 2), 'mdef每智');
inline('formula', 'st.critRate = clamp(', P('2-屬性派生', '暴擊率', 0), '暴擊率基底');
inline('formula', 'st.critRate = clamp(5 + st.agi * ', P('2-屬性派生', '暴擊率', 1), '暴擊率每敏');
inline('formula', 'st.critDmg = ', P('2-屬性派生', '暴擊傷害', 0), '暴傷基底');
inline('formula', 'st.aspd = clamp(', P('2-屬性派生', '攻擊速度', 0), '攻速基底');
inline('formula', 'st.aspd = clamp(1.0 * (1 + (A.aspdPct + st.agi * ', P('2-屬性派生', '攻擊速度', 1), '攻速每敏');
inline('formula', 'st.hit = ', P('2-屬性派生', '命中率', 0), '命中基底');
inline('formula', 'st.evasion = clamp(st.agi * ', P('2-屬性派生', '閃避率', 0), '閃避每敏');
inline('formula', 'st.weight = Math.round(st.str * ', P('2-屬性派生', '負重上限', 0), '負重每力');
...
objFieldML('data', 'STAT_CAPS = {', 'pRes', '2-屬性上限', '物理抗性 上限', 0, '上限-物抗');
objFieldML('data', 'STAT_CAPS = {', 'mRes', '2-屬性上限', '魔法抗性 上限', 0, '上限-魔抗');
objFieldML('data', 'STAT_CAPS = {', 'elemRes', '2-屬性上限', '六系元素抗性 上限', 0, '上限-元素抗');
...
inline('formula', 'dodge: Math.min(', P('4-高塔BOSS', '閃避', 0), 'boss閃避基');
inline('formula', 'dodge: Math.min(5 + floor, ', P('4-高塔BOSS', '閃避', 1), 'boss閃避上');
...
numCtx('formula', 'var v = (', ' + item.level * 1.2)', P('6-裝備', '附魔-攻擊類', 0), '附魔攻-基');
inline('formula', 'var v = (5 + item.level * ', P('6-裝備', '附魔-攻擊類', 1), '附魔攻-每級');
...
inline('formula', 'Math.floor(G.player.level / ', P('1-成長經驗', '技能裝載欄', 0), '裝載欄-每級');
numCtx('formula', 'Math.max(', ', Math.floor(G.player.level', P('1-成長經驗', '技能裝載欄', 1), '裝載欄-下限');
...
numCtx('combat', 'hit: ', ',', P('3-戰鬥核心', '怪物固定戰鬥值', 2), '怪物-命中');
// 野外菁英掉落倍率（combat.js）
