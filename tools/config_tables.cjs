...
function sheetXml(rows) {
  ...
  const isSkills2 = rows[0] && rows[0].includes('作用方式與距離用途（唯讀說明）');
  if (isSkills2) {
    parts.push('<sheetViews><sheetView workbookViewId="0"><pane xSplit="3" ySplit="1" topLeftCell="D2" activePane="bottomRight" state="frozen"/></sheetView></sheetViews><cols>');
    rows[0].forEach((label, i) => {
      const width = i >= 21 ? (i === rows[0].length - 1 ? 90 : 34) :
        (/JSON|效果說明/.test(label) ? 55 : /名稱|解鎖/.test(label) ? 24 : 15);
  ...
  parts.push('</sheetData>');
  if (isSkills2) parts.push('<autoFilter ref="A1:'+colName(rows[0].length-1)+rows.length+'"/>');
...
/* ---- 特效欄位（2026-09-03 VFX Preset 化）----
   技能表（Skills／Skills2 每一列）六欄：施放特效／攻擊特效／飛行子彈／受擊特效／地板特效／持續場域特效；
...
const STATUS_EFFECT_KINDS = ['dot', 'hot', 'stat', 'ctrl', 'shield'];
const STATUS_GLOSSARY_ROWS = [
  ...
  ['stat＝屬性增減（攻擊、防禦、閃避…）；'],
  ['ctrl＝行動限制（暈眩、減速、無敵）；'],
  ['shield＝吸收護盾（占施法者最大生命%，吃護盾效率；被打完或時間到就消失）；'],
  [''],
  ['效果鍵值'],
  ...
  ['狀態效果＝hot 時固定填 hot；shield 時固定填 shield；dot 留空；'],
  ...
  ['maxHp＝占「目標最大生命」百分比（死亡詛咒）；持續回復與護盾則是占「施法者最大生命」百分比；'],
  ['value＝直接數值（不經百分比換算）；'],
  [''],
  ['狀態傷害'],
  ['每次作用的量，依「傷害來源」解讀（skill/maxHp 填百分比數字，value 填數值）；'],
  ['狀態效果＝hot 時＝每秒回復的最大生命%；shield 時＝護盾量占最大生命%；stat／ctrl 不使用本欄；'],
  ...
  ['護盾（shield）也吃這一欄：時間到時還沒被打掉的護盾會一併消失；'],
  ...
  [''],
  ['疊加規則 / 最大疊層'],
  ['refresh＝重新計時（後蓋前，增益的預設）；'],
  ['strongest＝取高並重新計時（目前持續傷害採用）；'],
  ['stack＝疊層：重複施加時「單層值取高、層數 +1」直到最大疊層，實際效果值＝單層值 × 層數；'],
  ['　　　　持續時間一律以最後一次施加重新計時，層數不會各自倒數；'],
  ['最大疊層＝疊加規則為 stack 時的上限（必須大於 1），其餘規則填 1；'],
  ['把任何一列的疊加規則改成 stack、最大疊層填 2 以上，該狀態立刻就會疊層，不必改程式；'],
  ['戰鬥畫面與提示框會在狀態名後面顯示「×層數」；'],
  [''],
  ['── 新增狀態注意 ──'],
  ...
  [''],
  ['施加特效 / 持續特效 / 作用特效（2026-09-03 VFX Preset 化；2026-09-18 狀態的畫面只填在這裡）'],
  ['中了狀態之後長什麼樣子只填在本表，不填在 Skills2——不論哪個技能附加這個狀態，畫面都一樣；'],
  ['每一格填 VFX 編輯器存出來的 Preset 檔名（vfx/presets/<檔名>.json，不含 .json；含 .json 也接受）；'],
  ['施加特效＝狀態第一次出現在目標身上時，在身體中心播一次（重新計時／疊層不重播）；'],
  ['持續特效＝狀態存在期間掛在目標腳底循環播放、畫在角色下面（Preset 應設 loop）；照 Preset 的世界尺寸畫，編輯器看到多大遊戲就多大；'],
  ['　代表一個範圍的狀態（以玩家為中心的領域：殺神領域、火神降臨、海淵葬界…）例外：持續特效依領域的實際半徑縮放並跟著玩家走，範圍變大畫面就跟著變大；'],
  ['　這類領域狀態的持續時間填 99999＝「領域在就一直在」，實際由技能決定何時撤掉；'],
  ['作用特效＝持續傷害每跳一次在身體中心播一次（只對 dot 有意義；同一拍多個敵人會合併成一則事件）；'],
  ['留白＝該狀態沒有那一段畫面。']
];
...
/* ---- Skills2 ← SKILLS2（js/skills2.js）新版主動技能群組（2026-08-13 技能改造第一批） ----
   ...
   「效果參數(JSON)」鍵值慣例：<鍵>＝不含升級效果的底值、<鍵>Per＝每級增量（值＝底值＋增量×等級）；
   Lv.1 就已含 1 級升級效果，底值本身不是任何實際等級會出現的數字。
...
const SKILLS2_GLOSSARY_ROWS = [
  ...
  ['環繞型技能（火狩）以此欄描述單一環繞體的體積（例如 3*3），半徑取長寬較大者的一半；'],
  [''],
  ['傷害類型 / 傷害屬性'],
  ...
  ['　　　　　反擊依最高已學習階段扣魔（超神優先，不累加）；二次反擊／狂化反殺觸發時另扣該階消耗；'],
  ['　　　　　主動技能目前只讀群組值，後續列填多少都不影響施放成本；'],
  [''],
  ['效果參數(JSON)'],
  ['其餘未拆成獨立欄的數值參數；距離、間隔、長寬與角度請改右側中文欄。命名慣例：<鍵>＝不含升級效果的底值、<鍵>Per＝每級增量，值＝底值＋增量×等級；'],
  ['　　　　Lv.1 就已含 1 級升級效果（底值＋增量×1），練滿 10 級＝底值＋增量×10；底值本身不會直接出現在遊戲裡；'],
  ...
  ['疾風斬調整：第 1 階 castM＝起手施放距離（米），gap＝每段攻擊間隔（秒，傷害／特效同步）；'],
  ['　　　　　　第 4 階 m＝以主目標為中心的擴散搜尋半徑（米）；第 7 階 m＝月牙傷害半徑（米），特效同步等比縮放；'],
  ['　　　　　　月牙使用第 7 階 castM，可與第 1 階獨立調整。range 欄不控制疾風斬／月牙斬；'],
  [''],
  ['解鎖轉生/等級'],
  ...
  [''],
  ['效果說明模板'],
  ['遊戲內顯示的說明文字；{鍵} 會代入目前等級的計算值（鍵名同各獨立數值欄對應的 fx 鍵或「效果參數(JSON)」）。'],
  [''],
  ['施放特效 / 攻擊特效 / 飛行子彈 / 受擊特效 / 地板特效 / 持續場域特效（2026-09-03 VFX Preset 化）'],
  ['每一格填 VFX 編輯器存出來的 Preset 檔名（vfx/presets/<檔名>.json，不含 .json；含 .json 也接受）；'],
  ['每一列＝該階（或該超神選項）「引入」的特效：第 1 階那一列是技能本體，後面各階只填那一階新增的畫面'],
  ['　　（例：火球術第 1 階＝火球＋受擊、第 3 階火球爆裂＝小火球＋大爆炸、第 7 階殞石術＝隕石＋落點預警）；'],
  ['程式依事件指定的來源列取用特效；本體特效逐欄繼承，觸發特效只讀本列，詳見置頂的本體／觸發特效說明。'],
...
// 排在置頂的本體／觸發特效說明（skills2Vfx.help）之後：置頂那一段是既有的閱讀順序（tests/skills2-vfx-usage.test.cjs 守著）
...
SCHEMAS.Skills2 = {
  ...
  rebuild(dataRows, header) {
    ...
    dataRows.filter(r => get(r, '群組ID').trim() !== '').forEach(r => {
      ...
      /* 每階的施法消耗供最高階基本消耗及追加效果使用；留白／0 不寫進字面值，
