...
rem Second marker on /__whoami.  Adopt only when the server explicitly says its
rem code is current: a server old enough to predate this handshake reports
rem neither marker, and "no answer" must count as stale.  Waiting for an old
rem process to admit it is old cannot work -- it has no code to do so.
set FRESH_MARK=idle-rpg-vfx-editor-ok
set PORT=
...
rem Flat, no parentheses: `call` plus `if defined` inside a block is exactly the
rem kind of batch construct that works until it doesn't.  Nothing here needs a
rem block, so don't use one.
call :scan
if not defined PORT goto :startnew
call :checkfresh
if not defined FRESH goto :stale
...
echo.
echo   （伺服器新到會自己回報狀態時，這裡才會放行；舊到不會回報的
echo     一律當成過期，因為舊程式沒辦法回報自己舊。）
echo.
...
rem ---- checkfresh : set FRESH only when the server says its code is current ----
rem Separate marker line on /__whoami, so the identity match above is untouched.
rem Absence of the marker means the server predates this handshake -- also stale.
:checkfresh
set FRESH=
curl -s --max-time 2 http://127.0.0.1:%PORT%/__whoami 2>nul | find /i "%FRESH_MARK%" > nul
if not errorlevel 1 set FRESH=1
goto :eof
