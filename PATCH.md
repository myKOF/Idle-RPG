...
- **規範寫入**：依使用者指示於 `AI_RULES.md` 第 16 節與 `docs/TASK238_ULT_SKILLS_DPS_REPORT.md` 第 1.1 節寫入「DPS 基準測試與變因控制規範」：
  - 只有該技能有特殊需求才允許替換（如反擊 2 階「招架」在副手盾牌配置 `blockRate`）。
  - 僅限替換掉防禦詞條，嚴禁替換掉會影響 DPS 的傷害相關詞條（如 `critDmg`, `atkPct`, `atkFlat`, `critRate`），確保 5996% 爆傷乘區與基礎物攻 100% 一致。
- **全量 96 場標準化重測**：
  - 涵蓋全部 8 大技能群組（突刺、迴旋斬、飛刀、疾風斬、血刃斬、雙刀亂舞、反擊、嗜血狂怒），共 24 個超神進化分支與 8 個基礎未進化對照組。
  - 【突刺 - 幻影八方陣】小怪群戰 DPS 達 **3,810,039,479（3.81B，1.77x 成長）**，完全符合「繼承 168 道八方貫穿彈幕 ＋ 12 米全額二次擴散」之正向成長預期。
