...
1. **【金剛不壞】的「生命值 +50%」目前只提高上限、不補血。** 拉高上限的同時送血等於每次施放
   白送一次治療，設計文檔沒有這一段；代價是施放瞬間生命**百分比**會下降（1000/1000 → 1000/1550），
   低血增傷類的效果會因此誤判。若希望改成「等比例一起漲」請說一聲。
2. **【金剛不壞】滿級的減傷是 99%**（90 ＋ 每級 0.9）。已夾在 99% 避免與其他來源疊成完全免疫，
   但這個量級本身要不要調，是參數表的決定。
3. **【伴生併發】的「1 個併生火狩飛出」實作成「額外放一團打出去」，不消耗場上的環繞體。**
   設計文字是「你的 1 個併生火狩會飛出攻擊」，若原意是**消耗**一團（打完就少一團），
   改法是把選中的環繞體從 `f.orbs` 移除；目前選的是不消耗，理由是沒投資第 3 階
   【伴生火狩】時場上根本沒有伴生體，消耗版會變成整條特效不生效。
4. **【無限星環】的螺旋外擴上限取 20 米**（設計文檔沒寫）。這個數字是我補的，
   取自「火狩本體環繞半徑 8 米」的 2.5 倍，需要調請直接改參數表 `m`。
5. **【火神降臨】的火狩星環威力取火狩第 1 階的每次命中傷害**（設計文檔沒給倍率）。
6. **【炎爆】沒有層數上限也沒有內部冷卻**（設計文檔未寫）：每滿 10 次就爆一次，
   命中頻率高時會相當密集。
