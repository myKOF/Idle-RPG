...
## UI 功能：場景頁籤 Tips 主要敵人屬性顯示百分佔比（2026-08-07）

- **屬性百分佔比**：修改 `js/ui.js` 之 `zoneElementTagsList`，計算各場景敵人屬性權重佔比，並於屬性標籤後方以括號顯示百分比（例如 `主要敵人屬性：🟢 毒屬性 (78%) 🟣 暗屬性 (9.8%) 🔥 火屬性 (4.9%)`）。
- **測試**：更新 `tests/zone-attr-tooltip.test.cjs` 包含屬性佔比驗證，全套件 1188 項測試與 `build_check` 全數通過。
