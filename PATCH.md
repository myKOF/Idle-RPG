...
- **寒霜的「疊層上限」與「凍結門檻」正式分家**（`sgApplyFrost`）：門檻恆為狀態表的
  ...
  【寒霜湧動】（規格帶著 `over`／`overPct`，只放寬水流彈自己塗的那一份）與超神
  【海淵葬界】（`sgAbyssOverStacks`，放寬領域內的每一份，不分來源）。上限再以「目前層數」
...
2. **「範圍增大 30%」放大的是爆散自己的 8 米半徑**，不是換成冰霜新星的表定半徑
   （設計文字接在「擴散爆炸」後面）。
3. **【寒霜湧動】的「每層再增加 50% 寒冰傷害」＝放大寒霜凍傷的每跳量**，而且只有
   **超過凍結門檻**的那幾層算。若原意是「該敵人受到的寒冰傷害 +50%」，那要改掛
   `skill2IceAmpACfg`（【寒冰逆轉】那一條），量級會完全不同。
...
- **【寒霜湧動】滿層時凍傷 ×3.5**（超出門檻 5 層 × 每層 +50%）；與【海淵葬界】同時成立時
  上限是 16 層，也就是 ×6.5。凍傷是施放當下定版的平坦 dps，不吃防禦，量級要一起看。
