...
## 超神進化全技能 DPS 基準測試與前後對比（Antigravity 2026-08-20）

- **測試覆蓋**：全 18 招超神進化分支（突刺、迴旋斬、飛刀、疾風斬、血刃斬、雙刀亂舞）以及 6 大技能群組未點超神進化（1~7 階滿級）基礎版本，共 72 場無頭戰鬥模擬。
- **測試環境**：全身 13 格 100 級傳奇裝備（強化 +40，3.0x 詞條倍率）、100 級冰原地圖（200 隻小怪波次串流、25 隻菁英波次串流、單體 BOSS）。
- **產出文件**：
  - `docs/TASK238_ULT_SKILLS_DPS_REPORT.md`：完整數據大表、各情境成長倍率、機制評析與 Tier List 矩陣。
  - `docs/AI_TASKS.md`：登錄 TASK-238 驗證與結果摘要。
  - `scratch/all_ult_dps_results.json` 與 `scratch/base_skills_dps_results.json`：原始模擬輸出數據集。


## 傳奇進化第三批：血刃斬／雙刀亂舞的十特效 ＋ 兩組超神進化（Claude 2026-08-20）

設計來源：使用者提供的「神力之巔_記事錄」試算表〈傳奇進化〉頁籤（血刃斬、雙刀亂舞兩段）。
前兩批的兩條收斂路徑（`legendarySkill2Mods` 傳奇橋、群組層 `ult` 超神進化）原封沿用；
本批唯一的新架構是**修羅亂舞**——全專案第一個「技能狀態決定裝備規則」的效果。

- **10 個新傳奇特效（`js/data.js` `PASSIVE_POOL`，池內共 68 個）**
  - 雙手魔劍 → 血刃斬 `bloodblade`：毒爆（中毒者死後留毒霧場域）、毒血祭（中毒每感染 1 人扣自身
    當下生命 1%，但中毒與流血傷害 ×1.5）、血霧（流血者死後留血霧，霧中敵人每次受傷回你 0.2% 生命）、
    血影（35% 揮出第 2 斬）、切割（只放大斬擊本體 +100%，DOT 基準不變）。
  - 雙手大劍 → 雙刀亂舞 `dualdance`：狂舞（暴風之舞期間每施放一次，下一拍間隔 ×0.94）、
    狂戰士（鐵血之舞比例 ×1.5，自傷與敵人流血同步）、不屈之誓（暴風之舞期間死亡延後 10 秒，
    期間所有傷害 +100%）、殺千刀（每殺 1 人化身 +0.2 秒）、雙生刃（擊中目標 +2）。
- **6 個新超神進化（`js/skills2.js` `SKILLS2[gid].ult`）**
  - 血刃斬：殺神領域（24 米永久領域，領域內擊殺疊增傷並回血，上限 100 層）、
    萬毒血霧（24 米永久領域，每 0.5 秒疊 1 層中毒，上限 10 層）、
    崩解（中毒與流血改為當場結清＋爆炸）。
  - 雙刀亂舞：毀滅之舞（每次施放付出當下生命，換 +200% 總傷）、
    火之神樂（每命中疊 1 層灼焰，上限 20 層）、修羅亂舞（見下）。

### 修羅亂舞：唯一一個改動裝備規則的技能效果

需求（使用者 2026-08-20）：裝備該技能才能同時裝兩把雙手武器；卸下後副手武器不再生效、
詞條加成恢復原樣。**資料不可遺失**，因此實作是「留在欄位裡但不計入」而不是「退回背包」。

- 判定入口只有一支：`skills2AsuraDualWield()`＝選了修羅亂舞（且前 7 階仍全滿）**且**
  雙刀亂舞裝配在技能列上。裝備端與屬性聚合全部呼叫它，不得各自重寫條件。
- 沿用既有的 `weapon2` 副手欄，**沒有新增第 14 個裝備欄位**：需求是「兩把雙手武器」，
  主手＋副手正好是兩把；新增欄位會多出一格所有人都看得到、只有這個效果用得到的 UI，
  而且角色底圖沒有第三個武器位置。
- 四個接點：`equipSlotsForItem`（雙手武器多一個可裝欄位）、`slotBlockedByTwoHand`（副手不再算被佔用）、
  `equipItem`（兩段互斥條件化；例外只開給「副手也是雙手武器」，拿盾仍會擠掉主手）、
  `computeStats`（沒生效時副手的雙手武器整件跳過；生效時雙手武器的**詞條**值 ×(1+20%)）。
- 主執行緒沒有 `G`，自己算一定得到「沒生效」，因此 `slotBlockedByTwoHand`／`equipSlotsForItem`
  多一個 `asuraOverride` 參數，由 `js/ui.js uiAsuraDualWield()` 以面板快照重算後傳入。
- **快取失效**：`markStatsDirty()` 原本沒有任何一個呼叫點與超神進化或技能裝載欄有關；
  少了它，玩家選／卸之後 `_statsCache` 會沿用舊值而且沒有任何機制會偵測到。
  已補在 `skills2UltPick`／`skills2UltLearn`／`skills2UltDowngrade` 與
  `equipSkillToLoadout`／`unequipSkillFromLoadout`。
- 面板：沒生效的副手雙手武器以灰階 ＋ 🚫 標示（`.eq-slot.eq-inert`），避免「面板掛著一把武器、
  屬性卻對不上」被當成算錯。

### 順帶收斂與修正

- **`skills2AllDamageUpPct(pEnt)`**：「你造成的所有傷害提高」原本在 `sgAtkCfg` 與
  `playerAtkCfg` 兩邊各自列舉（時空凝滯、死亡收割者），本批又要加兩個來源。
  改為單一加總入口，兩邊都讀它——否則下次新增來源必然會漏掉其中一邊。
- **`legendaryFx` 白名單補 `gap`／`interval`**：雙手武器補償會把 fx 數值鍵 ×2，
  但把「作用間隔」放大等於讓 DOT 跳得更慢＝變弱，方向與補償的用意相反。
  補之前唯一用到 `gap` 的是單手魔劍的風捲殘雲（倍率恆為 1），因此不影響任何既有數值。
- **`skills2OnEnemyDamaged` 拆分**：原本整支被「血飲術沒學就 return」擋在最前面，
  新增的血霧吸血掛不上去。拆成兩件互相獨立的事。

### 已知的設計取捨（不是漏判）

- 【崩解】與傳奇【毒爆】／【血霧】天生互斥：崩解讓中毒與流血當場結清，敵人死時身上不會有
  這兩個狀態，那兩個場域幾乎不會觸發。
- 【殺神領域】依賴敵人死亡掛勾，而該鏈只由野外擊殺（`onFieldKill`）驅動，**高塔沒有鏡射**，
  因此在高塔不生效——與死亡屍爆／爆燃／死亡新星同一條既有邊界。
- 【毀滅之舞】的生命代價取「當下生命」的比例並保底留 1 點，因此暴風之舞每 0.35 秒一次的
  自動施放會讓生命指數逼近 1 而不會踏過死亡線，並與 T6【嗜血狂化】形成刻意的正回饋。

### 驗證

- `npm run build` 296 檔通過；`npm test` 1678 案例／1671 綠，7 紅全為改動前既有
  （skill2-earth ×2、skill2-ice ×4、skill2-vfx ×1）。
- 新增 21 個測試：`tests/skill2-ult-evolution.test.cjs` 補 15 個（16 個效果的行為），
  新檔 `tests/skill2-asura-dualwield.test.cjs` 6 個（修羅亂舞的判定、穿戴互斥、自動穿裝、
  屬性聚合的生效與復原、詞條加成範圍、快取失效）。
- 參數表三張以 `--gen` 重生（Skills2 179 列／Status 71 列／Equipment_Affix 160 列），
  `--apply` 往返後 JS 無語意變更。Worker 協議版本不變（v23，沒有新增指令）。

## 傳奇進化第二批：飛刀／疾風斬的十特效 ＋ 兩組超神進化（Claude 2026-08-20）

設計來源：使用者提供的「神力之巔_記事錄」試算表〈傳奇進化〉頁籤（飛刀、疾風斬兩段）。
承接 2026-08-19 第一批建立的兩條收斂路徑（`legendarySkill2Mods` 傳奇橋、群組層 `ult` 超神進化），
本批**沒有新增架構**，只是把新的資料與施放端掛鉤接上去。

- **10 個新傳奇特效（`js/data.js` `PASSIVE_POOL`，池內共 58 個）**
  - 匕首 → 飛刀 `knife`：連鎖（彈射 +2）、分裂者（擊殺分裂 2 把 50% 小刀、各再彈射 2 次）、
    處刑者（彈射優先跳向生命值最高者並 +30% 總傷）、影刃（飛刀數 +2）、
    輪舞刃（第 1 把改為周身 10 米繞行 5 秒的刀環）。
  - 單手魔劍 → 疾風斬 `gale`：風捲殘雲（命中 35% 生成 4 段風系龍捲風）、斬殺（35% 秒殺 20% 血以下的非 BOSS）、
    雙影（50% 對附近 1 人追加 50% 傷害）、風行者（附加**既有的風切狀態**）、神速斬（每次命中冷卻 -0.1 秒）。
  - **名稱決策**：本批名稱全部照設計文檔原文，唯一例外是【神速】——`PASSIVE_POOL` 已有同名詞條
    （`godHaste`，攻速 +15%）。玩家只看得到名字，同名等於裝備詞條無法分辨，故改為【神速斬】。
    測試新增「傳奇特效池顯示名稱必須全域唯一」的守門。
- **6 個新超神進化（`js/skills2.js` `SKILLS2[gid].ult`）**
  - 飛刀：暴雨梨花（飛行路徑上的敵人各吃一段）／死亡收割者（擊殺疊層增傷）／無限追魂刃（額外一支高傷無限彈射刀）。
  - 疾風斬：霹靂一閃（最後一斬對周身打「單段 × 連擊數 × 倍率」閃電傷害）／雷神斬（每次斬擊附加落雷）／
    千鳥（月牙斬不再均分傷害、每人吃完整傷害並再 +50%）。
  - 解鎖條件、選擇／升級／降級、存檔與 UI 全部沿用第一批，`SG_TIER_COUNT` 仍是 7、Worker 協議版本不變。
- **既有 Bug 修復：`sgWindRend` 沒有狀態表列（`js/status.js`）**
  - `sgApplyWindRend` 一直在塗 `sgWindRend` 這個鍵，但 `STATUS` 沒有對應列，`applyStatus` 查不到就靜默
    `return false`——**真空斬【風切】與暴風屏障【亂風切】的移速 -80%／命中率 -50% 從來沒有生效過**，
    只有風系持續傷害（`sgWindCut`）在跑。新傳奇【風行者】重用同一支，因此必須先補這一列。
  - 補上後 `tests/skill2-wind.test.cjs` 原本紅的 3 個案例轉綠（層數上限與每層加傷）。
- **收斂點（沿用既有設計，不另建一套）**
  - 飛刀的四種發射來源（主刀／彈射／分裂刃／追魂刃）共用 `sgKnifeHit` ＋ `sgKnifeBounceChain`，
    衍生命中一律帶 `derived` 旗標＝不再往下分裂，避免無限展開。
  - 疾風斬的三條結算路徑（單體、擴散、月牙斬均分）共用 `sgGaleOnHit`。
  - 【死亡收割者】的傷害增益同時掛在 `sgAtkCfg`（新版技能）與 `playerAtkCfg`（普攻）的 `totalDmgPct`，
    與潛力【時空凝滯】`allDmgUp` 相加——「造成的所有傷害提高」只有這一組鍵。
  - 【輪舞刃】用 `sgSpawnOrbitField`（火狩／雷球／虛空斬同一套），畫面沿用 `void-disc` 並把屬性改物理；
    另立新變體會在兩個渲染器都沒有分支時整個沒有畫面。
  - 【無限追魂刃】的「彈射次數不受限制」以「場上每個敵人各被命中一次」收斂：彈射規則允許 A→B→A，
    單純不設上限會讓迴圈不終止。
- **參數表**：Skills2／Status／Equipment_Affix 三張表以 `--gen` 重新產生（CSV ＋ xlsx），
  `套用參數.bat` 往返後 `js/` 無差異。
- **快取版號**：`index.html`（data 1.0.33／status 1.0.12／skills2 1.0.57／combat 1.0.34／bridge 1.0.12）、
  `js/worker/sim.worker.js` 與 `js/bridge.js` 的資產版本改為 `20260820-ult-evolution-2`。
  **Worker 協議版本不變（v23）**——沒有新增或修改任何指令。
- **測試**：`tests/skill2-ult-evolution.test.cjs` 由 18 → 26 個案例（新增飛刀／疾風斬各五個傳奇特效、
  六個超神效果、名稱唯一性、Status 表落表）；`tests/legendary-affix.test.cjs` 特效總數 48 → 58；
  `tests/skill2-vfx.test.cjs` 更新飛刀彈射的原始碼守門與資產版號。
...
## 傳奇進化十特效 ＋ 超神進化（技能第 8 格三選一）（Claude 2026-08-19）
...
- **參數表管線：「施法消耗」升格為每階真欄位**：
  - `tools/config_tables.cjs` 的 Skills2 schema 改為把每一列的「施法消耗」寫進 `tiers[i].cost`（0／留白不寫入），`extract` 也改由 `tiers[i].cost` 回寫該欄；原本「其餘列僅供對照」的註記機制只留給「冷卻時間」。
  - 以 `--apply --write Skills2` 重建 `SKILLS2` 字面值：23 個群組每階都帶上 `cost`，除新增欄位外語意零變更（已逐行比對驗證）。數值本身沿用使用者 2026-08-19 已同步的 Excel/CSV，不需再改參數表。
- **反擊逐階扣魔（`js/skills2.js`）**：
  - 新增 `SG_TRIGGER_MP_TIERS`（哪些階有觸發時機）、`skills2TierTriggerMp()`（該階觸發一次要付的法力）、`sgCounterPayMp()`（付得起才扣、扣不起回報不觸發）與 `skills2PassiveMinMp()`（UI 的法力門檻）。
  - `sgCounterOnPlayerDamaged` 的六個觸發點各自付費：T1 反擊 5、T2 招架 10、T4 反擊盾 40、T5 破甲擊 60、T6 二次反擊 80、T7 狂化反殺 100；T3【強化反擊】是恆時傷害加成、無獨立觸發時機，不扣魔。
  - 只在「效果確定要發生」時扣魔：機率沒中、攻擊者已死、狂化反殺範圍內沒有目標都不扣；扣魔順序＝結算順序，付不起的那一階這一次不觸發、其餘階照常（低魔時自然退化成只剩便宜的階生效）。
- **UI（`js/ui.js`）**：技能升級面板與提示框對被動群組改顯示該階的「🔵 N MP／次觸發」，群組提示的各階清單逐階標出消耗；法力低於「已投資各階最便宜的觸發消耗」時，快捷列與戰鬥技能列不再宣稱恆時生效，改用一般技能的無魔樣式。
- **連帶修正：法力不再被扣成負數（`js/skills.js`）**：舊技能的扣魔（`pEnt.mp -= manaCost`）沒有下限夾。法力門檻是在「開始吟唱」時檢查的，吟唱途中被其他來源吃掉法力（現在的反擊逐階扣魔、既有的魔法盾代付傷害）就會扣成負數，回復還得先把負數填回來。改為 `Math.max(0, …)`，與 `castSkill2`／魔法盾同一慣例；實機驗證前有 −11.8 的取樣、修正後 60 次取樣最低 1、無負值。
- **測試與文件**：
  - `tests/skill2-counter-bloodrage.test.cjs` 新增 §3.5 共 7 個法力案例（每階消耗來源、觸發才扣、不足不觸發且不扣、各階各自判定、T3 不扣但加成照吃、T6/T7 各自付費、反殺無目標不扣、零魔全停擺），既有機制案例改用充足法力以維持原本的驗證目標。
  - 一併修好兩個先前就紅著的斷言：嗜血狂怒的扣魔改讀參數表群組值（原本寫死 50，表上早已是 25）；`tests/skill2-system.test.cjs` 的「被動群組無耗魔」不變式改為只要求無群組冷卻。
  - 更新 `game_formula.md` 反擊／主動型被動條目與 `docs/SKILL_TEST_SPEC.md` 的反擊測試須知。
  - 快取破壞：`index.html`（`js/skills2.js?v=1.0.51`、`js/ui.js?v=1.0.52`、`js/skills.js?v=1.0.23`、`js/bridge.js?v=1.0.10`）；戰鬥邏輯跑在 Worker，故一併換 `WORKER_ASSET_VERSION` 與 `js/worker/sim.worker.js` 內 `skills.js`／`skills2.js` 的 `?v=`（`20260819-counter-mp`），否則 Worker 會繼續執行舊的技能程式；`tests/skill2-vfx.test.cjs` 釘住版號的那組斷言同步更新。
...
- **高風險與機制驗證（E1~E33）**：寒霜疊層與 95% 緩速、傷害不隨層數提高、滿層瞬間凍結與重塗不重置、BOSS 免疫凍結但可疊層、極致寒霜跨群組放大、貫穿冰箭自動延長、寒霜凍結結清凍傷、追蹤冰箭接觸判定與穿梭、凍結結束冰爆、寒冰逆轉時序與獨立乘區、寒流爆散彈射、寒霜擴散、水龍捲追加與凍結 2 倍、冰霜新星自身 12 米半徑施放與射程成長、寒冰體受擊附霜、暴風雪跟隨玩家等 33 項 100% PASS。
- **雙路徑專屬特效（V-I1~V-I10）**：Canvas 與 DOM 雙路徑驗收通過（寒冰箭與貫穿冰箭、追蹤冰晶穿梭、冰爆圓環、水流彈 8 米拋物線、四頂點水龍捲、冰霜新星擴散圓、暴風雪粒子與跟隨場域），Console 零報錯。
- **報告路徑**：`docs/skill-tests/20260818-ice-three-antigravity.md`。

## 冰系三大新技能：寒冰箭／水流彈／冰霜新星（Claude 2026-08-17）

設計文檔〈魔法〉區塊再新增三個群組共 21 階，全部是魔法傷害／寒冰屬性。

三棵樹共用同一個核心狀態【寒霜】——文檔把寒霜的整段說明重複寫在寒霜箭（寒冰箭 T2）、
寒流彈（水流彈 T3）與冰霜新星（T1）三處，實際上是同一個狀態，因此寒霜寫在群組共用層，
行為參數（單層緩速、層數上限、跳動間隔、持續時間、凍結秒數）一律以狀態表為權威。

**寒冰箭 icearrow（CD 18 秒／射程 30 米／解鎖 Lv.450~750）**
  三種形態共用同一支命中結算：單體冰箭（扇形 45 度挑目標，一支箭一個敵人）→
  貫穿冰箭（T4，路徑貫穿）→ 追蹤冰箭（T7，貫穿後轉為追擊場域）。
  T4 的貫穿長度以「打得到主目標」為地板：文檔字面的 10 米＋每級 2 米會讓投資 T4 的
  射程 30 米技能只剩 12 米行程、遠處主目標反而打不到，這在升級語意上是降級。
  T6【寒霜凍結】把還沒跳完的凍傷一次結清（比照血刃斬【零日感染】），結清後那筆凍傷結束；
  追加層數在結清之後才塗，新塗上的一份不會被同一次結算清掉。
  T7 的冰爆掛在「凍結結束」的時刻，走完整傷害管線（不是衍生傷害）。

**水流彈 waterball（CD 14 秒／射程 30 米／解鎖 Lv.500~800）**
  拋物線水彈；離地最高點 8 米是表定值，兩條渲染路徑都吃同一個數字（見下）。
  T2【寒冰逆轉】強制改寫敵人的屬性標籤，掛在 monsterDefCfg 的 attr 欄——那是全專案
  「防守方屬性標籤」的唯一出口，因此攻方的「對屬性敵人傷害%」與守方的「對屬性敵人抗性%」
  兩條既有規則會一起認得改寫。逆轉塗在傷害之後：本次命中不吃自己造成的受傷增幅
  （比照落雷術「先暈後增傷」的既有處理原則）。
  T4【寒流爆散】依文檔為「改為」＝範圍＋彈射；T7【水龍捲】文檔未寫「改為」故為追加
  （比照雷殞天落的既有決策），四道地板場域落在我方 10×10 米正方形的四個頂點。

**冰霜新星 frostnova（CD 20 秒／解鎖 Lv.550~850）**
  以自身為圓心的範圍爆發，是三棵樹裡唯一不需要目標座標的技能（高塔也完整生效）。
  T2【冰霜衝擊】的 13 米以 max 為地板，避免第 1 階練滿後投資第 2 階反而縮小範圍。
  T4【極致寒霜】依文檔「寒霜狀態的傷害／持續時間」沒有限定來源，掛在寒霜共用層，
  三個群組塗出來的寒霜一起被放大。
  T7【暴風雪】同樣為追加，是一道跟隨我方的地板場域。

**引擎新增四個群組共用能力（收斂點，非本三技能特例）**
  13. 寒霜狀態（sgApplyFrost／sgFrostStacks／sgTickFrost）：可疊層的緩速兼持續傷害，
      疊滿即凍結。同時把「場域型緩速」收斂成 skill2SlowAspdFactor／skill2SlowMoveFactor，
      formula.js 與 battlefield.js 從此只認得通用緩速一個掛點（泥沼與寒霜相乘）。
  14. 敵人屬性標籤強制改寫（skill2ForcedAttr）＋單一屬性的受傷增幅（skill2IceAmpACfg，
      掛 resolveHit 既有的 skillElemAmp 每系乘區，刻意不混進 totalDmgPct——那會把
      同一次攻擊的火／雷等其他屬性段一起放大）。
  15. 跟隨我方的地板場域（sgSpawnGround 的 follow）：圓心恆等於玩家當下座標。
  16. 追擊場域（sgSpawnGround 的 chaseM ＋ contact）：抵達落點後改鎖範圍內的隨機敵人
      繼續飛，並採環繞場域的接觸判定（進入才算一次命中）。追蹤冰箭因此不必另寫模擬迴圈，
      也不會退化成「每個節拍都全額命中」的傷害爆炸。
  另外兩處小幅泛用化：sgTryStun 改回傳實際暈到的秒數（凍結標記要跟著控場遞減後的
  真實長度）、skills2CastRangePx 改吃 sgVal（支援 castMPer；既有群組都沒有定義，
  回傳值與改造前完全相同，測試中有一項專門守住這件事）。

**使用者決策（實作前確認）**
  - 【寒霜狀態】的持續傷害**不隨層數提高**：層數只累積移動與攻速的下降，疊滿才凍結。
    因此寒霜刻意拆成 sgFrost（層數＋緩速，疊層規則）與 sgFrostBite（傷害，取高並重計）
    兩筆狀態——併成一筆走疊層規則會變成「單層值 × 層數」而違反此決策。
  - 【凍結】**走既有控場管線**：行動限制本身交給暈眩承擔，因此完整沿用 BOSS 控場免疫、
    韌性折減與控場遞減。BOSS 疊得滿寒霜層數但不會被凍結。

**表現層與計算層同步（AI_RULES 8.3）**
  - 三種新場域（暴風雪矩形／水龍捲圓形／追蹤冰箭小圓）在 Canvas（spawnIceField）與
    DOM（vfxIceField）兩條路徑都以模擬層送來的 area 為唯一錨點，畫面範圍＝實際判定範圍。
  - 水流彈的拋物線弧高改由事件帶出（sgEmitVfx 的 arcM），兩條路徑各自換算成畫面單位；
    原本所有投射物共用的固定 18px 微弧退為「沒帶 arcM 時」的預設值。

**修改檔案**
  `js/skills2.js`（群組表＋冰系實作＋四個共用能力）、`js/status.js`（sgFrost／sgFrostBite／
  sgFrozen／sgIceRevert 四筆狀態）、`js/combat.js`（monsterDefCfg 屬性改寫掛點）、
  `js/formula.js`（slowFactor 改吃通用緩速）、`js/battlefield.js`（敵人移速同上）、
  `js/vfx.js`／`js/battle-renderer.js`／`css/style.css`（冰系場域與拋物線）、
  `config/CSV/Skills2.csv`＋`config/Excel/Skills2.xlsx`（+21 列）、
  `config/CSV/Status.csv`＋`config/Excel/Status.xlsx`（+4 列）、
  `index.html`／`js/bridge.js`／`js/worker/sim.worker.js`（快取版號）。

**驗證項目**
  - 新增 `tests/skill2-ice.test.cjs` 34 項全通過；`tests/skill2-vfx.test.cjs` 補 2 項（18 項全通過）；
    `tests/skill2-system.test.cjs` 群組清單更新為 20 個。
  - 完整 `npm test`：1562 項 / 1558 通過 / 4 失敗；同一 commit 的乾淨基準線為
    1526 項 / 1522 通過 / **同樣 4 項失敗、失敗原因逐字相同**（既有參數表漂移：
    `counter` 施法消耗、`bloodrage`、泥沼術範圍兩項）。本次新增 36 項全數通過、零回歸。
  - `node tools/build_check.cjs` 290/290 通過。
  - `config_tables --apply` 試跑：Skills2 與 Status 皆「語意變更 0」（表與程式完全往返）。
  - `apply_params` 試跑「將變更 0、錨點問題 0」，對應參數總數 554 與基準線一致；
    `--check-anchors` 554 個錨點各命中一次。

**已知風險／待確認**
  - 寒霜疊滿 5 層＝緩速 100%，實作沿用既有的 95% 夾限（與泥沼同一條規則），
    等於 BOSS 也會被壓到 -95% 攻速與移速。這是文檔字面值（每層 -20% × 5 層）的直接結果，
    若要調整，改 `config/Excel/Status.xlsx` 的 `sgFrost` 單層值即可（引擎不寫死）。
  - 三種新特效（暴風雪、水龍捲、追蹤冰箭）與水流彈拋物線尚未實機目視確認：
    程式面已由原始碼定向測試守住 Canvas 與 DOM 兩條路徑的接線與參數傳遞，但畫面需實機檢查。
  - 設計文檔的水流彈階序有一處筆誤：【寒流彈】與【寒流爆散】都標為 3 階，
    且其後直接跳到 5 階；已判定【寒流爆散】為第 4 階實作。
...
## 新版技能全群組數值計算與顯示驗證報告（Antigravity 2026-08-17）

- **測試對象**：新版技能系統全 17 群組 × 7 階（共 119 階）數值算法（`base + per * lv`）
- **測試報告**：`docs/skill-tests/20260817-skill2-value-formula-antigravity.md`
- **驗證摘要**：
  - **A 類主線顯示（A1~A8）**：全 17 群組 × 7 階（119 階）滿級值（Lv.10）100% 精確吻合設計文檔「底值 + 增量 × 10」；Lv.1 顯示「底值 + 增量 × 1」；增量嚴格線性；定值參數保持恆定；負增量階（火球術 T4、大地守護 T7）全程 > 0；Lv.0 顯示 Lv.1 數值，Lv.10 自動抑制下一級預覽（100% PASS）。
  - **B 類實測傷害（B1~B7）**：突刺、飛刀、火球、火狩、連鎖閃電、落雷術實測飄字傷害倍率與面板數值完全相符；物攻/魔攻歸屬無誤；多階累加增傷結算準確；DoT 每跳與間隔吻合（100% PASS）。
  - **C 類版本一致性（C1~C4）**：主執行緒 (`v=1.0.35`) 與 Worker (`v=20260817-tier-level-includes-first-upgrade`) 載入一致，一般重新整理與 Ctrl+F5 均無快取漂移（100% PASS）。
  - **D 類機率追加次數（D1~D4）**：連鎖閃電 T3【雷鳴術】Lv.1（add=1.1）10,000 次測試統計 1 次命中佔 90.79%、2 次命中佔 9.21%，嚴格保底 1 次；Lv.10（add=2.0）100% 穩定輸出 2 次（100% PASS）。
  - **E 類回歸（E1~E6）**：舊技能系統、潛力/天賦、存檔往返、技能快捷列、傷害統計面板全數正常，Console 零 Error（100% PASS）。
  - **F 類平衡觀察（F1~F3）**：相較改動前，新版技能全 1 檔位 DPS 提升約 +10.0%，全滿檔位 DPS 提升約 +5.26%，新手前期拓荒流暢度提升，高塔極限層數推進 1~2 層，無斷層式失衡。
- **單元測試與建置檢查**：`node --test tests/skill2-*.test.cjs` 通過，`node tools/build_check.cjs` 287/287 檔全數通過。

## 雷系三大新技能（連鎖閃電／落雷術／雷球）實機與回歸測試報告（Antigravity 2026-08-17）

- **測試對象**：`chainlightning`（連鎖閃電）、`thunderstrike`（落雷術）、`thunderorb`（雷球）
- **測試報告**：`docs/skill-tests/20260817-lightning-three-antigravity.md`
- **驗證摘要**：
  - **§2 共用基建泛用化回歸（R1~R8）**：全數 PASS。火狩環繞/伴生/延長、殞石術天降錯開落地、火龍捲/無限火牆原地釘死、火球術飛行引爆、泥沼術靜止成長、狀態列獨立圖標（☄️/🔵）、控場遞減與 BOSS 免疫、傷害統計面板均保持原始行為。
  - **§3 高風險決策點（E1~E19）**：全數 PASS。連鎖閃電 30m 彈射範圍邊界、雷幻身單體中繼吃滿整條鏈、電殛擴散僅彈射追加、雷鳴術額外擊數、雷電暴風三道鏈並發、落雷術落地才結算、雷電脈衝範圍暈眩、殛道落雷先暈後增傷 30%、迅雷重生 5 次收斂上限、雷球移動場域 6m/s 與停留 2s、擴增雷球 +15% 範圍、環體電球雙球環繞與 🔵 狀態、伴生雷球生成靜止雷球、強化雷球三形態增傷 +30%、雷殞天落追加召喚等 19 項全部符合規格。
  - **§4 專屬特效目視驗證（V-L1~V-L7）**：全數 PASS。Canvas 與 DOM 雙渲染管線正常呈現藍白連鎖電弧、落雷天降電光、雷殞天落深藍巨球與地面衝擊波、雷球電漿球與停駐脈衝，Console 零 Error。
  - **單元測試與建置檢查**：`node --test tests/skill2-lightning.test.cjs` 29/29 PASS，`node tools/build_check.cjs` 287/287 PASS。

## 雷系三大新技能：連鎖閃電／落雷術／雷球（Claude 2026-08-17）

設計文檔〈魔法〉區塊再新增三個群組共 21 階，全部是魔法傷害／雷屬性。

- **連鎖閃電 `chainlightning`（主動，CD 18 秒／射程 30 米）**：一道在敵人之間逐跳彈射的閃電鏈，
  每一跳都接在前一跳的飛行時間之後（傷害飄字與電弧同步，比照飛刀的彈射鏈）。
  - 彈射目標＝「彈射範圍 30 米內、本輪還沒跳過的最近敵人」；跳不到就結束。
  - 第 6 階【雷幻身】是唯一的例外：沒有其它目標時以玩家自己當中繼點，清空走訪紀錄後繼續彈射，
    中繼那一下不消耗彈射數 → 單一敵人也吃得滿整條鏈（A→自身→A→…）。
  - 第 5 階【電殛擴散】只在「彈射」時追加，起手那一擊不算（否則單體時也會白拿一份擴散）。
- **落雷術 `thunderstrike`（主動，CD 14 秒／射程 30 米）**：天降打擊，落地那一刻才結算傷害
  （沿用殞石術的同一條佇列）。每道間隔 0.2 秒錯開。
  - 第 5 階【雷電脈衝】的暈眩塗在傷害之後，第 7 階【殛道落雷】對「已在暈眩中」的敵人才加成——
    同一次施放先落的雷把人暈住、後落的雷吃到 +30%，這個順序是刻意的。
  - 第 6 階【迅雷重生】的接力有表定上限（`max`），避免機率接力在極端情況下無限延伸。
- **雷球 `thunderorb`（主動，CD 20 秒／射程 30 米）**：三種形態共用既有基建，沒有新的模擬迴圈。
  - 飛行雷球＝地板場域＋移動；環體電球＝環繞場域（與火狩同一套接觸判定）；
    雷殞天落＝天降打擊佇列（與殞石術同一條落地時間軸）。
  - 第 7 階【雷殞天落】依使用者決策為**追加**而非「改為」（文檔未寫「改為」）：飛行雷球照常召喚。
  - 第 6 階【伴生雷球】的機率「一顆環繞體一拍只判定一次」，即使同一拍掃到多個敵人也只算一次。

**引擎新增三個群組共用能力（收斂點，非本三技能特例）**

1. 移動場域：`sgSpawnGround` 的 `from`／`dest`／`speed`，場域可沿直線飛行、抵達後停駐
2. 天降打擊佇列泛用化：`sgQueueMeteor` 的 `extra`（gid／特效變體／落地回呼／每目標傷害加成）
3. 環繞場域泛用化：`sgSpawnOrbitField` 的 `statusId`／`auraVariant`／`hitVariant`／`onStrike`

火系既有行為完全不變（火狩仍用 `sgFirehunt` 狀態與火焰爆點、殞石仍走 `fireball` 落地結算），
測試中各有一項專門守住這件事。

**數值校正（使用者決策 2026-08-17，實作前確認）**

- 【電殛擴散】文檔寫「每級 +25%」，與其它擴散階（+2%／+3%）差一個數量級 → 實作為**每級 +2.5%**。
  **這一格是校正過的：若要改回文檔原值，請在 `config/Excel/Skills2.xlsx` 的
  `chainlightning` 第 5 階把 `pctPer` 由 2.5 改為 25。**
- 【雷幻身】的 +50% ＝整道鏈恆時生效（比照第 2 階的寫法）。
- 【雷殞天落】＝追加，不取代飛行雷球。

**解鎖門檻**（延續既有魔法系的節奏，各階 +50 級）：連鎖閃電 0|300～0|600、
落雷術 0|350～0|650、雷球 0|400～0|700。

- **測試與驗證**：
  - 新增 `tests/skill2-lightning.test.cjs`（29 項）；`tests/skill2-vfx.test.cjs` 補 1 項顯示層接線；
    `tests/skill2-system.test.cjs` 的群組清單改為 17 個。
  - `npm test`：1519 項，1513 通過／6 項失敗——與乾淨基準線（1489 項、1483 通過、同樣 6 項）
    **逐項相同**，皆為既有的參數表漂移（火狩三項、counter 冷卻消耗、嗜血狂怒、ui.js 版號）。
  - `npm run build`：287 檔全數通過。
  - `node tools/config_tables.cjs --sync` + `--apply`（dry-run）回報「語意變更 0」。
- **快取版號**：`index.html` 的 `style.css`→1.0.39、`status.js`→1.0.8、`vfx.js`→1.0.37、
  `battle-renderer.js`→1.6.66、`skills2.js`→1.0.33；Worker 的 `status.js`／`skills2.js` 改為
  `?v=20260817-lightning-skills`。
...
- **岩甲術 `rockarmor`（主動，CD 25 秒／射程 30 米）**：施放給自己一層占最大生命比例的護盾，
  並在護盾存在期間開啟一整組「以護盾為燃料」的效果。
  - 生效條件（使用者決策）：第 4 階【護盾增幅】＝**主動型被動**，裝配到技能列即恆時生效；
    第 3、5、6、7 階一律綁岩甲護盾。否則第 7 階【天地逆返】會變成「平時 0 護盾＝白拿滿額減傷、
    放了技能反而變弱」的反向設計。
  - 第 6／7 階的分母是「施放當下的護盾總量」（`SKILL2_RT.rock.base`），不是 `shieldMax`——
    護盾被打光的那一刻 `shieldMax` 會歸零，分母跟著消失，最後一段損失就漏算了。
  - 第 3 階【岩甲尖刺】走**獨立的地屬性傷害管線**（吃地屬性加成與敵人地抗），與反震各自結算。
- **泥沼術 `mire`（主動，CD 18 秒／射程 20 米）**：10×10 米的地板場域，本體**不造成任何傷害**，
  只發三種 debuff：緩速（`sgMire`）、中毒（`sgMirePoison`）、熔岩灼燒（`sgMireLava`）。
  - 緩速另立 stat 減益而不用既有的 ctrl `slow`：`slow` 是固定 −30% 攻速、且吃控場遞減與韌性，
    一個每 0.5 秒重塗的場域會被遞減瞬間打成 0 秒；本技能還要同時降攻速與移速、強度會被第 6 階換代。
  - 狀態是「站在裡面才有」：每跳重塗、持續時間只給兩跳，離開後很快自然消失。
  - 持續時間取各改寫階的**最大值**（第 1 階滿級 7.6 秒 ＞ 第 5 階表定 6 秒），避免升級變降級。
- **大地守護 `earthguard`（主動式被動技）**：七階全是掛在既有收斂點上的乘區與攔截，沒有主動施放。
  傷害減免與生命上限、全屬性傷害、生命／法力回復與吸血／吸魔（**兩個不同倍率**）、
  魔法盾（法力承傷，付得起多少算多少）、生命反射之盾、天地共生（死亡攔截＋自身冷卻顯示於技能格）。
- **引擎新增的五個群組共用能力**（都是收斂點，不是這三個技能的特例）：
  我方防禦側乘區、護盾效率乘算、可變緩速（攻速＋移速）、法力承傷、復活攔截。
- **參數表產生器修正**：`--gen Skills2` 過去會把「冷卻時間／施法消耗」在階數 ≥2 的**人工註記值**
  用群組值蓋掉（那兩欄程式只讀階數＝1 那一列）。比照 Skills 表的「完整描述」，重建時從現有 CSV 帶回來。
...
每一階各自設門檻；留白＝無門檻。目前的配置：突刺／劇斬／火球 `0|1` 起手，飛刀 `0|50`、狂風 `0|100`、
火狩 `0|100`、血刃 `0|200`、雙舞 `0|250`、反擊 `0|300`、嗜血狂怒 `0|400`，每階往上加 50 級。

- **進度比較是「轉生數優先，同轉生數才比等級」**：達到 1 轉即視為 0 轉的門檻全部通過，2 轉視為 1 轉以前全通過。
  轉生會把人物等級打回 1，若改用「兩者都要達標」，每次轉生都會把整份技能表重新鎖上。
  要卡「1 轉之後才開放」的玄階，門檻就填 `1|200`。
- **未解鎖的階一律視為 Lv.0，含第 1 階的預設開啟**：技能本身也不能施放、不能裝配生效。
  已投入的等級留在存檔裡不會消失，達到門檻就原樣回來。
- 升級、一鍵滿級都擋；技能面板的階段節點依門檻置灰，升級彈窗與 tooltip 會直接說出「需達到 ×× 級才能解鎖」。
...
## 普攻不出手與擊殺無動作修正（Claude 2026-08-16）

使用者回報「轉頭後不會馬上普攻、常常站在原地好幾秒也不會普攻」。查出來是兩個各自獨立的缺陷疊在一起，傷害其實一直都有算。

- **模擬層：普攻冷卻在沒有敵人時整個停住**（`js/combat.js`）
  - `fieldTick` 把 `p.atkCd` 的遞減放在「場上沒有可交戰敵人就 return」與「施放技能中」兩道閘門之後，
    於是整波清空的空窗、新怪還在進場（0.45~1 秒）、施放硬直（0.2 秒）期間冷卻完全不走。
    敵人終於走到面前時冷卻還停在滿值，還要再等一整個攻擊週期才出得了手。
  - `atkCd` 是「下一刀什麼時候好」的計時器，不是「現在有沒有人可以打」；改成每個 tick 固定遞減一次，
    仍然夾在 0（＝ready），`4be1b10`「追擊時不得累積負數欠債」的行為不變。暈眩期間維持停住。
  - 實測（真實 `fieldTick` 探針）：敵人站在近戰距離內、進場倒數 3 秒的情境，出手時間由 t=5.0 提前到 t=4.0。
- **顯示層：擊殺的那一刀不播出手動作**（`js/battle-renderer.js`）
  - 普攻特效會延後 240ms 才播（與位置緩衝對齊），而面板同步早就把被殺的敵人標成「垂死」了。
    守門把垂死也當成失效，等於**每一記擊殺都丟掉自己的劍氣與角色動作**——一刀一隻時幾乎永遠不播普攻動作。
    傷害飄字走另一條判定（只擋已離場），所以數字照跳、動作沒播，看起來就像沒出手。
  - 失效條件收斂成「已離場」，與飄字同一條線。垂死期間敵人還在畫面上演死亡動畫，劍氣打在牠身上正是擊殺該有的畫面。
    `e0fb728`「戰鬥結束後不得殘留動作」的原始意圖不變（實體銷毀後仍會擋下）。
- **副作用（需留意）**：施放硬直期間冷卻改成照走，技能後銜接普攻變順，DPS 略升；
  這符合 `SKILL_CAST_LOCK`「技能不改動普攻 atkCd」的既有設計註記。暈眩維持凍結，不動控場強度。
...
## 新版技能第四批：魔法系「火狩」與環繞場域（Claude 2026-08-16）

- **設計來源**：記事錄「技能」頁籤〈魔法〉區塊新增的技能群組「火狩」（7 階、冷卻 26 秒）。
- **引擎擴充（群組共用能力，不是這個技能的特例）**：
  - **環繞場域** `SKILL2_RT.orbits`：釘在**玩家身上**、持續旋轉的環繞體。與地板場域（火柱）的差別
    只在錨點與命中判定——地板場域釘在座標上按節拍反覆作用，環繞場域以**接觸判定**命中
    （進入才算一次，離開後再碰到才會再命中），因此「碰到敵人即命中一次」不需要另設再命中間隔，
    命中頻率就是旋轉速度本身。無座標時（高塔）退化為每轉一圈打一次主目標。
- **火狩（firehunt）**：距自身 8 米環繞、體積 3×3 米、每秒 1 圈，召喚 2 團、每團 120% 火屬性傷害、持續 4 秒；
  強化火狩（體積與環繞範圍同步擴大）／伴生火狩（命中機率在後方伴生一團，每團只伴生一個、伴生體不再伴生）／
  三重火狩（改為 3 團、150%）／極速火狩（旋轉速度 +25%）／再生（每擊殺延長整組持續時間）／
  狩神之舞（改為 2 道反向火狩、外圈距內圈 6 米、每團出現時自帶伴生、200%、持續 6 秒）。
- **參數表**：`config/CSV/Skills2.csv`＋`config/Excel/Skills2.xlsx` 新增 7 列；群組 `range` 欄用來描述
  **環繞體的體積**（3*3），新增 `rps`（每秒圈數）與 `rings`（道數）兩個鍵，欄位定義頁同步補說明。
  往返驗證「語意變更 0」。
- **協議**：`js/worker/protocol.js` 的 `area` 補上環形形狀 `{ x, y, r, orbR, orbs, spin }`（無新欄位，
  shim 原本就整包轉送 `area`）。這組數字就是模擬層實際判定接觸的圓，顯示層不得再套第二組縮放。
- **特效**：`js/battle-renderer.js` 新增 `aura/firehunt` 畫法——圓心**逐幀取玩家座標**（火狩本來就跟著玩家跑，
  用施放當下的座標會整團留在原地）；同一道（半徑＋方向）只保留一個節點，讓【再生】延長持續時間時
  補送的事件延長既有環而不是愈疊愈多團。
- **待調數值**：第 4 階【三重火狩】設計表寫「每級+5%機率」但該階沒有機率參數，暫定為每級 +15% 火屬性傷害；
  設計表未定義的施法消耗（50）與施法距離（8 米）同樣已填入參數表，可直接在 Excel 調整。
- **測試與建置驗證**：新增 `tests/skill2-magic-firehunt.test.cjs`（14 項，涵蓋接觸判定、環繞半徑、各階效果、
  生命週期與高塔退化）；`node --test "tests/*.test.cjs"` 1,448 項全數通過；
  `node tools/build_check.cjs` 282 個檔案全數通過。
- **實機驗證（本機 8331 埠）**：GM `sglv firehunt max` 後實戰 30 秒，`skill2:firehunt` 累計 191 次命中／327,649 傷害；
  VFX 事件為兩道反向環（r=102.8 內圈、162.8 外圈，orbR=19.275），Canvas 畫法執行完整生命週期無 Console 錯誤。
  **未確認**：分頁未顯示無法截圖，環繞特效的實際外觀仍需目視確認。
...
- **設計來源**：記事錄「技能」頁籤新增〈魔法〉區塊兩個技能群組，以及新欄位「傷害範圍說明」
  （說明每一階傷害的作用範圍法算：射程、爆炸半徑、擴散半徑、矩形長寬）。
- **引擎擴充（三個共用能力，不是這兩個技能的特例）**：
  - `js/skills2.js` 群組層新增 `dmgType`／`elem`：魔法群組吃魔攻與魔穿，本體傷害整段歸屬
    火屬性（`sgAtkCfg` 寫入 `skillElem`，比照 `skillElemApplyACfg` 的技能屬性化規則）。
  - **施法距離**：各階 `fx.castM`（米）決定射程，取代「新版技能一律近戰起手」的寫死規則；
    判定收斂在 `skills2CastRangePx`／`skills2CanReach`，`js/skills.js` 的施放閘門改吃同一支。
    八個武技群組未定義 `castM`＝仍為普攻近戰距離，行為不變。
  - **地板場域** `SKILL2_RT.grounds`：釘在座標上、按節拍反覆作用的區域，可移動、可重生；
    無座標時（高塔）退化為固定打主目標。
- **火球術（fireball）**：射程 30 米、爆炸 6 米；燃燒／火球爆裂（20 米內分裂 3 顆）／
  強化燃燒（縮短作用間隔）／爆燃（我方 12 米內 2 敵，依整段燃燒累積傷害引爆）／
  火焰增幅（我方 20 米內每次燃燒作用疊火傷，無限疊加）／殞石術（3 顆、範圍 15 米、射程 20 米）。
- **火柱（firepillar）**：射程 30 米、範圍 3 米連續 5 段；強化火柱（範圍擴大）／
  雙重火柱（20 米內 2 個目標）／燃燒／追擊（6 米每秒）／重生（消失後機率在 20 米內重來）／
  無限火牆（3 道橫向 6×18 米火牆、每道 8 段，每道只再召喚 1 次）。
- **狀態表**：新增 `sgBurn`（烈焰燃燒，火屬性 DoT）與 `sgFireAmp`（火焰增幅）。
  兩棵樹塗的是同一個「燃燒」，因此爆燃與火焰增幅對火球術與火柱一視同仁。
- **屬性傷害提升掛點**：火焰增幅接在 `js/legendary.js legendaryElementDamageUp`——
  全專案「屬性傷害提升%」的唯一收斂點，故普攻元素附傷、舊技能與新技能一體生效。
- **參數表**：`config/CSV/Skills2.csv`＋`config/Excel/Skills2.xlsx` 新增「傷害類型」「傷害屬性」
  兩個群組欄位並重生（`--gen Skills2`）；`Status` 表同步重生。欄位定義頁補上
  `castM`／`len`／`wid`／`respawn` 的說明。往返驗證「語意變更 0」。
- **特效**：火球走既有 `projectile/fireball`＋`burst`、殞石走 `rain/meteor`、火柱走
  `impact/pillar`＋地面 footprint；新增 `aura/firewall` 由 `js/battle-renderer.js`
  依 `area{x,y,w,h,a}` 畫出方向正確的矩形（傷害範圍與畫面範圍共用同一組數字）。
- **測試與建置驗證**：新增 `tests/skill2-magic-fire.test.cjs`（18 項，涵蓋傷害類型、施法距離、
  各階範圍法算、地板場域推進與高塔退化）；執行 `node --test "tests/*.test.cjs"`
  （1,431 個單元測試全數通過）與 `node tools/build_check.cjs`（281 個檔案編譯全數通過）。
...
- **技能頁裝備欄格式統一**：
  - 技能頁裝備欄（`#skill-loadout`）重構為與戰鬥區技能快捷列完全一致的 10 格大圖示外框介面（包含已裝備技能、未裝備空槽位 `＋` 與未解鎖 `🔒` 槽位）。
  - 已裝備技能同步顯示 Emoji、等級角標、主動型被動 🌀 旋轉邊框與懸停說明。
- **點擊選取與小叉叉卸下機制**：
  - 點擊技能頁裝備欄中的技能不再直接卸下，而是改為選取狀態（高亮金黃色外框與發光陰影）。
  - 選取的技能圖示右上角出現紅底小叉叉按鈕（`×`），點擊小叉叉才會觸發卸下。
- **戰鬥區技能快捷列規範**：
  - 點擊戰鬥區「已裝備技能」無效果（不再卸下或跳轉）；點擊「未裝備空槽位 `＋`」時維繫跳轉至技能頁。
  - 戰鬥區「已裝備技能」同樣支援 drag-and-drop 拖曳置換位置。
- **技能雙向拖曳位置置換**：
...
## 戰鬥系統與界面優化：修復護盾技能自動施放條件、狀態圖示正方形圖標、圓形倒計時遮罩與 Tips 即時倒計時（Antigravity 2026-08-16）

- **修復護盾技能（魔法屏障）自動施放判定**：
  - 修復 `skillConditionOk` 判定：當護盾技能自身的限時主動 BUFF 已結束或破盾時（`!statusActive(pEnt, 'shield')`），即便身上有來自被動技能（如反擊盾）、吸血盾或過量治療的殘餘護盾值，AI 亦能立即自動重新施放，確保隨時補充 64% 最大生命厚盾與 22% 格擋增益。
  - 同時確保護盾技能在主動 BUFF 生效中且剩餘盾量低於 20% 時，仍可提前安全補盾。
- **狀態圖示正方形化與佈局對齊調整**：
  - 狀態圖示調整為正方形 icon（尺寸 28px × 28px，精準為技能槽 42px 的 2/3 長寬）。
  - 狀態列調整為靠左對齊（`justify-content: flex-start`，寬度 492px 左右 padding 8px 與技能槽第 1 格精準貼齊），並向上微調 5px（`bottom: 65px`），使圖示自左向右整齊排列。
  - 保留依狀態種類區分的精緻邊框與光暈（增益綠光、護盾亮天藍光、減益/控場紅光）。
- **圓形倒計時黑色遮罩**：
  - 狀態圖示持續時間倒數採用與技能快捷列完全相同的圓形黑色倒數遮罩（`conic-gradient` 扇形倒數）。
  - 結合 60fps 動態器即時平滑轉動遮罩，圖示內部不再顯示倒計時數字，維持乾淨清爽。
- **右下角疊層數字與 Tips 右上角即時倒計時**：
  - 當狀態具備多層疊加時（`stacks > 1`），直接於圖示右下角顯示疊層數字（`.bbb-stacks`）。
  - 狀態詳細效果與數值移入 Tips 提示框；倒計時間固定顯示 1 位小數（如 `6.0s`、`4.7s`，即便為整數或 0 亦完整保留一位小數）並置於 Tips 右上角（`.skt-remain`），於 Tips 開啟期間結合 60fps 動態器即時每幀高頻平滑刷新剩餘時間。
- **測試與建置驗證**：
  - 更新 `tests/skill-gcd.test.cjs` 與 `tests/active-buffs-panel.test.cjs` 驗證護盾技能施放判定、倒計時遮罩與 Tips 右上角即時倒計時。
  - `node --test "tests/*.test.cjs"`（1,401 個測試全數通過）與 `node tools/build_check.cjs` 驗證 100% 通過。



- **修復新版技能樹已解鎖節點高亮展示**：
  - 確保當前置技能滿級達標時，下一個解鎖可學習的階層節點（`sg-stage-unlocked`）圖標呈現完整彩色 Emoji（`filter: none`）與亮青藍邊框（`border-color: #74c9e8`），讓玩家能清晰辨識該技能已解鎖可升級。
  - 未解鎖節點（`sg-stage-locked`）維持深灰暗化鎖定狀態。
  - 頂部提示文字維持「亮起＝已解鎖，灰色＝未解鎖（可查看）」。
- **測試與建置驗證**：
  - `node --test "tests/*.test.cjs"`（1,400 個測試全數通過）與 `node tools/build_check.cjs` 驗證通過。

## UI 修復：修復新版技能樹未學習／降回 0 級時技能節點仍處於高亮亮起狀態的問題（Antigravity 2026-08-15）

- **修復新版技能階層節點視覺狀態判斷**：
  - 原本 `.sg-stage-unlocked` 在前置階層達標時會直接將整格技能亮起（彩色 Emoji、高亮青藍色邊框與背景），導致未投資點數或降為 Lv.0 的技能呈現亮起狀態。
  - 修正樣式分級：
    - **已學會（`Lv > 0` / `.sg-stage-learned`）**：呈現高亮彩色 Emoji、亮青藍邊框（`#74c9e8`）、背景色與發光光暈。
    - **已解鎖但未學習（`Lv.0` / `.sg-stage-unlocked` 且非 learned）**：維持灰色暗色圖標（`filter: grayscale(.9); opacity: .75;`），邊框呈現淡暗青藍（`border-color: rgba(116, 201, 232, .35)`），等級字體為暗色，可點擊升級但不亮起。
    - **未解鎖（`sg-stage-locked`）**：維持深灰暗化鎖定狀態。
  - 同步微調技能樹頂部提示文字為「亮起＝已學會，灰色＝未學／未解鎖」。
- **測試與建置驗證**：
  - `node --test "tests/*.test.cjs"`（1,398 個測試全數通過）與 `node tools/build_check.cjs` 驗證通過。

## 戰鬥界面改造：移除獨立 BUFF 面板按鈕，改於技能快捷列上方即時顯示 BUFF 狀態膠囊（Antigravity 2026-08-15）

- **移除獨立 BUFF 按鈕與彈出式面板**：
  - 移除戰鬥場景左上角之 `💪` BUFF 按鈕（`.buff-btn` / `#battle-buff-btn-slot`）以及展開的彈出式玩家資訊面板（`#player-info-popup`）。
  - 將 `#btn-enemy-tip`（敵情提示鈕）回歸左上角基準對齊。
- **技能快捷列上方即時 BUFF 狀態列（`#battle-buff-bar`）**：
  - 於戰鬥區底部技能快捷列（`#battle-skill-bar`）正上方居中新增動態 BUFF 狀態列容器。
  - 即時同步玩家當前生效中之所有狀態（包含護盾、屬性增益、持續回復、持續傷害、控場與無敵等），無狀態時自動隱藏不遮蔽視線。
  - 每個狀態以精緻半透明深色玻璃擬態膠囊（Badge）呈現：包含狀態 Emoji 圖示、狀態名稱/疊層、數值（如護盾吸收量、屬性百分比）與倒數秒數。
  - 依狀態類型賦予專屬邊框光暈色彩（增益為綠色、護盾為天藍色、減益/控場為紅色）。
  - 支援完整懸停/點擊 Tooltip 說明（包含狀態詳細描述、數值與剩餘時間秒數）。
  - 結合 60fps 動態器即時更新倒數秒數。
- **測試與建置驗證**：
  - 更新 `tests/active-buffs-panel.test.cjs` 驗證 `#battle-buff-bar` 狀態列與增益展示。
  - 全套 1,391 個單元測試與 `build_check.cjs` 100% 通過。

## 技能特效重做：連鎖閃電天雷彈射與電紋刻印巨型紫電/雷印法陣（Antigravity 2026-08-14）

- **連鎖閃電（Chain Lightning）連續橫向折射電鏈重構**：
  - **天頂大型天雷（Mega Thunder Strike）**：首個目標從天頂劈下震撼大型落雷，具備多段碎形擬真折線、三層筆刷（外層電漿光暈、中層金色雷光、內層高亮白芯）與分叉副電弧。
  - **連續橫向穿梭折射閃電鏈（Continuous Zig-Zag Chain Polyline）**：命中首個目標後，立刻拉出一條帶有銳利折角、發著厚厚白金外暈與高亮白芯的連貫橫向折射電鏈（`spawnContinuousChainLightning`），在各個敵人間連續穿梭折射貫穿。
  - **目標節點命中光圈（Node Target Rings）**：在每個被折射貫穿的敵人身上生成同心發光標靶環（`spawnNodeRing`）與雷暴衝擊火花，呈現極致雷電穿梭手感。
  - **場景動態延伸**：若場上敵人少於 3 隻，電鏈自動向周遭偏移放電點連續折射，確保任何情況下皆能清晰呈現橫向連續穿梭電網。
...
- **優化 `#quest-bar` 任務快捷列顯示**：
  - 任務達成時已具備綠色指示燈（`.quest-dot`）與發光效果，移除快捷列尾部的「可領取！」文字。
  - 縮短任務條寬度，徹底避免任務長度過長時壓到右上角「暫停」與控制按鈕。
...
- **實機與邏輯 15 項驗證 (15/15 PASS)**：
  ...
  - **B. 野外實戰與邊界**：6 個技能群組（突刺/迴旋斬/飛刀/疾風斬/血刃斬/雙刀亂舞）招式與特效、冷卻條、DoT (流血/血毒刃/零日感染)、暈眩 (Boss 免疫判定)、狂風斬攻速倍率、鐵血之舞生命扣減均符合預期；**暴風之舞（第 7 階）普攻計時器於化身期間夾回 0，徹底解決化身結束後多刀欠帳爆發連發問題，暈眩中亦正確暫停自動施放且不補發**；單目標邊界多段判定平順無報錯。
  - **C. 高塔與相容性**：高塔戰單體退化施放無誤且傷害計入統計；存檔重載 `sg:` 狀態完整保留；卸下 `sg:` 技能後舊技能系統（`SKILLS`）施放與數值無干擾（兩者並行零互擾）；全程 Console 無新增 Error/Warning。
- **自動化測試與 DoD 構建**：
  - `tests/skill2-system.test.cjs` 18/18、`tests/skill2-review-fixes.test.cjs` 3/3、`tests/worker-protocol.test.cjs` 8/8 PASS（合計 unit tests 29/29 PASS）。
  - `node tools/build_check.cjs` 270 個檔案語法/編譯檢查 100% OK 全數通過。
...
- **真的舊存檔裁切與狀態保留**：
  - 當讀入裝載欄 >10 格（如 15 格）且技能等級 >10 的舊存檔（無 `loadoutCapClampV1` 旗標）時，裝載欄正確修剪保留前 10 格（5 轉現行上限），並自動產生公告訊息通知（如「技能裝載欄上限調整為 10 格：已卸下超出的 5 個技能」）。
  - 技能等級與已學狀態 100% 保持完好，不會影響玩家已練技能。
- **重複讀檔冪等性與無干擾**：
  - 首次讀檔完成後自動注入 `loadoutCapClampV1: true` 旗標。
  - 第二次讀檔（或後續讀檔）時，直接識別該旗標，不會重複觸發裁切，亦不會再次彈出公告。
- **轉生確認窗 10 轉後技能上限顯示**：
  - 轉生 confirmation dialog 的技能上限差值計算 `skillAdd = Math.max(0, nextSkillMax - curSkillMax)` 於 10 轉至 20 轉期間查表皆為 30，差值精確計算為 0，正確呈報 +0（技能上限已封頂，不顯示多餘上限增加字串）。
- **存檔持久化落盤寫入**：
  - 存檔經 `saveGame()` 保存或寫入 `localStorage` / JSON 快照時，`loadoutCapClampV1: true` 旗標與裁切後的裝載欄位狀態均完美持久化寫入，再次重新從 `localStorage` 或存檔資料夾加載時皆完全符合期望。
- **測試**：新增自動化單元/整合驗證測試 `tests/save-migration-persistence-verification.test.cjs` (4/4 PASS)，全專案 1238 項單元測試全數通過；`node tools/build_check.cjs` 構建檢查通過。

## 驗證：各具名地圖與神界菁英出怪數、荒漠早期難度與棋盤夾限（Antigravity 2026-08-10）

- **四張具名地圖與神界三圖菁英關出怪數**：
  - 荒漠 `desert`：出怪數落在權重表 `[[1, 95], [2, 95], [3, 5]]` 範圍內（1~3 隻）。
  - 冰原 `Icefield`：出怪數落在權重表 `[[1, 75], [2, 75], [3, 20], [4, 5]]` 範圍內（1~4 隻）。
  - 沼澤 `swamp`：出怪數落在權重表 `[[1, 50], [2, 50], [3, 10], [4, 10], [5, 6], [6, 2]]` 範圍內（1~6 隻）。
  - 亡靈山脈 `undead_mountains`：出怪數落在權重表 `[[1, 35], [2, 35], [3, 20], [4, 20], [5, 10], [6, 5], [7, 2]]` 範圍內（1~7 隻）。
  - 神界三圖 (`god_realm_1` / `god_realm_2` / `god_realm_3`)：沿用「500關之後」權重表 `FIELD_ELITE_COUNT_TABLE`（`[[1, 25], [2, 25], [3, 25], [4, 25], [5, 15], [6, 10], [7, 5], [8, 1]]`），出怪數落在 1~8 隻（上限 8 隻）。
- **荒漠 1~100 關菁英行為變更與早期難度可接受性**：
  - 移除荒漠 1~100 關菁英固定 1 隻的寫死規則，統一採用荒漠權重表（1~3 隻）。
  - 難度驗證：荒漠第 10, 20, 30... 關的菁英關卡中，1 隻 (48.7%)、2 隻 (48.7%)、3 隻 (2.6%)。第 10 關普通怪底血約 20 HP，菁英怪倍率 4 (約 80 HP)，2 隻合為 160 HP，玩家初始四維基礎力敏智耐 5 (HP 200+，防禦減傷後承受傷害極小)，能在 2~3 秒內平順擊殺，無秒殺死鎖現象，早期難度轉折完全可接受。
- **小怪分段與 BOSS 出怪數量**：
  - 小怪：荒漠 1~100 關每 20 關正確套用對應分段表（1~20 關 `1~3` 隻、21~40 關 `1~3` 隻、41~60 關 `1~4` 隻、61~80 關 `1~6` 隻、81~100 關 `1~8` 隻）；101 關後恢復 `FIELD_ENEMY_COUNT_TABLE`（`1~16` 隻）。
  - BOSS：未通關關卡之 BOSS 階（50 的倍數）固定出怪 1 隻（`FIELD_BOSS_COUNT_TABLE` = `[[1, 100]]`）。
- **棋盤放不下時的夾限**：
  - 棋盤為 4x4 共 16 格 (`bfCellCount()`)，`rollFieldEnemyCount('elite')` 透過 `Math.min(count, bfCellCount())` 約束夾限，即便權重表最高抽到 8 隻或棋盤可用格數受限，出怪數絕不出超過剩餘棋盤格子數。
- **測試**：新增專屬驗證測試 `tests/zone-elite-count-verification.test.cjs` (4/4 PASS)，更新 `tests/multi-enemy.test.cjs`、`tests/skill-fusion-rework.test.cjs` 與 `tests/skill-loadout.test.cjs` 斷言。
- **Build 檢查**：`node tools/build_check.cjs` 256 個檔案全數 OK 零錯誤。
...
## UI 功能：依據 NPC 出現權重計算場景敵人屬性佔比（2026-08-07）

- **依據 NPC 表權重計算**：同步 `config/CSV/NPC.csv` 的 `出現權重` 至 `js/data.js` 之各 Pool 怪物定義、`registerNpcPool` 與 `ZONE_ENEMY_TABLES`，使 `zoneElementTagsList` 精確依據 NPC 出現權重（如荒漠: 地15/火14/暗1、沼澤: 毒15/暗14/地1）計算各場景屬性佔比。
- **佔比顯示**：場景頁籤 Tips 的主要敵人屬性精確顯示各屬性實際出現機率（例如荒漠 `🪨 地屬性 (50%) 🔥 火屬性 (46.7%) 🟣 暗屬性 (3.3%)`）。
- **測試**：更新 `tests/zone-attr-tooltip.test.cjs` 斷言，1188 項測試與 `build_check` 252 檔案檢查全數通過。

## UI 樣式：移除所有按鈕與停用狀態之禁止符號鼠標（2026-08-07）

- **全面移除禁止符號 (`cursor: not-allowed`)**：全專案（`css/style.css`、`js/ui.js`、`ui_head.js`）全面移除 `not-allowed` 鼠標樣式。按鈕於點擊在途（Pending）或停用狀態時，鼠標保持預設箭頭樣式 (`cursor: default`)，不再短暫出現禁止符號圖案。
- **鼠標行為規範**：僅在遇到可點擊物件時顯示手指圖案 (`cursor: pointer`)，其餘停用或不可點擊狀態一律使用預設箭頭樣式。

## 統計面板：傷害數字簡寫與傷害佔比排序（2026-08-07）

- **傷害排行**：統計面板（`js/combat.js` 之 `generateSummaryHtml`）改按技能傷害高至低（`damage` 降序）排序，傷害佔比最高之技能顯示於最上方。
- **傷害簡寫**：傷害數字由完整數字改為統一調用大數字簡寫函式 `fmt(sk.damage)`（例如 `305.04B`、`13.37T`），提升易讀性。
- **測試**：更新 `tests/summary.test.cjs`，新增傷害排序與簡寫格式之單元測試驗證，並通過 `build_check.cjs`。
...
- **曲線性質改變**：飽和曲線趨近但到不了 100%，敵方防禦永遠保留一小部分——上一版的「封頂 100%」夾限因此成為僅防呆用（a 設 0／負值時），正常參數下不會觸發。低穿透段大幅變強（100% 穿透：22.9% → 40%），高穿透段變弱（1000%：100% → 87%），邊際收益持續遞減但不再歸零。
- **無存檔影響**：穿透值本身仍不設上限（`STAT_CAPS.pPen/mPen = 0`），詞條、技能 `penUp` 增益與既有存檔數值皆不變。
- 面板：穿透 tips 改列新公式與 350% → 70% 的範例，移除「已達上限」標示（新曲線不會封頂）。
- 參數表：「3-戰鬥核心／穿透忽略防禦」列公式／說明改寫，a=1.5、b=0、c=0（`apply_params` 一致 547／將變更 0）；**xlsx 由使用者自行同步**。
- `game_formula.md` §2 屬性表、§3.1（含曲線對照表改為 100%/250%/350%/500%/1000%/1500%/3000%）、§3.2 同步更新。
- 驗證：`tests/attr-skill-rework-2026-07-30.test.cjs` 三項穿透測試改寫（公式恆等、350% → 70% 基準點、單調遞增且恆 < 100%、resolveHit 傷害與公式逐項對照）。

## 調整：取消穿透溢出增傷（2026-08-06）

- **忽略防禦封頂 100%**：`penIgnoreRatio`（js/formula.js §3）回傳值改為 `min(1, a×(穿透%÷100×b)^c)`，物理穿透與魔法穿透皆適用。曲線與 a/b/c 參數不變（0.01／100／0.68），僅在換算結果超過 100% 時夾回——約 873% 穿透即達封頂，敵方防禦歸零，**超出的穿透不再有任何效果**。
- **移除溢出增傷乘區**：刪除 `penOverflowDmgMultiplier`，`resolveHit` 三處呼叫點（物理段、魔法段、玩家承受元素攻擊的魔法減免段）一併移除；`penDefMultiplier` 因上游已夾限，內部多餘的 `Math.min(1, ...)` 一併簡化。ROI 評估器 `scripts/sim/evaluator.js` 的物理／魔法兩段同步移除，維持與 `resolveHit` 逐段對齊。
- **無存檔影響**：穿透值本身仍不設上限（`STAT_CAPS.pPen/mPen = 0`），詞條、技能 `penUp` 增益與既有存檔數值皆不變，只有結算時的換算上限改變，無須遷移。
- 面板：穿透 tips 不再顯示溢出增傷倍率，改為在達 100% 時標示「已達上限，敵方防禦歸零」。
- 參數表：「3-戰鬥核心／穿透忽略防禦」列的公式與說明改為封頂版本（a/b/c 數值不變，`apply_params` 一致 549／將變更 0）；**xlsx 尚未同步，需使用者於 Excel 更新同兩格文字**。
- `game_formula.md` §2 屬性表、§3.1、§3.2 同步更新（曲線對照表 1000%／1500% 兩格改為「100%（封頂）」）。
- 驗證：`tests/attr-skill-rework-2026-07-30.test.cjs` 兩項穿透測試改寫為封頂語意（含「`penOverflowDmgMultiplier` 已移除」與「5000% 穿透傷害 = 1000% 穿透傷害」斷言）。
...
- `js/main.js`：新增背景休眠——隱藏逾 60 秒（`BG_SUSPEND_AFTER_MS`）後 `gameTick` 休眠（GT 不前進）；回前景依隱藏時長觸發既有 `applyOfflineProgress()` 離線結算（上限 24 小時），結算前後 `saveGame()` 鎖定 `savedAt` 防重複入帳；開機結算後亦即刻存檔。迷你監控視窗（PiP）開啟時不休眠。注意：回頁判定不可用 `document.hidden`（事件觸發時已為 false），改以 `_hiddenAt` 時長判定（瀏覽器實測抓出）。
- `js/ui.js`：`floatText`／`renderStatsPanel` 在背景直接跳過；`js/main.js` `checkForUpdates`、`js/param_autoreload.js` 輪詢在背景跳過。
- `game_formula.md` §11.1 新增背景分頁休眠規則說明。
- 驗證：`tests/background-suspend.test.cjs` 4/4（含 vm 功能測試）、建置 139 檔零錯誤、隔離埠 8124 實測：隱藏逾時 GT 凍結、回頁觸發結算且無大量補進、主控台 0 錯誤。（既有失敗 `ui-performance` 場景列簽章測試於 HEAD 即失敗，與本次無關。）

## 新增：附檔傳奇特效完整實作（2026-07-24）

- 解析《神力之巔_記事錄.xlsx》「傳奇特效」工作表：原始 32 列中「劇毒血霧」重複 1 列，去重後新增 31 個傳奇特效；既有 7 個效果保留，`PASSIVE_POOL` 合計 38 個。
- `Equipment_Affix` 分類由「特殊被動」更名為「傳奇特效」，新增「效果類型／關聯技能／觸發技能／限定武器類型／效果參數JSON」欄；重生後共 124 筆資料，CSV／XLSX 與 JS round-trip 語意變更 0。
- 效果說明中的括號裝備類別已轉為 `weaponTypes` 精確限制；掉落與合成大成功補效果時都只會從相符武器類型池抽取，非武器仍可取得無類型限制的既有傳奇特效。
- 新增 `js/legendary.js` 集中處理 31 個固定效果：包含關聯技能傷害／冷卻／消耗／元素型態改寫、未解鎖技能以至少 Lv.1 免費觸發、控場與最終增傷、DoT、護盾、反擊、飛刀／鬼娃召喚、定時技能與領域效果；野外與高塔共用同一執行期路由。
- `game_formula.md` §6.5 補齊 31 個效果、武器限制及觸發規則；玩法指南與參數表說明同步使用「傳奇特效」。
- 驗證：傳奇專項測試 8/8、建置 137 檔零錯誤、`Equipment_Affix` 公式錯誤掃描 0、隔離埠 Playwright 新存檔連續戰鬥無 console error。

## 調整：技能未解鎖提醒改用紅色字（2026-07-24）

- `js/ui.js`：技能升級視窗與 Tooltip 的人物等級解鎖提示加上 `skill-unlock-hint` 樣式類別。
- `css/style.css`：未解鎖提醒使用淡紅色粗體，清楚區隔一般說明文字且維持深色背景可讀性。
- `tests/skill-unlock.test.cjs`：補上紅色提醒樣式與 UI 接線檢查。

## 調整：GM 降低人物等級時重新判定技能解鎖（2026-07-24）

- `js/gm.js`：`level`／`lv` 指令若把人物等級調低，呼叫技能解鎖重判定；重新鎖定未學且高於新等級門檻的技能。
- `js/skills.js`：新增 `recheckSkillUnlocksForGMLevelChange()`；已學會技能保留可用，未學技能依新人物等級重新建立或移除解鎖紀錄。
- 正常升級、轉生與一般存檔載入仍維持「達標後永久解鎖」規則，只有 GM 降低等級觸發例外。
- `tests/skill-unlock.test.cjs`：新增 GM 降級重判定測試；GM 接線與既有 GM 測試均通過。

## 修正：套用參數仍引用已移除的技能樹門檻（2026-07-24）

- 根因：技能前置投入點數已取消並移除 `TIER_GATE_POINTS`，但 `tools/apply_params.cjs` 和主參數表仍保留舊錨點／舊列，導致「錨點匹配 0 次」而中止寫入。
- 修正：移除 `TIER_GATE_POINTS` 錨點，從 `config/CSV/game_parameters.csv` 與 `config/Excel/game_parameters.xlsx` 移除「技能樹門檻」列。
- 驗證：xlsx→CSV 323 列；`apply_params` dry-run 顯示 496/496 一致、將變更 0、錨點問題 0；新增回歸斷言防止舊錨點復發。

## 新增：技能人物等級解鎖與永久解鎖狀態（2026-07-24）

- `Skills` 表新增「解鎖等級」欄位；初始測試值為 `powerSlash`／`arcaneBurst`＝1、其他一般技能＝5、潛力技能＝0，之後可直接在 CSV/XLSX 調整。
- `js/skills.js`：新增人物等級解鎖判定；角色達到欄位門檻時寫入 `G.player.skillUnlocks`，既有已學技能也會自動補記，確保解鎖永久保留。
- `js/player.js`：新存檔初始化 `skillUnlocks`；舊存檔缺欄位時由技能判定函式惰性補建。
- `js/ui.js`：未解鎖技能格顯示小鎖頭；升級視窗與正式 Tooltip 會顯示「需人物達到 Lv.N 才解鎖」。潛力技能仍沿用原本的潛力節點解鎖規則。
- 「變量定義」頁補上欄位格式、永久解鎖規則與 `0`／空白語意；新增 `tests/skill-unlock.test.cjs` 覆蓋人物等級、永久狀態、舊存檔相容與 Tooltip 接線。

## 新增：45 新技能 × 11 機制族＋屬性標籤系統（2026-07-23~24）

- 規模：`SKILLS` 54→**99**（物理 21／魔法 21／防禦 19／特殊 19／被動 19，各系 9 個新技接在各系尾端、無前置點數限制、零存檔遷移）；`UNLOCKS` 同步 45 筆 M4/M8 里程碑；含潛力 10 個，`Skills` 表合計 64→**109** 列。全部技能以特殊機制為賣點、禁止堆屬性；唯一權威 spec＝PLAN.md「45 新技能 × 11 機制族」章。
- 11 機制族（fx 鍵驅動，公式詳見 game_formula.md §9.5）：skillAmp 技能增幅（含 perCdSec 依冷卻計幅、comboWindow 連段窗）、brand 印記儲能/引爆（儲能端固定不隨級）、procCast 機率引動（recast/freeAttack/castRandom/replayBest 重播最強技）、cdFlow 冷卻操作（cdShift/cdResetOnKill/cdOnHitTaken）、echo 回響與快照窗（echo/dmgWindow/healEcho）、resourceConvert 資源轉換（mpDump/shieldBurst/hpSacrifice/overhealDmg）、stackCharge 疊層引爆、dotSynergy DoT 互動（引爆/脈衝/加速/dotAmpPer/requiresTargetDot）、freeCast 免費施放、buffExtend 時光延續、periodicField 領域；另 9 個被動觸發鍵經 `PASSIVE_TRIGGER_KEYS` 白名單路由。
- 基建：`fxVal(v,lv)` 取值工具（純量或 {base,per}）；`computeStats` 被動觸發路由（`effectiveFx` 聚合至 `st.skillTriggers`，被動里程碑生效）；`doPlayerAttack` 可選 opts（forceCrit/free）；applyBuff/applyDot 補存原始持續 `dur`/累計延長 `ext`；`SKILL_RT` 模組級執行期狀態（js/skills.js 頂部，**不掛 G＝不入存檔**，`resetSkillRT()` 於戰鬥/關卡切換與讀檔重建）；`tickSkillSchedulers` 排程器由 combat.js fieldTick 與 **tower.js towerTick 鏡射**呼叫；敵側印記掛敵人實體（純 JSON、until 自然過期）。
- 防失衡硬上限（js/data.js 具名常數，引擎強制封頂）：DoT 引爆 ≤100%（`DOT_DETONATE_CAP_PCT`）、溢療轉傷 ≤90%（`OVERHEAL_DMG_CAP_PCT`）、護盾引爆 ≤魔攻×10（`SHIELD_BURST_ATK_MULT_CAP`）、buffExtend 累計延長 ≤原始持續 100%（`BUFF_EXTEND_CAP_PCT`；剩餘 <2 秒加倍門檻 `BUFF_EXTEND_LOW_REMAIN_SEC`）、引動遞迴深度 ≤1（`SKILL_PROC_DEPTH_MAX`）；擊殺/受擊類觸發皆設內部冷卻 icdSec（時戳存 SKILL_RT.icd）。
- 屬性標籤系統（PLAN.md §3.5）：`SKILLS.tags` 陣列＋Skills 表「標籤」欄雙向（**JS 為 SSOT**）；凡造成傷害的魔法技能（dmgType magic/true 或 stat matk，不限系統分類）必有元素標籤，奧術/法力/星辰/時空/命運/萬象歸光系 light、虛空/暗影/詛咒/汲取/血魔法歸 dark；16 個無 fx.elem 的直擊魔法技補 `elem:{type,portion:0.4}`（true 類與純 DoT 僅標籤）；新增 `skillElemOf(sk,fx)` 系別判定統一入口（tags[0] 優先→fx.elem.type→fx.elems 最大占比），雷霆過載雷電系增幅、連鎖閃電、skillAmp `'elem:'` scope 皆改走此 helper；技能說明改六元素風味語句＋徽章元素 emoji＋「X系」。
- 表格重生：`config_tables --gen` 重生四表（Skills **109** 列含標籤欄、Gems 37、Talents 80、Equipment_Affix 93）；`--apply` dry-run **語意變更 0**（tags round-trip 一致）；config_tables 的初版標籤種子 fallback 移除、改以 JS tags 為唯一準據。
- 文件：game_formula.md §9 規模註記、**§9.5 技能機制族**（11 族公式＋硬上限表＋被動觸發路由＋SKILL_RT 不入存檔說明）、**§9.6 屬性標籤**新章；§9.1/§9.4/§10 詮釋備註補 `skillElemOf` 交叉引用與 `requiresTargetDot` castAI 條件。
- 驗證：`npm run build` 133 檔零錯誤；skill-tags／skill-prerequisite／skill-tree-layout 等技能測試全過；Node vm 冒煙（skillElemOf 六情境／scope 比對／風味語句）17 斷言全過；僅 combo-hits 3 個為既有基線失敗（HEAD 亦失敗，與本次無關）。

## 技能標題新增類別與元素標籤（2026-07-24）

- `js/ui.js`：新增共用 `skillTagsHTML()`；技能類別沿用 `SKILL_CATS`，元素標籤改讀 `Skills` 表的 `tags` 欄位，接入技能升級視窗與正式技能 Tooltip。
- `css/style.css`：新增技能標籤樣式；元素標籤依火／冰／雷／毒／聖／暗套用淡色半透明底色，維持文字可讀性。
- 標籤文字不使用 `【】` 符號。
- `tools/config_tables.cjs`：Skills 表新增 `標籤` 欄位與「變量定義」說明頁，支援 `--apply Skills --write` 單表寫回；初版將奧術技能歸入聖系、劇毒／疫病／蝕骨技能歸入毒系。
- `tests/skill-tags.test.cjs`：新增魔法／聖系、無元素技能及元素色彩類別的回歸測試。

## 取消技能前置投入點數限制（2026-07-24）

- `js/skills.js`：`tierLockReason()` 不再依技能階層或同系已投入點數鎖定技能；所有技能仍保留技能點與金幣消耗。
- `js/ui.js`：移除技能樹「需投入 N 點」門檻文案；`js/formula.js` 移除 `TIER_GATE_POINTS`。
- `game_formula.md` 與 `PLAN.md` 同步規則；新增 `tests/skill-prerequisite.test.cjs` 回歸測試。
- 驗證：技能限制測試通過；`npm.cmd run build` 通過（132 個檔案）。完整測試套件仍有既有基線失敗，與本次變更無關。

## 新增：六種屬性傷害提升效果＋「光」屬性改稱「聖」屬性（2026-07-23）

- 機制：新增火/冰/雷/毒/聖/暗**屬性傷害提升%**六種效果——提升「自身」造成的該屬性元素傷害（固定值元素攻擊＋元素附傷%成分＋技能元素成分 ×(1+%)，於元素抗性結算前套用、元素特效衍生值一併放大；與既有「對X屬性傷害%」＝打帶該屬性標籤敵人的加成為**不同效果**）。
- 程式：`js/formula.js` 新增 `affixElemDmgUp`（`elemDmgFire`…鍵 → 元素）、computeStats `elemDmgUp` 聚合桶（詞條/寶石/天賦管線已通）＋`st.elemDmgUp`、resolveHit 元素附加段乘區（僅玩家攻擊端 aCfg 傳入）；combat.js／skills.js／potential.js 三處攻擊配置接線。**目前尚無取得來源＝預設 0**，供武器專屬能力等後續來源掛入。
- 顯示：屬性面板攻擊組新增六列「X屬性傷害提升」。
- 光→聖改名（識別鍵 light/dmgVsLight/resLight 不變，僅顯示文字）：詞條「對光屬性傷害%→對聖屬性傷害%」、黃水晶 statName 同步、聖光抗性附魔敘述「聖光屬性→聖屬性」、面板「對光屬性傷害→對聖屬性傷害」；`ELEM_INFO` 新增 `short`（火/冰/雷/毒/聖/暗）供「X屬性」語境——技能「X屬性佔 N%」、敵人情報「屬性：X屬性／受對X屬性傷害加成」、高塔 BOSS 屬性標籤改用 short。
- 配置表：Equipment_Affix／Gems 名稱變更經 `config_tables --gen` 重生四表（重生前後 --apply dry 均 0 語意差異）。
- 文件：game_formula.md 屬性表新增「屬性傷害提升」列、§3.2 步驟 5 公式補乘區、§4.1/§6 光→聖字樣。
- 驗證：建置 130 檔通過；隔離埠 8124——resolveHit 確定性驗證（固定元素攻擊 100、提升 100% → 元素段 ×2）、面板六列與聖屬性字樣、主控台 0 錯誤。
- 追加（同日）：**「對X屬性傷害」全面改名「對X屬性敵人傷害」**（六屬性；識別鍵 `dmgVsX` 不變，僅顯示文字）——詞條名、寶石效果名（尖晶石等六種 statName）、屬性面板六列、敵人情報 tips「受對X屬性敵人傷害加成影響」、game_formula.md 全部相關段落；`config_tables --gen` 重生 Equipment_Affix／Gems 表（--apply dry 0 語意差異）。與新增的「X屬性傷害提升」（自身元素輸出）名稱自此明確區分。
- 追加（同日）：**掛上取得來源**——`AFFIX_POOL` 新增六條「X屬性傷害提升%」詞條（`elemDmgFire`…；base=2、lv=0.01、權重 3、minR 3 獨特起、部位 武器/戒指/項鍊）；`GEM_TYPES` 新增六種「X核寶石」（`coreFire`…焰核🌋/冰核🌨️/雷核🌩️/毒核🦠/聖核🌟/暗核🌚；base=0.2 linear，曲線同對屬性傷害寶石，10 階 32%），掉落/商店/合成/轉換/拆解/神鑄走既有寶石通用管線。`config_tables --gen` 重生四表（Equipment_Affix 93 列、Gems 37 列），--apply dry 0 語意差異；數值可在表上自行調整。game_formula.md §2 屬性表／§6 詞條／§8 寶石段落同步。

## 改造：武器系統＝主/副手部位＋12 種武器類型架構（2026-07-23）

- 機制：裝備欄位語意改為「主手（weapon）／副手（weapon2）」；武器分三大類——**單手武器**、**雙手武器**（裝在主手並同時佔據副手，副手不可再裝）、**副手武器**（僅副手）。12 種類型（`WEAPON_TYPES` → js/data.js）：單手匕首（**唯一可雙持**，主副手皆可、一次兩把）、單手劍/單手魔杖/單手魔劍（僅主手）、雙手大劍/雙手斧/雙手法杖/雙手魔劍（佔雙欄）、盾牌/法器/魔法書/水晶球（僅副手）。
- 產出：`makeEquipment` 武器均等隨機擲類型（`opts.weaponType` 可指定）、名稱改用類型專屬名稱池；`weaponAbility:null` 預留欄位＋`WEAPON_ABILITY_POOL` 空能力池（**武器專屬特殊能力僅預留架構，待使用者設計**）。**詞條暫沿用現行 weapon 部位詞條池、所有類型相同**（分類型詞條待後續改造）。
- 裝備規則（js/player.js）：`equipTargetSlot`／`tryAutoEquip` 改用實例層 `equipSlotsForItem`；`equipItem` 裝雙手武器時副手自動退回背包、主手為雙手時改裝副手則雙手退回背包（退回走背包滿載既有規則）；自動穿裝不填被雙手武器佔用的副手欄、雙手武器需雙欄皆空。
- UI（js/ui.js＋css/style.css）：副手欄被雙手武器佔用顯示 ⛓️ 標記（twohand-occupied）；物品格新增 `data-eqslots` 供選取比對（依實例可裝欄位）；詳情頁顯示「類型（大類）」與類型 emoji。
- 存檔：載入整理 pass（save.js，比照太古殘欄清理、永久正規化）為所有容器內武器補 `weaponType`（預設 `sword1h`；**副手欄舊武器補 `dagger1h`** 保持位置合法、不強制卸下）與 `weaponAbility:null`。
- 追加（同日）：使用者更正——**單手劍僅限主手**，可雙持（一次裝兩把）的只有單手匕首；`WEAPON_TYPES.sword1h.slots` 改 `['weapon']`，舊存檔補類型改為上述「副手欄補匕首」規則，GM_command.md／game_formula.md §6.1.1 同步。
- GM：equip 指令部位參數支援直接填武器類型鍵（如 `shield`／`staff2h`）；`weapon2`／`ring2` 欄位鍵折算回物品種類。GM_command.md 同步。
- 文件：game_formula.md §6.1.1 新增武器類型章節、自動穿裝段落補主副手規則；PLAN.md 登錄計畫。
- 驗證：建置 130 檔通過；隔離埠 8124 實機——200 件隨機武器 12 類型全出現、weaponAbility 皆 null、名稱皆出自類型名稱池；可裝欄位（單手劍=雙欄/魔杖=主手/法杖=主手/盾=副手）、雙持、雙手佔雙欄與互擠退回、自動穿裝防呆、舊格式補 sword1h、副手佔用標記 DOM、data-eqslots 全數通過；主控台 0 錯誤、主迴圈正常。

## 新增：裝備詞條「魔法防禦%」（2026-07-23）

- 背景：詞條池只有「物理防禦%」沒有「魔法防禦%」（歷史遺留不對稱），使用者定調補齊、數值比照物理防禦%。
- `js/data.js`：`AFFIX_POOL` 新增 `mdefPct`（base 4、每級 0.02、pct、權重 6，緊接 mdefFlat 之後）；`AFFIX_CATS.def` 補 `'mdefPct'`（防禦系配色）；`SCORE_WEIGHTS` 補 `mdefPct: 1.8`。屬性引擎本就支援（computeStats 的 mdefPct 桶與魔防公式乘區），無公式改動。
- `Equipment_Affix` 表（xlsx+CSV）：經 `config_tables --gen` 重生入表；同時把「出現部位」欄的多行表頭註解固化進 `tools/config_tables.cjs` 的 schema header（先前 --gen 會洗掉手寫註解），數值格式順帶正規化（0.55000000000000004→0.55 等，語意不變）。
- 驗證：config_tables --sync/--apply round-trip 語意 0 差異；全套測試與基線一致 0 新增失敗；建置 130 檔通過；實機抽樣 2000 件神話裝 11.7% 出現魔法防禦%，+20% 詞條使魔防 ×1.202、物防不變，主控台 0 錯誤。
- 文件：game_formula.md §2 魔防%來源說明更新。

## 改造：太古詞條機制＝產出時定終身（2026-07-23）

- 機制：太古詞條不再由「洗煉＋太古精華」洗出，改為**裝備產出時**依「詞條數量→太古條數」權重表（`ANCIENT_COUNT_WEIGHTS`，2~10 詞條各一列）擲骰條數、隨機挑位置後**永久固定**；不限等級/稀有度/掉落來源（野外、高塔、離線、神鑄、合成、GM 皆同一規則）。
- 洗煉：非太古位置規則不變但**永不洗出太古**；太古位置洗煉**必為滿值**（上限×1.35）、只變換詞條種類（整件與單詞條皆同）。洗煉不再消耗太古精華；「使用太古精華」開關整組移除（index.html 控件、ui.js sync/listener、item.js 費用顯示與判定）。
- 程式：`js/formula.js` 新增 `rollAncientAffixCount`、移除 `ancientAffixChanceForEnemy`／`ancientBossAffixChanceForBoss`／`rerollAncientEssenceCostFor`；`js/item.js` `rollAffixes` 末參改太古位置集合＋`pickAncientPositions`，洗煉沿用原位置；combat/save/tower/forge 拆 `ancientRate`；`js/data.js` 移除舊機制常數（ANCIENT_EQUIP_MIN_*／ANCIENT_AFFIX_*RATE*／ANCIENT_BOSS_AFFIX_*／ANCIENT_REROLL_CHANCE／REROLL_ANCIENT_ESSENCE_COST）。太古精華掉落/拆解與數值倍率 ×1.35 全部保留。
- 存檔：載入時清除殘留的 `it.useAncientEssence` 欄位（save.js，永久正規化，非一次性遷移）；既有裝備的太古位置以現況為準永久固定。
- 參數表（CSV+xlsx 同步、重編號、round-trip 驗證一致）：刪 4 列（野外太古詞條機率、高塔太古詞條機率、太古精華洗煉、太古精華洗煉消耗）、於「5-太古詞條」新增 9 列產生率（詞條數量 2~10）；apply_params 對應拆線＋新增逐列重建錨點（492 參數全一致、0 錨點問題）。
- 文件：game_formula.md §4.3／§6.3（產生率表全表）／§7.2 洗煉改寫。
- 驗證：太古 Node 測試改寫 12/12 通過；全套測試與 HEAD 基線失敗集合完全一致（44 個既有失敗皆與本次無關、0 新增）；建置 127 檔通過。
- 註：太古精華目前**只剩取得、暫無消耗用途**（掉落與拆解照舊），使用者表示新用途再想想、先保留。
- 追加（同日）：依「game_parameters ＞ 程式寫死值 ＞ 文件」優先級修正太古精華掉落敘述——game_formula.md §5 野外材料表與 §11 離線收益改為「49 級起 `min(10%, 1% + (敵人等級-49)×0.1%)`」（原誤寫 250+/20%/0.2%）；參數表「5-野外材料／太古精華」中文說明同步改為以參數字母敘述（CSV+xlsx，round-trip 驗證一致、apply_params 492 參數 0 錨點問題）。

## 重做：雷霆過載＝電擊攻擊技能（非純 buff）（2026-07-22）

- 使用者定調：雷霆過載本身是電擊攻擊技能，同時附帶雷系傷害增益——「造成 XXXXX% 的魔法傷害，彈跳 N 次，電屬性佔 100%」。
- 本體雷擊（js/potential.js firePotentialLightning）：造成 `(atkBase＋atkPer×Lv)% 魔攻`（預設 1000＋200×Lv）的魔法傷害，電屬性佔 100%，於存活敵人間彈跳 `bounces`（預設 5）次、每跳完整傷害段；結算與一般技能同規格（爆擊/破甲/真傷/裝備固定元素攻擊/混沌雙修/敵種/屬性/時空凝滯全傷）；施放時先掛雷電增益、本體自身吃到乘算；浮字 🌩️（爆擊黃字）、傷害統計來源「雷霆過載」。
- 增益照舊：期間雷電系技能整體傷害 ×(1+值%)＋命中引動連鎖雷鏈（3＋連擊數 次、各 10%）。
- 可調參數：fx JSON 新增 `atkBase`/`atkPer`/`bounces`（Skills.xlsx 潛力列可調；變量定義頁同步補充說明）。
- 文件：game_formula.md §10 雷霆過載列全面改寫；技能說明面板改為攻擊技能格式（傷害%藍字/固定值橘字）。
- 驗證：Node 測試 10/10（含以使用者實際量級魔攻 7.1T 驗算：Lv60 一次施放 ≈9.4Q）；全套回歸 7/7＋4/4＋14/14；--gen 重生四表＋dry apply 語意 0；建置 126 檔通過；瀏覽器主控台 0 錯誤。
- 追加（同日）：**持續轟擊**——從一次性彈射改為「持續時間內每 1 秒轟一輪」（`tickPotentialOverdrive`，掛入野外 fieldTick 與高塔 tick、與聖療逆轉同位置、不受暈眩影響、增益結束即停；高塔輪傷計入 `TOWER.dmgDealt`）。施放當下先轟首輪，其後每秒一輪（8 秒 ≈ 8 輪）。說明/文件同步。驗證：Node 測試 13/13（每秒一輪 ×7~8、輪傷與首發同級、增益結束即停；期望值改由當前定義動態推得——使用者已在 Skills.xlsx 實際調參 atkBase 70000/atkPer 4000/增益 base 100+5每級，新欄位管線首次實戰回寫成功）。

## 調整：技能結算補齊到與普攻同規格（2026-07-22）

- 背景：追查雷霆過載傷害偏低時發現，普攻（playerAtkCfg）獨享三項技能（castSkill）沒吃到的加成——裝備附魔固定值元素攻擊（st.elemAtk）、神鑄破甲（sunder）、真傷被動（trueDmgPct）。使用者定調全部技能補齊。
- castSkill 傷害段：合併裝備固定值元素攻擊進技能的元素附加表（與技能自身元素占比疊加）；aCfg 補 sunder 與 trueDmgPct（真傷以技能傷害基底計）。
- 雷霆過載連動：雷電系技能於 buff 期間，裝備固定值「雷電」附傷同步 ×(1+值%)（技能自身占比已隨 baseVal 乘過，不重複乘）。
- 文件：game_formula.md 技能傷害表新增「技能結算加成」列。
- 驗證：Node 測試 4/4——固定火附傷精確 +4e12（2hits×2目標）、雷電附傷隨 buff 乘算、真傷/破甲皆生效；雷霆過載回歸 7/7；建置 125 檔通過；瀏覽器主控台 0 錯誤。

## 修正：雷霆過載傷害加成改整體乘算＋連鎖浮字爆擊黃字（2026-07-22）

- 傷害：雷霆過載的 +值% 原本只加成技能的「雷電占比」部分（如連鎖閃電占比 40%，+100% 實際只 +40%）。改為雷電系技能（fx.elem=lightning 或融合技含 lightning）於 castSkill 的 baseVal 直接乘 (1＋值%)——先吃滿物攻/魔攻、混沌雙修、昇華倍率等，之後照常吃爆擊、時空凝滯全傷、元素附加等後續加成；連鎖閃電每跳 10% 也按放大後的最終傷害計算。
- 浮字：連鎖閃電彈跳浮字原為無樣式白字，改沿用一般技能規則——本擊爆擊時顯示「⚡爆擊 值」黃字（crit enemy-skill）、未爆擊顯示「⚡值」（dmg enemy-skill）；聖療逆轉溢傷浮字同步補上 dmg 樣式。必殺一擊本就是黃字不變。
- 同步：技能說明改「使雷電系技能的整體傷害額外提高 X%（吃滿所有加成後再乘算）」；game_formula.md §10 詮釋備註更新。
- 辨識度：連鎖浮字前綴由 ⚡ 改 🌩️——與「⚡天罰」（神鑄特效，250% 物攻真傷）及連鎖閃電技能本體的 ⚡ 區隔（法系玩家物攻低時天罰數字很小，極易被誤認為潛力技能傷害）；連鎖傷害並列入傷害統計（來源「雷霆過載·連鎖」）。
- 驗證：Node 施放測試 7/7——+100% buff 實測總傷 ×1.975（含 ±5% 亂數）、連鎖 3 跳每跳＝總傷 10%、爆擊/未爆擊浮字樣式與前綴皆正確；建置 125 檔通過；隔離瀏覽器主控台 0 錯誤。
...
- ⚠️ 上述差異已由 `config/CSV/game_parameters.csv:7` 取代並完成套用；現值為 Lv.1 且 1 轉解鎖。
...
- **修正**（`js/skills.js`）：新增執行期 `_skillReadyOrder`／`_skillReadySeq`。初次同時就緒依裝載順序；技能冷卻降至 0 時以序號加入佇列尾端；施放選擇以就緒先後排序，裝載順序只作同時就緒的平手判定。狀態不入存檔，重新開始戰鬥時自然重建。
- **測試**：`tests/skill-gcd.test.cjs` 新增 16 個傷害技能輪轉測試，確認首輪不會因第一個短 CD 技能重複施放而跳過後排技能；`skill-gcd`＋`summary` 7/7 通過，`npm run build` 123 檔通過。
- **統計界面結論**：未修改界面筆數限制；`#stats-scroll` 本來就可捲動，且所有統計技能桶均會被輸出。畫面中看到的 10～11 筆是實際被施放並記錄的技能數量。
- **完整測試**：全套測試仍有既有基線失敗（涉及參數表／近期既有 UI 與數值變更），本次新增與技能統計相關測試無失敗。

## 移除：熔爐自動「寶石升階」三合一功能（2026-07-20）

- 需求：刪除「熔爐自動三合一寶石」功能，連同寶石庫存分頁的界面說明與勾選框一併移除。此功能與手動「寶石合成」（2 顆同種同級 → 下一階）無關，後者保留。
- 移除範圍（`gemMerge` 開關 / `#gem-merge-toggle`）：
  - `index.html`：刪除寶石庫存分頁「⚙️ 熔爐自動『寶石升階』：3 顆同種同級 → 1 顆高一級」勾選框與說明。
  - `js/factory.js` `synthTick()`：刪除 `if (f.synth.gemMerge){…}` 自動三合一整段。
  - `js/ui.js`：刪除 (1) `gem-merge-toggle` 勾選狀態同步、(2) 寶石轉換結果尾端「自動化開啟中」提示行、(3) toggle 的 change 事件監聽。
  - `js/player.js`：`synth` 預設物件移除 `gemMerge: true`。
- 備註：此功能原本即因 `SYNTHESIS_ENABLED=false` 未實際運作（合成節點停用），本次為介面與死碼一併清除。舊存檔殘留的 `synth.gemMerge` 屬性已無引用、無害，不需遷移。
- 文件同步：`game_formula.md` §8.3 標題改為「寶石合成（手動）」並刪除「寶石升階」列、加註移除說明。
- 未動：`config` 參數表第 203 列（8-寶石／寶石升階，變動欄=0 未標「刪除」）與 `ui_head.js`（未被 index.html 載入的死檔）——見對話回報。
- 驗證：`npm run build` 全綠（123 檔 OK）；全域 grep `gemMerge` / `gem-merge-toggle` 在已載入的 `js/*.js`、`index.html` 中 0 殘留。
...
- ⚠️ 此歷史門檻已由 `config/CSV/game_parameters.csv:7` 取代；現值為 Lv.1 且 1 轉解鎖。
...
## 摘要
技能標籤（`tags[0]`）即傷害屬性：帶標籤的傷害技能，整段傷害皆為該屬性傷害。
魔攻／物攻只是加成基礎，不再有「純魔法段＋元素附加段」的混用結構。

新結算順序（`resolveHit`，§3.2 步驟 2.5）：
`本體傷害 →(防禦/物魔抗)→ ×元素抗性 ×(1+屬性傷害提升%) → ±浮動 → ×(1+爆傷%) → ×(1+對敵種類%) → ×(1+對屬性敵人%) → ×(1+總傷%)`

## 變更
- `js/formula.js`
  - 新增 `skillElemMixOf(aCfg)`：解析 `aCfg.skillElem`／`skillElemMix`（權重自動正規化為合計 1）。
  - `resolveHit` 新增「技能屬性化」段，位於防禦/物魔抗之後、浮動與暴擊之前；支援 `skillElemAmp`（元素領域每系獨立乘區）。
  - 元素特效改為「技能屬性化段 ＋ 元素附加段」該系合計後**只判定一次**（同屬性不再重複觸發）。
  - 新增 `ELEM_PROC` 常數集中冰／雷／毒／光／暗特效的機率與係數。
- `js/skills.js`
  - `SKILLS`／`UNLOCKS` 移除全部 30 筆 `fx.elem:{type,portion}`。
  - 新增 `skillElemApplyACfg(aCfg, sk, fx)`；接到四條傷害路徑：`castSkill`、`skillRtSimpleCast`（回響／引動／重演／幻影連鋒）、`skillRtOpenField`（領域跳傷）。
  - `skillElemOf` 改為 `elemOverride` → `tags[0]` → `fx.elems` 最大權重。
  - `skillRtFieldAmpACfg` 的元素領域改為同時放大技能屬性化本體段。
  - `fusionAggregateFx`：`fx.elems` 語意改為「屬性權重」（每個帶標籤素材各一票，正規化為合計 1）。
  - `describeSkill`：「造成 X% 魔攻 的火屬性傷害」＋不帶比例的屬性風味語句；融合技列出權重；移除「昇華／佔 N%」句型。
- `js/legendary.js`
  - `legendaryDamageCfg`：帶屬性的特效傷害改走 `skillElem`（吃得到暴擊），穿透跟隨實際結算類型。
  - 死亡領域改用 `fx.elemOverride`，移除 `forceFullElement`。
- `js/potential.js`：雷霆過載本體改用 `skillElem: 'lightning'`（整段電屬性）。
- `tools/config_tables.cjs`：Skills 表術語表更新（標籤＝傷害屬性、`elems` 為權重、新增 `elemOverride`）；`config/CSV|Excel/Skills.*` 已 `--gen` 重生。
- `tests/skill-element-damage.test.cjs`：新增 9 項回歸測試。
- `tests/legendary-affix.test.cjs`：死亡領域斷言改為 `elemOverride`。
- `game_formula.md`：§3.2 / §3.3 / §6 傳奇 / §9.2 / §9.3 / §9.6 同步。

## 數值行為變更（預期內）
1. 舊制的元素段位於暴擊之後 → **吃不到爆擊**；新制屬性傷害在暴擊之前結算 → **吃得到爆擊**。
2. 舊制元素段只吃元素抗性；新制屬性傷害先吃魔防／魔抗，再吃元素抗性。
3. 元素特效（連鎖電擊／中毒／暗影汲取）的作用基數由「元素附加值」放大為「整段屬性傷害」。

## 存檔相容
不需要一次性遷移：`SKILLS` 為靜態資料；帶素材記錄的融合技每次動態重建；舊融合快照的 `fx.elems`（占比合計 <1）由 `skillElemMixOf` 自動正規化。
