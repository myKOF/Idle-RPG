...
## 300 級怪物 0 防禦（DEF=0）高血量單波實測（打到死為止）全 8 大技能群組基準重測（Antigravity 2026-08-21）

- **測試設定調整**：
  - 依使用者指示調整為 **300 級怪物（沼澤 Swamp Stage 300），防禦降為 0（DEF=0, MDEF=0）**，血量達千億至兆級（小怪 ~1,776 億、菁英 ~7,105 億、BOSS ~2.66 兆），完全排除防禦減傷曲線對各傷害類型之影響。
  - **單波出怪（1 波打到死為止）**：小怪 20 隻/波、菁英 5 隻/波、BOSS 1 隻/波，全部由戰場外圍生成逼近，打到全滅為止。
  - 全域公平基準屬性維持：格擋率 50%、格擋減傷 80%、連擊數 3、全屬性增傷害 +1000%、暴擊率 100% 滿暴。
  - 補齊持續傷害（DoT）輸出追蹤，涵蓋萬毒血霧、流血、燃燒跳傷。
- **全量 96 場實測結果更新**：
